...
! Auto-generated at 2026-04-28 18:43:09.601959 by ops-translator
SUBROUTINE chemin
  ...
  END DO
    ...
    DO jspec = 1, nspec
      ...
      DO itint = 1, ntint(ispec)
        READ(ncchem, '(2(1PE12.4),I5)') tintlo(itint, ispec), tinthi(itint, ispec), ncofcp(itint, ispec)
    ...
    DO jstep = 1, nstep
      ...
      DO jspec = 1, nrclen(istep)
        READ(ncchem, '(2I5,I8,1PE12.4)') istep, ispec, nrcpec(ispec, istep), crspec(ispec, istep)
      END DO
    END DO
    ...
    DO jstep = 1, nstep
      ...
      DO jspec = 1, npclen(istep)
        READ(ncchem, '(2I5,I8,1PE12.4)') istep, ispec, npcpec(ispec, istep), cpspec(ispec, istep)
      END DO
    END DO
  ...
  END DO
    ...
    !       VM: sign change based on Khalil
  ...
  END DO
    ...
    DO itint = 1, ntint(ispec)
      ...
      END DO
      amasch(ncenth(itint, ispec), itint, ispec) = amascp(ncenth(itint, ispec), itint, ispec)
  ...
  END DO
    ...
    DO itint = 1, ntint(ispec)
      ...
      amascs(ncenpy(itint, ispec), itint, ispec) = amascp(ncenpy(itint, ispec), itint, ispec)
    END DO
  END DO
  ...
  DO ispec = 1, nspec
    DO itint = 1, ntint(ispec)
      amolgb(1, itint, ispec) = amolcp(ncenpy(itint, ispec), itint, ispec) - amolcp(1, itint, ispec) + giblet
      DO icp = 2, ncpoly(itint, ispec)
        amolgb(icp, itint, ispec) = amolcp(icp, itint, ispec) / REAL(icp * (icp - 1), kind = 8)
      END DO
      amolgb(ncenth(itint, ispec), itint, ispec) = amolcp(ncenth(itint, ispec), itint, ispec)
...
!$OMP TARGET UPDATE TO(rlamda)
!$OMP TARGET UPDATE TO(alamda)
#endif
