...
! Auto-generated at 2026-04-28 18:43:09.200573 by ops-translator
SUBROUTINE rhsvel

  USE ops_fortran_declarations
  USE ops_fortran_rt_support
  USE maths_kernel_equ_fused_module
  USE boundary_kernel_velcomp_xdir_module
  USE boundary_kernel_velcomp_ydir_module
  USE boundary_kernel_velcomp_zdir_module
  USE boundary_kernel_eqa_xdir_module
  USE boundary_kernel_eqa_ydir_module
  USE boundary_kernel_eqa_zdir_module
  ...
  !                    ops_arg_dat(d_drhs, 1, s3d_000, "real(kind=8)", OPS_READ))
  CALL maths_kernel_equ_fused_host("A = B/C", senga_grid, 3, rangexyz, &
ops_arg_dat(d_utmp, 1, s3d_000, "real(kind=8)", OPS_WRITE), &
ops_arg_dat(d_vtmp, 1, s3d_000, "real(kind=8)", OPS_WRITE), &
ops_arg_dat(d_wtmp, 1, s3d_000, "real(kind=8)", OPS_WRITE), &
ops_arg_dat(d_urhs, 1, s3d_000, "real(kind=8)", OPS_READ), &
...
ops_arg_dat(d_store2, 1, s3d_000, "real(kind=8)", OPS_READ))
  ...
  CALL dfbydz(d_vtmp, d_store2)
    ...
    IF (fxlcnv) THEN
    rangexyz = [1, 1, 1, nyglbl, 1, nzglbl]
    CALL boundary_kernel_eqa_xdir_host("COLLECT VELOCITY COMPONENTS FOR BCs", senga_grid, 3, rangexyz, &
...
ops_arg_dat(d_store2, 1, s3d_000, "real(kind=8)", OPS_READ))
  ...
  CALL dfbydz(d_wtmp, d_store2)
    ...
    IF (fylcnv) THEN
    rangexyz = [1, nxglbl, 1, 1, 1, nzglbl]
    CALL boundary_kernel_eqa_ydir_host("COLLECT VELOCITY COMPONENTS FOR BCs", senga_grid, 3, rangexyz, &
...
ops_arg_dat(d_store2, 1, s3d_000, "real(kind=8)", OPS_READ))
  ...
  CALL dfbydy(d_utmp, d_store2)
    ...
    IF (fzlcnv) THEN
    rangexyz = [1, nxglbl, 1, nyglbl, 1, 1]
    CALL boundary_kernel_eqa_zdir_host("COLLECT VELOCITY COMPONENTS FOR BCs", senga_grid, 3, rangexyz, &
