! Auto-generated at 2024-04-21 01:07:44.778016 by ops-translator
...
#define OPS_ACC_MD1(d,x,y,z) ((x) + (xdim1*(y)) + (xdim1*ydim1*(z)) + (xdim1*ydim1*zdim1*(d-1)) + 1)

    INTEGER(KIND=4) :: multi_d2
    INTEGER(KIND=4) :: xdim2
    INTEGER(KIND=4) :: ydim2
    INTEGER(KIND=4) :: zdim2
#define OPS_ACC_MD2(d,x,y,z) ((x) + (xdim2*(y)) + (xdim2*ydim2*(z)) + (xdim2*ydim2*zdim2*(d-1)) + 1)
...
!DEC$ ATTRIBUTES FORCEINLINE :: temper_kernel_eqB
...
SUBROUTINE temper_kernel_eqB_wrap( &
    ...
    INTEGER(KIND=4), DIMENSION(*), INTENT(OUT) :: opsDat3Local
...
!   ==========================
    ...
    multi_d1 = getDatDimFromOpsArg(opsArg1) ! dimension of the multi-dim dat
    ...
    multi_d2 = getDatDimFromOpsArg(opsArg2) ! dimension of the multi-dim dat
