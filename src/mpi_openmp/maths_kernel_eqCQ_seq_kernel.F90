! Auto-generated at 2024-04-21 01:07:44.852914 by ops-translator

MODULE MATHS_KERNEL_EQCQ_MODULE
...
!DEC$ ATTRIBUTES FORCEINLINE :: maths_kernel_eqCQ
SUBROUTINE maths_kernel_eqCQ(dvtgvdx, dwtgvdx, dutgvdy, dwtgvdy, dutgvdz, dvtgvdz, drun, deltagx, deltagy, deltagz, enstro)

    real(kind=8), dimension(1), intent(in) :: dvtgvdx, dwtgvdx, dutgvdy, dwtgvdy, dutgvdz, dvtgvdz, drun
    real(kind=8), intent(in) :: deltagx, deltagy, deltagz
    real(kind=8) :: enstro

    real(kind=8) :: vort1, vort2, vort3, enstrs

    vort1 = dwtgvdy(OPS_ACC4(0,0,0))-dvtgvdz(OPS_ACC6(0,0,0))
    vort2 = dutgvdz(OPS_ACC5(0,0,0))-dwtgvdx(OPS_ACC2(0,0,0))
    vort3 = dvtgvdx(OPS_ACC1(0,0,0))-dutgvdy(OPS_ACC3(0,0,0))
    enstrs = vort1*vort1+vort2*vort2+vort3*vort3
    enstro = enstro+0.5_8*drun(OPS_ACC7(0,0,0))*enstrs*deltagx*deltagy*deltagz
...
SUBROUTINE maths_kernel_eqCQ_wrap( &
    opsDat1Local, &
    opsDat2Local, &
    opsDat3Local, &
    opsDat4Local, &
    ...
    REAL(KIND=8), DIMENSION(1), INTENT(INOUT) :: opsDat11Local
...
!   ===============
#ifndef OPS_LAZY
SUBROUTINE maths_kernel_eqCQ_host( userSubroutine, block, dim, range, &
    opsArg1, &
    opsArg2, &
    opsArg3, &
    opsArg4, &
...
#else
SUBROUTINE maths_kernel_eqCQ_host_execute( descPtr )
...
!   ==========================
    ...
    CALL c_f_pointer(getReductionPtrFromOpsArg(opsArg11,block), opsDat11Local, (/opsArg11%dim/))
    dat11_base = 1
...
SUBROUTINE maths_kernel_eqCQ_host( userSubroutine, block, dim, range, &
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    11, 549, dim, 0, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(maths_kernel_eqCQ_host_execute))

END SUBROUTINE
#endif

END MODULE MATHS_KERNEL_EQCQ_MODULE
