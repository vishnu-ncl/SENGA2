// Auto-generated at 2026-04-28 18:44:27.802133 by ops-translator

__constant__ int dims_turbin_kernel_eqC[3][3];
static int dims_turbin_kernel_eqC_h[3][3] = {{0}};

//  =============
//  User function
//  =============
__device__ void turbin_kernel_eqC_gpu(const ACC<double> &in_arr, const double *val1, double *reduction_var) {
...
//  ============================
__global__ void ops_turbin_kernel_eqC(double* __restrict arg0, int xstride_0, int ystride_0, int zstride_0, 
const double arg1, 
...
int size0, int size1, int size2) {
    ...
    arg0 += idx_x * xstride_0 + idx_y * ystride_0 * dims_turbin_kernel_eqC[0][0] + idx_z * zstride_0 * dims_turbin_kernel_eqC[0][0] * dims_turbin_kernel_eqC[0][1];

    if(idx_x < size0 && idx_y < size1 && idx_z < size2) {

        const ACC<double> argp0(dims_turbin_kernel_eqC[0][0], dims_turbin_kernel_eqC[0][1], arg0);

        turbin_kernel_eqC_gpu(argp0, &arg1, arg2_l);
...
//  ==============================
    for(int d = 0; d < 1; d++)
        ops_reduction_hip<OPS_INC>(&arg2[d+(blockIdx.x + blockIdx.y*gridDim.x + blockIdx.z*gridDim.x*gridDim.y)*1],arg2_l[d]);
...
//  ======================================================
    ...
    if (xdim0 != dims_turbin_kernel_eqC_h[0][0] || ydim0 != dims_turbin_kernel_eqC_h[0][1] || zdim0 != dims_turbin_kernel_eqC_h[0][2]) {
        dims_turbin_kernel_eqC_h[0][0] = xdim0;
        dims_turbin_kernel_eqC_h[0][1] = ydim0;
        dims_turbin_kernel_eqC_h[0][2] = zdim0;

        hipSafeCall(block->instance->ostream(), hipMemcpyToSymbol( dims_turbin_kernel_eqC, dims_turbin_kernel_eqC_h, sizeof(dims_turbin_kernel_eqC)));
    }
...
//  call kernel wrapper function, passing in pointers to data
    if (x_size > 0 && y_size > 0 && z_size > 0) {
        ...
        ops_turbin_kernel_eqC<<<grid, tblock, nshared >>> (
