...
! Auto-generated at 2026-04-28 18:43:09.574369 by ops-translator
SUBROUTINE indata
  ...
  USE maths_kernel_eqv_fused_module
  ...
  PARAMETER(pncont = 'input/cont', pnchem = 'input/chem', pndiff = 'input/diff', pnradn = 'input/radn', pnrept = 'output/rept', pnstat = 'output/stat', pndmpi = 'output/dmpi', pndmpo = 'output/dmpo', pnxdat = '.dat', pnxres = '.res')
...
ops_arg_dat(d_yinf2(ispec), 1, s3d_000_strid3d_yz, "real(kind=8)", OPS_WRITE))
  ...
  END IF
    ...
    IF (ABS(fornow - one) > ytoler) THEN
      WRITE(ncrept, *) 'Warning: INDATA: initial mass fractions do not sum to unity'
      WRITE(ncrept, *)
    END IF
  ...
  CALL chemin
    ...
    WRITE(ncrept, *) ' Spec.  No of T intervals   ', 'Interval   T low       T high       No of coeffs'
    DO ispec = 1, nspec
      ic = 1
      WRITE(ncrept, '(I5,6X,I5,9X,I5,8X,2(1PE12.4),I8)') ispec, ntint(ispec), ic, tintlo(ic, ispec), tinthi(ic, ispec), ncofcp(ic, ispec)
      DO ic = 2, ntint(ispec)
        WRITE(ncrept, '(25X,I5,8X,2(1PE12.4),I8)') ic, tintlo(ic, ispec), tinthi(ic, ispec), ncofcp(ic, ispec)
    ...
    DO ispec = 1, nspec
      DO ic = 1, ntint(ispec)
        jc = 1
        WRITE(ncrept, '(I5,2X,I5,5X,I5,4X,1PE15.7)') ispec, ic, jc, amascp(jc, ic, ispec)
    ...
    WRITE(ncrept, *) ' Spec.  T int.  Temp.       Cp', '          Enthalpy    Entropy     Molar Gibbs'
    DO ispec = 1, nspec
      DO ic = 1, ntint(ispec)
        DO icp = 1, 2
          ...
          END DO
          fornow = amascs(ncenpy(ic, ispec), ic, ispec) + fornow * ttemp(1) + amascs(1, ic, ispec) * LOG(ttemp(1))
          ttemp(4) = fornow
          ...
          END DO
          fornow = amolgb(ncenth(ic, ispec), ic, ispec) / ttemp(1) - amolgb(ncenpy(ic, ispec), ic, ispec) * LOG(ttemp(1)) - fornow
          ttemp(5) = fornow

            IF (icp == 1) THEN
            IF (ic == 1) THEN
              WRITE(ncrept, '(I5,2X,I5,X,"l",X,5(1PE12.4))') ispec, ic, (ttemp(jc), jc = 1, 5)
            ELSE
    ...
    END DO
      ...
      DO istep = 1, nstep
        IF (mllist(istep) /= 0) THEN
          WRITE(ncrept, '(I5,3(2X,1PE12.4))') istep, (rclind(icp, mllist(istep)), icp = 1, 3)
        END IF
      END DO
    END IF
    ...
    DO jspec = 1, nbody
      ...
      DO ispec = 1, nspec
        WRITE(ncrept, '(I5,3X,A10,1PE12.4)') ispec, spcsym(ispec), effy3b(ispec, jspec)
      END DO
    END DO
  ...
  CALL diffin
    ...
    IF (flmavt) THEN
      ...
      DO ispec = 1, nspec
        ...
        DO jspec = 1, ispec
          WRITE(ncrept, '(I5,5(1PE15.7))') jspec, (diffco(icoeff, jspec, ispec), icoeff = 1, ncodif)
        END DO
      END DO
      WRITE(ncrept, *)
      ...
      DO ispec = 1, nspec
        ...
        DO jspec = 1, ispec - 1
          WRITE(ncrept, '(I5,5(1PE15.7))') jspec, (tdrcco(icoeff, jspec, ispec), icoeff = 1, ncotdr)
        END DO
      END DO
      ...
      WRITE(ncrept, *)
      WRITE(ncrept, *) '         Diffusivity       Schmidt No', '        Lewis No          Thermal diff'
      DO ispec = 1, nspec
        WRITE(ncrept, '(I5,4(1PE18.7))') ispec, ctrans(ispec, 3), ttemp(1) / ctrans(ispec, 3), ttemp(2) / (ctrans(ispec, 3) * ttemp(3)), ctrans(ispec, 4)
      END DO
...
ops_arg_dat(d_wrun, 1, s3d_000, "real(kind=8)", OPS_READ))
  ...
  CALL maths_kernel_eqv_fused_host("A = A*B", senga_grid, 3, rangexyz, &
...
9200 FORMAT(12I6)
9250 FORMAT(6I7)
9270 FORMAT(2I5, 5(1P, E12.4))
9300 FORMAT(1P, E12.4, 6I8)
9400 FORMAT(6(1P, E12.4))
9450 FORMAT(I7, 1P, E15.7)
