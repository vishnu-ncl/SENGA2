! Auto-generated at 2026-04-28 18:44:33.074183 by ops-translator
...
!$OMP DECLARE TARGET(xdim1_turbin_kernel_eqC)
    INTEGER(KIND=4) :: xdim1_turbin_kernel_eqC
    INTEGER(KIND=4) :: xdim1_turbin_kernel_eqC_h = -1
!$OMP DECLARE TARGET(ydim1_turbin_kernel_eqC)
    INTEGER(KIND=4) :: ydim1_turbin_kernel_eqC
...
SUBROUTINE turbin_kernel_eqC_wrap( &
    opsDat1Local, &
    opsGblDat2Device, &
    opsDat3Local, &
    dat1_base, &
    ...
    DO n_z = 1, end_indx_3-start_indx_3+1
        DO n_y = 1, end_indx_2-start_indx_2+1
            DO n_x = 1, end_indx_1-start_indx_1+1
                ...
                CALL turbin_kernel_eqC( &
                opsDat1Local(dat1_base + ((n_x-1)*1) + ((n_y-1)*xdim1_turbin_kernel_eqC*1) + ((n_z-1)*ydim1_turbin_kernel_eqC*xdim1_turbin_kernel_eqC*1)), &
                opsGblDat2Device, &
                reduction_var &
               )
...
!$OMP END TARGET TEAMS LOOP
    ...
    opsDat3Local(dat3_base) = opsDat3Local(dat3_base) + reduction_var
...
!   ===============
#ifndef OPS_LAZY
SUBROUTINE turbin_kernel_eqC_host( userSubroutine, block, dim, range, &
    opsArg1, &
    opsArg2, &
    opsArg3 &
    )
...
#else
SUBROUTINE turbin_kernel_eqC_host_execute( descPtr )
...
!   ==========================
    ...
    CALL setKernelTime(557, kernelName//c_null_char, 0.0_8, 0.0_8, 0.0_4, 1)
    ...
    CALL c_f_pointer(getReductionPtrFromOpsArg(opsArg3,block), opsDat3Local, [opsDat3Cardinality])
    dat3_base = 1

    IF (&
         (xdim1 .NE. xdim1_turbin_kernel_eqC_h) .OR. &
         (ydim1 .NE. ydim1_turbin_kernel_eqC_h) &
       ) THEN
            xdim1_turbin_kernel_eqC = xdim1
            xdim1_turbin_kernel_eqC_h = xdim1
!$OMP TARGET UPDATE TO(xdim1_turbin_kernel_eqC)
            ydim1_turbin_kernel_eqC = ydim1
            ydim1_turbin_kernel_eqC_h = ydim1
!$OMP TARGET UPDATE TO(ydim1_turbin_kernel_eqC)
    END IF
...
!   ========================
    ...
    CALL setKernelTime(557, kernelName//c_null_char, t3__-t2__, t2__-t1__, transfer_total, 0)
...
SUBROUTINE turbin_kernel_eqC_host( userSubroutine, block, dim, range, &
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    3, 557, dim, 1, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(turbin_kernel_eqC_host_execute))

END SUBROUTINE
#endif

END MODULE TURBIN_KERNEL_EQC_MODULE
