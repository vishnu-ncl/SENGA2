! Auto-generated at 2026-04-28 18:44:30.586962 by ops-translator

MODULE LINCOM_KERNEL_EQD_MODULE
...
!$OMP DECLARE TARGET(xdim1_lincom_kernel_eqD)
    INTEGER(KIND=4) :: xdim1_lincom_kernel_eqD
    INTEGER(KIND=4) :: xdim1_lincom_kernel_eqD_h = -1
!$OMP DECLARE TARGET(ydim1_lincom_kernel_eqD)
    INTEGER(KIND=4) :: ydim1_lincom_kernel_eqD
    INTEGER(KIND=4) :: ydim1_lincom_kernel_eqD_h = -1
#define OPS_ACC1(x,y,z) ((x) + (xdim1_lincom_kernel_eqD*(y)) + (xdim1_lincom_kernel_eqD*ydim1_lincom_kernel_eqD*(z)) + 1)

!$OMP DECLARE TARGET(xdim2_lincom_kernel_eqD)
    INTEGER(KIND=4) :: xdim2_lincom_kernel_eqD
    INTEGER(KIND=4) :: xdim2_lincom_kernel_eqD_h = -1
!$OMP DECLARE TARGET(ydim2_lincom_kernel_eqD)
    INTEGER(KIND=4) :: ydim2_lincom_kernel_eqD
    INTEGER(KIND=4) :: ydim2_lincom_kernel_eqD_h = -1
#define OPS_ACC2(x,y,z) ((x) + (xdim2_lincom_kernel_eqD*(y)) + (xdim2_lincom_kernel_eqD*ydim2_lincom_kernel_eqD*(z)) + 1)

!$OMP DECLARE TARGET(xdim3_lincom_kernel_eqD)
    INTEGER(KIND=4) :: xdim3_lincom_kernel_eqD
    INTEGER(KIND=4) :: xdim3_lincom_kernel_eqD_h = -1
!$OMP DECLARE TARGET(ydim3_lincom_kernel_eqD)
    INTEGER(KIND=4) :: ydim3_lincom_kernel_eqD
    INTEGER(KIND=4) :: ydim3_lincom_kernel_eqD_h = -1
#define OPS_ACC3(x,y,z) ((x) + (xdim3_lincom_kernel_eqD*(y)) + (xdim3_lincom_kernel_eqD*ydim3_lincom_kernel_eqD*(z)) + 1)
...
SUBROUTINE lincom_kernel_eqD_wrap( &
    opsDat1Local, &
    opsDat2Local, &
    opsDat3Local, &
    dat1_base, &
    ...
    DO n_z = 1, end_indx_3-start_indx_3+1
        DO n_y = 1, end_indx_2-start_indx_2+1
            DO n_x = 1, end_indx_1-start_indx_1+1
                ...
                CALL lincom_kernel_eqD( &
                opsDat1Local(dat1_base + ((n_x-1)*1) + ((n_y-1)*xdim1_lincom_kernel_eqD*1) + ((n_z-1)*ydim1_lincom_kernel_eqD*xdim1_lincom_kernel_eqD*1)), &
                opsDat2Local(dat2_base + ((n_x-1)*1) + ((n_y-1)*xdim2_lincom_kernel_eqD*1) + ((n_z-1)*ydim2_lincom_kernel_eqD*xdim2_lincom_kernel_eqD*1)), &
                opsDat3Local(dat3_base + ((n_x-1)*1) + ((n_y-1)*xdim3_lincom_kernel_eqD*1) + ((n_z-1)*ydim3_lincom_kernel_eqD*xdim3_lincom_kernel_eqD*1)) &
               )
...
!   ===============
#ifndef OPS_LAZY
SUBROUTINE lincom_kernel_eqD_host( userSubroutine, block, dim, range, &
    opsArg1, &
    opsArg2, &
    opsArg3 &
    )
...
#else
SUBROUTINE lincom_kernel_eqD_host_execute( descPtr )
...
!   ==========================
    ...
    CALL setKernelTime(311, kernelName//c_null_char, 0.0_8, 0.0_8, 0.0_4, 1)
    ...
    IF (&
         (xdim1 .NE. xdim1_lincom_kernel_eqD_h) .OR. &
         (ydim1 .NE. ydim1_lincom_kernel_eqD_h) .OR. &
         (xdim2 .NE. xdim2_lincom_kernel_eqD_h) .OR. &
         (ydim2 .NE. ydim2_lincom_kernel_eqD_h) .OR. &
         (xdim3 .NE. xdim3_lincom_kernel_eqD_h) .OR. &
         (ydim3 .NE. ydim3_lincom_kernel_eqD_h) &
       ) THEN
            xdim1_lincom_kernel_eqD = xdim1
            xdim1_lincom_kernel_eqD_h = xdim1
!$OMP TARGET UPDATE TO(xdim1_lincom_kernel_eqD)
            ydim1_lincom_kernel_eqD = ydim1
            ydim1_lincom_kernel_eqD_h = ydim1
!$OMP TARGET UPDATE TO(ydim1_lincom_kernel_eqD)
            xdim2_lincom_kernel_eqD = xdim2
            xdim2_lincom_kernel_eqD_h = xdim2
!$OMP TARGET UPDATE TO(xdim2_lincom_kernel_eqD)
            ydim2_lincom_kernel_eqD = ydim2
            ydim2_lincom_kernel_eqD_h = ydim2
!$OMP TARGET UPDATE TO(ydim2_lincom_kernel_eqD)
            xdim3_lincom_kernel_eqD = xdim3
            xdim3_lincom_kernel_eqD_h = xdim3
!$OMP TARGET UPDATE TO(xdim3_lincom_kernel_eqD)
            ydim3_lincom_kernel_eqD = ydim3
            ydim3_lincom_kernel_eqD_h = ydim3
!$OMP TARGET UPDATE TO(ydim3_lincom_kernel_eqD)
    END IF
...
!   ========================
    ...
    CALL setKernelTime(311, kernelName//c_null_char, t3__-t2__, t2__-t1__, transfer_total, 0)
...
SUBROUTINE lincom_kernel_eqD_host( userSubroutine, block, dim, range, &
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    3, 311, dim, 1, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(lincom_kernel_eqD_host_execute))

END SUBROUTINE
#endif

END MODULE LINCOM_KERNEL_EQD_MODULE
