! Auto-generated at 2026-04-28 18:44:32.393591 by ops-translator

MODULE BOUNDS_KERNEL_EQO_ZL_MODULE
...
!$OMP DECLARE TARGET(xdim1_bounds_kernel_eqO_zl)
    INTEGER(KIND=4) :: xdim1_bounds_kernel_eqO_zl
    INTEGER(KIND=4) :: xdim1_bounds_kernel_eqO_zl_h = -1
!$OMP DECLARE TARGET(ydim1_bounds_kernel_eqO_zl)
    INTEGER(KIND=4) :: ydim1_bounds_kernel_eqO_zl
    INTEGER(KIND=4) :: ydim1_bounds_kernel_eqO_zl_h = -1
#define OPS_ACC1(x,y,z) ((x) + (xdim1_bounds_kernel_eqO_zl*(y)) + (xdim1_bounds_kernel_eqO_zl*ydim1_bounds_kernel_eqO_zl*(z)) + 1)
...
SUBROUTINE bounds_kernel_eqO_zl(bclyzl)

    real(kind=8), dimension(1) ::  bclyzl

    bclyzl(OPS_ACC1(0,0,0)) = 0.0_8 

END SUBROUTINE

#undef OPS_ACC1

SUBROUTINE bounds_kernel_eqO_zl_wrap( &
    opsDat1Local, &
    dat1_base, &
    dat1_dim, &
    start_indx, &
    ...
    DO n_z = 1, end_indx_3-start_indx_3+1
        DO n_y = 1, end_indx_2-start_indx_2+1
            DO n_x = 1, end_indx_1-start_indx_1+1
                ...
                CALL bounds_kernel_eqO_zl( &
                opsDat1Local(dat1_base + ((n_x-1)*1) + ((n_y-1)*xdim1_bounds_kernel_eqO_zl*1) + ((n_z-1)*ydim1_bounds_kernel_eqO_zl*xdim1_bounds_kernel_eqO_zl*0)) &
               )
...
!   ===============
#ifndef OPS_LAZY
SUBROUTINE bounds_kernel_eqO_zl_host( userSubroutine, block, dim, range, &
    opsArg1 &
    )
...
#else
SUBROUTINE bounds_kernel_eqO_zl_host_execute( descPtr )
    ...
    kernelName = "bounds_kernel_eqO_zl"
...
!   ==========================
    ...
    CALL setKernelTime(469, kernelName//c_null_char, 0.0_8, 0.0_8, 0.0_4, 1)
    ...
    IF (&
         (xdim1 .NE. xdim1_bounds_kernel_eqO_zl_h) .OR. &
         (ydim1 .NE. ydim1_bounds_kernel_eqO_zl_h) &
       ) THEN
            xdim1_bounds_kernel_eqO_zl = xdim1
            xdim1_bounds_kernel_eqO_zl_h = xdim1
!$OMP TARGET UPDATE TO(xdim1_bounds_kernel_eqO_zl)
            ydim1_bounds_kernel_eqO_zl = ydim1
            ydim1_bounds_kernel_eqO_zl_h = ydim1
!$OMP TARGET UPDATE TO(ydim1_bounds_kernel_eqO_zl)
    END IF
...
!   ==============================
    CALL bounds_kernel_eqO_zl_wrap( &
                        opsDat1Local, &
                        dat1_base, &
                        opsDat1Cardinality, &
                        start_indx, &
...
!   ========================
    ...
    CALL setKernelTime(469, kernelName//c_null_char, t3__-t2__, t2__-t1__, transfer_total, 0)
...
SUBROUTINE bounds_kernel_eqO_zl_host( userSubroutine, block, dim, range, &
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    1, 469, dim, 1, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(bounds_kernel_eqO_zl_host_execute))

END SUBROUTINE
#endif

END MODULE BOUNDS_KERNEL_EQO_ZL_MODULE
