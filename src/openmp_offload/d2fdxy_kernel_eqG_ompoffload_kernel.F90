! Auto-generated at 2026-04-28 18:44:28.582696 by ops-translator

MODULE D2FDXY_KERNEL_EQG_MODULE
...
!$OMP DECLARE TARGET(xdim1_d2fdxy_kernel_eqG)
    INTEGER(KIND=4) :: xdim1_d2fdxy_kernel_eqG
    INTEGER(KIND=4) :: xdim1_d2fdxy_kernel_eqG_h = -1
!$OMP DECLARE TARGET(ydim1_d2fdxy_kernel_eqG)
    INTEGER(KIND=4) :: ydim1_d2fdxy_kernel_eqG
    INTEGER(KIND=4) :: ydim1_d2fdxy_kernel_eqG_h = -1
#define OPS_ACC1(x,y,z) ((x) + (xdim1_d2fdxy_kernel_eqG*(y)) + (xdim1_d2fdxy_kernel_eqG*ydim1_d2fdxy_kernel_eqG*(z)) + 1)

!$OMP DECLARE TARGET(xdim2_d2fdxy_kernel_eqG)
    INTEGER(KIND=4) :: xdim2_d2fdxy_kernel_eqG
    INTEGER(KIND=4) :: xdim2_d2fdxy_kernel_eqG_h = -1
!$OMP DECLARE TARGET(ydim2_d2fdxy_kernel_eqG)
    INTEGER(KIND=4) :: ydim2_d2fdxy_kernel_eqG
    INTEGER(KIND=4) :: ydim2_d2fdxy_kernel_eqG_h = -1
#define OPS_ACC2(x,y,z) ((x) + (xdim2_d2fdxy_kernel_eqG*(y)) + (xdim2_d2fdxy_kernel_eqG*ydim2_d2fdxy_kernel_eqG*(z)) + 1)
...
SUBROUTINE d2fdxy_kernel_eqG_wrap( &
    opsDat1Local, &
    opsDat2Local, &
    dat1_base, &
    dat2_base, &
    ...
    DO n_z = 1, end_indx_3-start_indx_3+1
        DO n_y = 1, end_indx_2-start_indx_2+1
            DO n_x = 1, end_indx_1-start_indx_1+1
                ...
                CALL d2fdxy_kernel_eqG( &
                opsDat1Local(dat1_base + ((n_x-1)*1) + ((n_y-1)*xdim1_d2fdxy_kernel_eqG*1) + ((n_z-1)*ydim1_d2fdxy_kernel_eqG*xdim1_d2fdxy_kernel_eqG*1)), &
                opsDat2Local(dat2_base + ((n_x-1)*1) + ((n_y-1)*xdim2_d2fdxy_kernel_eqG*1) + ((n_z-1)*ydim2_d2fdxy_kernel_eqG*xdim2_d2fdxy_kernel_eqG*1)) &
               )
...
!   ===============
#ifndef OPS_LAZY
SUBROUTINE d2fdxy_kernel_eqG_host( userSubroutine, block, dim, range, &
    opsArg1, &
    opsArg2 &
    )
...
#else
SUBROUTINE d2fdxy_kernel_eqG_host_execute( descPtr )
...
!   ==========================
    ...
    CALL setKernelTime(24, kernelName//c_null_char, 0.0_8, 0.0_8, 0.0_4, 1)
    ...
    IF (&
         (xdim1 .NE. xdim1_d2fdxy_kernel_eqG_h) .OR. &
         (ydim1 .NE. ydim1_d2fdxy_kernel_eqG_h) .OR. &
         (xdim2 .NE. xdim2_d2fdxy_kernel_eqG_h) .OR. &
         (ydim2 .NE. ydim2_d2fdxy_kernel_eqG_h) &
       ) THEN
            xdim1_d2fdxy_kernel_eqG = xdim1
            xdim1_d2fdxy_kernel_eqG_h = xdim1
!$OMP TARGET UPDATE TO(xdim1_d2fdxy_kernel_eqG)
            ydim1_d2fdxy_kernel_eqG = ydim1
            ydim1_d2fdxy_kernel_eqG_h = ydim1
!$OMP TARGET UPDATE TO(ydim1_d2fdxy_kernel_eqG)
            xdim2_d2fdxy_kernel_eqG = xdim2
            xdim2_d2fdxy_kernel_eqG_h = xdim2
!$OMP TARGET UPDATE TO(xdim2_d2fdxy_kernel_eqG)
            ydim2_d2fdxy_kernel_eqG = ydim2
            ydim2_d2fdxy_kernel_eqG_h = ydim2
!$OMP TARGET UPDATE TO(ydim2_d2fdxy_kernel_eqG)
    END IF
...
!   ========================
    ...
    CALL setKernelTime(24, kernelName//c_null_char, t3__-t2__, t2__-t1__, transfer_total, 0)
...
SUBROUTINE d2fdxy_kernel_eqG_host( userSubroutine, block, dim, range, &
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    2, 24, dim, 1, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(d2fdxy_kernel_eqG_host_execute))

END SUBROUTINE
#endif

END MODULE D2FDXY_KERNEL_EQG_MODULE
