! Auto-generated at 2026-04-28 18:44:33.069347 by ops-translator
...
!$OMP DECLARE TARGET(xdim1_turbin_kernel_eqB)
    INTEGER(KIND=4) :: xdim1_turbin_kernel_eqB
    INTEGER(KIND=4) :: xdim1_turbin_kernel_eqB_h = -1
!$OMP DECLARE TARGET(ydim1_turbin_kernel_eqB)
    INTEGER(KIND=4) :: ydim1_turbin_kernel_eqB
    INTEGER(KIND=4) :: ydim1_turbin_kernel_eqB_h = -1
...
SUBROUTINE turbin_kernel_eqB_wrap( &
    ...
    REAL(KIND=8), DIMENSION(1), INTENT(INOUT) :: opsDat2Local
    REAL(KIND=8) :: reduction_var
...
#ifdef _CRAYFTN
!$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO PRIVATE(n_x,n_y,n_z) REDUCTION(+:reduction_var) 
#else
!$OMP TARGET TEAMS LOOP PRIVATE(n_x,n_y,n_z) REDUCTION(+:reduction_var) 
#endif
    DO n_z = 1, end_indx_3-start_indx_3+1
        DO n_y = 1, end_indx_2-start_indx_2+1
            DO n_x = 1, end_indx_1-start_indx_1+1
                ...
                opsDat1Local(dat1_base + ((n_x-1)*1) + ((n_y-1)*xdim1_turbin_kernel_eqB*1) + ((n_z-1)*ydim1_turbin_kernel_eqB*xdim1_turbin_kernel_eqB*1)), &
                reduction_var &
               )
...
!$OMP END TARGET TEAMS LOOP
    ...
    opsDat2Local(dat2_base) = opsDat2Local(dat2_base) + reduction_var
...
!   ==========================
    ...
    CALL setKernelTime(556, kernelName//c_null_char, 0.0_8, 0.0_8, 0.0_4, 1)
    ...
    CALL c_f_pointer(getReductionPtrFromOpsArg(opsArg2,block), opsDat2Local, [opsDat2Cardinality])
    dat2_base = 1

    IF (&
         (xdim1 .NE. xdim1_turbin_kernel_eqB_h) .OR. &
         (ydim1 .NE. ydim1_turbin_kernel_eqB_h) &
       ) THEN
            xdim1_turbin_kernel_eqB = xdim1
            xdim1_turbin_kernel_eqB_h = xdim1
!$OMP TARGET UPDATE TO(xdim1_turbin_kernel_eqB)
            ydim1_turbin_kernel_eqB = ydim1
            ydim1_turbin_kernel_eqB_h = ydim1
!$OMP TARGET UPDATE TO(ydim1_turbin_kernel_eqB)
    END IF
...
!   ========================
    ...
    CALL setKernelTime(556, kernelName//c_null_char, t3__-t2__, t2__-t1__, transfer_total, 0)
...
SUBROUTINE turbin_kernel_eqB_host( userSubroutine, block, dim, range, &
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    2, 556, dim, 1, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(turbin_kernel_eqB_host_execute))

END SUBROUTINE
#endif

END MODULE TURBIN_KERNEL_EQB_MODULE
