! Auto-generated at 2026-04-28 18:44:30.510275 by ops-translator

MODULE MATHS_KERNEL_EQG_MODULE
...
!$OMP DECLARE TARGET(xdim1_maths_kernel_eqG)
    INTEGER(KIND=4) :: xdim1_maths_kernel_eqG
    INTEGER(KIND=4) :: xdim1_maths_kernel_eqG_h = -1
!$OMP DECLARE TARGET(ydim1_maths_kernel_eqG)
    INTEGER(KIND=4) :: ydim1_maths_kernel_eqG
    INTEGER(KIND=4) :: ydim1_maths_kernel_eqG_h = -1
#define OPS_ACC1(x,y,z) ((x) + (xdim1_maths_kernel_eqG*(y)) + (xdim1_maths_kernel_eqG*ydim1_maths_kernel_eqG*(z)) + 1)
...
SUBROUTINE maths_kernel_eqG_wrap( &
    opsDat1Local, &
    opsGblDat2Device, &
    dat1_base, &
    dat2_base, &
    ...
    REAL(KIND=8), VALUE ::  opsGblDat2Device
    ...
    DO n_z = 1, end_indx_3-start_indx_3+1
        DO n_y = 1, end_indx_2-start_indx_2+1
            DO n_x = 1, end_indx_1-start_indx_1+1
                ...
                CALL maths_kernel_eqG( &
                opsDat1Local(dat1_base + ((n_x-1)*1) + ((n_y-1)*xdim1_maths_kernel_eqG*1) + ((n_z-1)*ydim1_maths_kernel_eqG*xdim1_maths_kernel_eqG*1)), &
                opsGblDat2Device &
               )
...
!   ===============
#ifndef OPS_LAZY
SUBROUTINE maths_kernel_eqG_host( userSubroutine, block, dim, range, &
    opsArg1, &
    opsArg2 &
    )
...
#else
SUBROUTINE maths_kernel_eqG_host_execute( descPtr )
...
!   ==========================
    ...
    CALL setKernelTime(294, kernelName//c_null_char, 0.0_8, 0.0_8, 0.0_4, 1)
    ...
    IF (&
         (xdim1 .NE. xdim1_maths_kernel_eqG_h) .OR. &
         (ydim1 .NE. ydim1_maths_kernel_eqG_h) &
       ) THEN
            xdim1_maths_kernel_eqG = xdim1
            xdim1_maths_kernel_eqG_h = xdim1
!$OMP TARGET UPDATE TO(xdim1_maths_kernel_eqG)
            ydim1_maths_kernel_eqG = ydim1
            ydim1_maths_kernel_eqG_h = ydim1
!$OMP TARGET UPDATE TO(ydim1_maths_kernel_eqG)
    END IF
...
!   ========================
    ...
    CALL setKernelTime(294, kernelName//c_null_char, t3__-t2__, t2__-t1__, transfer_total, 0)
...
SUBROUTINE maths_kernel_eqG_host( userSubroutine, block, dim, range, &
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    2, 294, dim, 1, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(maths_kernel_eqG_host_execute))

END SUBROUTINE
#endif

END MODULE MATHS_KERNEL_EQG_MODULE
