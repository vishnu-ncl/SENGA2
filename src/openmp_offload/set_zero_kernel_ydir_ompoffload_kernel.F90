! Auto-generated at 2026-04-28 18:44:28.512673 by ops-translator

MODULE SET_ZERO_KERNEL_YDIR_MODULE
...
!$OMP DECLARE TARGET(xdim1_set_zero_kernel_ydir)
    INTEGER(KIND=4) :: xdim1_set_zero_kernel_ydir
    INTEGER(KIND=4) :: xdim1_set_zero_kernel_ydir_h = -1
!$OMP DECLARE TARGET(ydim1_set_zero_kernel_ydir)
    INTEGER(KIND=4) :: ydim1_set_zero_kernel_ydir
    INTEGER(KIND=4) :: ydim1_set_zero_kernel_ydir_h = -1
#define OPS_ACC1(x,y,z) ((x) + (xdim1_set_zero_kernel_ydir*(y)) + (xdim1_set_zero_kernel_ydir*ydim1_set_zero_kernel_ydir*(z)) + 1)
...
SUBROUTINE set_zero_kernel_ydir_wrap( &
    opsDat1Local, &
    dat1_base, &
    dat1_dim, &
    start_indx, &
    ...
    DO n_z = 1, end_indx_3-start_indx_3+1
        DO n_y = 1, end_indx_2-start_indx_2+1
            DO n_x = 1, end_indx_1-start_indx_1+1
                ...
                CALL set_zero_kernel_ydir( &
                opsDat1Local(dat1_base + ((n_x-1)*1) + ((n_y-1)*xdim1_set_zero_kernel_ydir*0) + ((n_z-1)*ydim1_set_zero_kernel_ydir*xdim1_set_zero_kernel_ydir*1)) &
               )
...
!   ===============
#ifndef OPS_LAZY
SUBROUTINE set_zero_kernel_ydir_host( userSubroutine, block, dim, range, &
    opsArg1 &
    )
...
#else
SUBROUTINE set_zero_kernel_ydir_host_execute( descPtr )
    ...
    kernelName = "set_zero_kernel_ydir"
...
!   ==========================
    ...
    CALL setKernelTime(3, kernelName//c_null_char, 0.0_8, 0.0_8, 0.0_4, 1)
    ...
    IF (&
         (xdim1 .NE. xdim1_set_zero_kernel_ydir_h) .OR. &
         (ydim1 .NE. ydim1_set_zero_kernel_ydir_h) &
       ) THEN
            xdim1_set_zero_kernel_ydir = xdim1
            xdim1_set_zero_kernel_ydir_h = xdim1
!$OMP TARGET UPDATE TO(xdim1_set_zero_kernel_ydir)
            ydim1_set_zero_kernel_ydir = ydim1
            ydim1_set_zero_kernel_ydir_h = ydim1
!$OMP TARGET UPDATE TO(ydim1_set_zero_kernel_ydir)
    END IF
...
!   ==============================
    CALL set_zero_kernel_ydir_wrap( &
                        opsDat1Local, &
                        dat1_base, &
                        opsDat1Cardinality, &
                        start_indx, &
...
!   ========================
    ...
    CALL setKernelTime(3, kernelName//c_null_char, t3__-t2__, t2__-t1__, transfer_total, 0)
...
SUBROUTINE set_zero_kernel_ydir_host( userSubroutine, block, dim, range, &
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    1, 3, dim, 1, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(set_zero_kernel_ydir_host_execute))

END SUBROUTINE
#endif

END MODULE SET_ZERO_KERNEL_YDIR_MODULE
