! Auto-generated at 2026-04-28 18:44:33.067044 by ops-translator
...
!$OMP DECLARE TARGET(xdim1_turbin_kernel_eqA)
    INTEGER(KIND=4) :: xdim1_turbin_kernel_eqA
    INTEGER(KIND=4) :: xdim1_turbin_kernel_eqA_h = -1
!$OMP DECLARE TARGET(ydim1_turbin_kernel_eqA)
    INTEGER(KIND=4) :: ydim1_turbin_kernel_eqA
...
!$OMP DECLARE TARGET(xdim2_turbin_kernel_eqA)
    INTEGER(KIND=4) :: xdim2_turbin_kernel_eqA
    INTEGER(KIND=4) :: xdim2_turbin_kernel_eqA_h = -1
!$OMP DECLARE TARGET(ydim2_turbin_kernel_eqA)
    INTEGER(KIND=4) :: ydim2_turbin_kernel_eqA
...
!$OMP DECLARE TARGET(xdim3_turbin_kernel_eqA)
    INTEGER(KIND=4) :: xdim3_turbin_kernel_eqA
    INTEGER(KIND=4) :: xdim3_turbin_kernel_eqA_h = -1
!$OMP DECLARE TARGET(ydim3_turbin_kernel_eqA)
    INTEGER(KIND=4) :: ydim3_turbin_kernel_eqA
...
!$OMP DECLARE TARGET(xdim4_turbin_kernel_eqA)
    INTEGER(KIND=4) :: xdim4_turbin_kernel_eqA
    INTEGER(KIND=4) :: xdim4_turbin_kernel_eqA_h = -1
!$OMP DECLARE TARGET(ydim4_turbin_kernel_eqA)
    INTEGER(KIND=4) :: ydim4_turbin_kernel_eqA
...
!$OMP DECLARE TARGET(xdim5_turbin_kernel_eqA)
    INTEGER(KIND=4) :: xdim5_turbin_kernel_eqA
    INTEGER(KIND=4) :: xdim5_turbin_kernel_eqA_h = -1
!$OMP DECLARE TARGET(ydim5_turbin_kernel_eqA)
    INTEGER(KIND=4) :: ydim5_turbin_kernel_eqA
...
!$OMP DECLARE TARGET(xdim6_turbin_kernel_eqA)
    INTEGER(KIND=4) :: xdim6_turbin_kernel_eqA
    INTEGER(KIND=4) :: xdim6_turbin_kernel_eqA_h = -1
!$OMP DECLARE TARGET(ydim6_turbin_kernel_eqA)
    INTEGER(KIND=4) :: ydim6_turbin_kernel_eqA
...
SUBROUTINE turbin_kernel_eqA_wrap( &
    opsDat1Local, &
    opsDat2Local, &
    opsDat3Local, &
    opsDat4Local, &
    ...
    REAL(KIND=8), DIMENSION(1), INTENT(INOUT) :: opsDat7Local
    REAL(KIND=8) :: tket
...
#ifdef _CRAYFTN
!$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO PRIVATE(n_x,n_y,n_z) REDUCTION(+:tket) 
#else
!$OMP TARGET TEAMS LOOP PRIVATE(n_x,n_y,n_z) REDUCTION(+:tket) 
#endif
    DO n_z = 1, end_indx_3-start_indx_3+1
        DO n_y = 1, end_indx_2-start_indx_2+1
            DO n_x = 1, end_indx_1-start_indx_1+1

                CALL turbin_kernel_eqA( &
                opsDat1Local(dat1_base + ((n_x-1)*1) + ((n_y-1)*xdim1_turbin_kernel_eqA*1) + ((n_z-1)*ydim1_turbin_kernel_eqA*xdim1_turbin_kernel_eqA*1)), &
                opsDat2Local(dat2_base + ((n_x-1)*1) + ((n_y-1)*xdim2_turbin_kernel_eqA*1) + ((n_z-1)*ydim2_turbin_kernel_eqA*xdim2_turbin_kernel_eqA*1)), &
                opsDat3Local(dat3_base + ((n_x-1)*1) + ((n_y-1)*xdim3_turbin_kernel_eqA*1) + ((n_z-1)*ydim3_turbin_kernel_eqA*xdim3_turbin_kernel_eqA*1)), &
                opsDat4Local(dat4_base + ((n_x-1)*1) + ((n_y-1)*xdim4_turbin_kernel_eqA*1) + ((n_z-1)*ydim4_turbin_kernel_eqA*xdim4_turbin_kernel_eqA*1)), &
                opsDat5Local(dat5_base + ((n_x-1)*1) + ((n_y-1)*xdim5_turbin_kernel_eqA*1) + ((n_z-1)*ydim5_turbin_kernel_eqA*xdim5_turbin_kernel_eqA*1)), &
                opsDat6Local(dat6_base + ((n_x-1)*1) + ((n_y-1)*xdim6_turbin_kernel_eqA*1) + ((n_z-1)*ydim6_turbin_kernel_eqA*xdim6_turbin_kernel_eqA*1)), &
                tket &
               )
...
!$OMP END TARGET TEAMS LOOP
    ...
    opsDat7Local(dat7_base) = opsDat7Local(dat7_base) + tket
...
!   ===============
#ifndef OPS_LAZY
SUBROUTINE turbin_kernel_eqA_host( userSubroutine, block, dim, range, &
    opsArg1, &
    opsArg2, &
    opsArg3, &
    opsArg4, &
...
#else
SUBROUTINE turbin_kernel_eqA_host_execute( descPtr )
...
!   ==========================
    ...
    CALL setKernelTime(555, kernelName//c_null_char, 0.0_8, 0.0_8, 0.0_4, 1)
    ...
    CALL c_f_pointer(getReductionPtrFromOpsArg(opsArg7,block), opsDat7Local, [opsDat7Cardinality])
    dat7_base = 1

    IF (&
         (xdim1 .NE. xdim1_turbin_kernel_eqA_h) .OR. &
...
!   ========================
    ...
    CALL setKernelTime(555, kernelName//c_null_char, t3__-t2__, t2__-t1__, transfer_total, 0)
...
SUBROUTINE turbin_kernel_eqA_host( userSubroutine, block, dim, range, &
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    7, 555, dim, 1, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(turbin_kernel_eqA_host_execute))

END SUBROUTINE
#endif

END MODULE TURBIN_KERNEL_EQA_MODULE
