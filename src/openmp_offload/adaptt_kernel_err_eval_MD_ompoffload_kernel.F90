! Auto-generated at 2026-04-28 18:44:30.603359 by ops-translator

MODULE ADAPTT_KERNEL_ERR_EVAL_MD_MODULE
...
!$OMP DECLARE TARGET(xdim1_adaptt_kernel_err_eval_MD)
    INTEGER(KIND=4) :: xdim1_adaptt_kernel_err_eval_MD
    INTEGER(KIND=4) :: xdim1_adaptt_kernel_err_eval_MD_h = -1
!$OMP DECLARE TARGET(ydim1_adaptt_kernel_err_eval_MD)
    INTEGER(KIND=4) :: ydim1_adaptt_kernel_err_eval_MD
    INTEGER(KIND=4) :: ydim1_adaptt_kernel_err_eval_MD_h = -1
#define OPS_ACC1(x,y,z) ((x) + (xdim1_adaptt_kernel_err_eval_MD*(y)) + (xdim1_adaptt_kernel_err_eval_MD*ydim1_adaptt_kernel_err_eval_MD*(z)) + 1)

!$OMP DECLARE TARGET(xdim2_adaptt_kernel_err_eval_MD)
    INTEGER(KIND=4) :: xdim2_adaptt_kernel_err_eval_MD
    INTEGER(KIND=4) :: xdim2_adaptt_kernel_err_eval_MD_h = -1
!$OMP DECLARE TARGET(ydim2_adaptt_kernel_err_eval_MD)
    INTEGER(KIND=4) :: ydim2_adaptt_kernel_err_eval_MD
    INTEGER(KIND=4) :: ydim2_adaptt_kernel_err_eval_MD_h = -1
#define OPS_ACC2(x,y,z) ((x) + (xdim2_adaptt_kernel_err_eval_MD*(y)) + (xdim2_adaptt_kernel_err_eval_MD*ydim2_adaptt_kernel_err_eval_MD*(z)) + 1)
...
SUBROUTINE adaptt_kernel_err_eval_MD_wrap( &
    opsDat1Local, &
    opsDat2Local, &
    opsGblDat3Device, &
    opsGblDat4Device, &
    ...
    REAL(KIND=8), DIMENSION(1), INTENT(INOUT) :: opsDat5Local
    REAL(KIND=8) :: ertot
    ...
    ertot = -1.0_8*HUGE(ertot)

#ifdef _CRAYFTN
!$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO PRIVATE(n_x,n_y,n_z) REDUCTION(MAX:ertot) 
#else
!$OMP TARGET TEAMS LOOP PRIVATE(n_x,n_y,n_z) REDUCTION(MAX:ertot) 
#endif
    DO n_z = 1, end_indx_3-start_indx_3+1
        DO n_y = 1, end_indx_2-start_indx_2+1
            DO n_x = 1, end_indx_1-start_indx_1+1

                CALL adaptt_kernel_err_eval_MD( &
                opsDat1Local(dat1_base + ((n_x-1)*1) + ((n_y-1)*xdim1_adaptt_kernel_err_eval_MD*1) + ((n_z-1)*ydim1_adaptt_kernel_err_eval_MD*xdim1_adaptt_kernel_err_eval_MD*1)), &
                opsDat2Local(dat2_base + ((n_x-1)*1) + ((n_y-1)*xdim2_adaptt_kernel_err_eval_MD*1) + ((n_z-1)*ydim2_adaptt_kernel_err_eval_MD*xdim2_adaptt_kernel_err_eval_MD*1)), &
                opsGblDat3Device(1), &
                opsGblDat4Device, &
                ertot &
               )
...
!$OMP END TARGET TEAMS LOOP
    ...
    opsDat5Local(dat5_base) = MAX(opsDat5Local(dat5_base), ertot)
...
!   ===============
#ifndef OPS_LAZY
SUBROUTINE adaptt_kernel_err_eval_MD_host( userSubroutine, block, dim, range, &
    opsArg1, &
    opsArg2, &
    opsArg3, &
    opsArg4, &
...
#else
SUBROUTINE adaptt_kernel_err_eval_MD_host_execute( descPtr )
    ...
    kernelName = "adaptt_kernel_err_eval_MD"
...
!   ==========================
    ...
    CALL setKernelTime(316, kernelName//c_null_char, 0.0_8, 0.0_8, 0.0_4, 1)
    ...
    CALL c_f_pointer(getReductionPtrFromOpsArg(opsArg5,block), opsDat5Local, [opsDat5Cardinality])
    dat5_base = 1

    IF (&
         (xdim1 .NE. xdim1_adaptt_kernel_err_eval_MD_h) .OR. &
         (ydim1 .NE. ydim1_adaptt_kernel_err_eval_MD_h) .OR. &
         (xdim2 .NE. xdim2_adaptt_kernel_err_eval_MD_h) .OR. &
         (ydim2 .NE. ydim2_adaptt_kernel_err_eval_MD_h) &
       ) THEN
            xdim1_adaptt_kernel_err_eval_MD = xdim1
            xdim1_adaptt_kernel_err_eval_MD_h = xdim1
!$OMP TARGET UPDATE TO(xdim1_adaptt_kernel_err_eval_MD)
            ydim1_adaptt_kernel_err_eval_MD = ydim1
            ydim1_adaptt_kernel_err_eval_MD_h = ydim1
!$OMP TARGET UPDATE TO(ydim1_adaptt_kernel_err_eval_MD)
            xdim2_adaptt_kernel_err_eval_MD = xdim2
            xdim2_adaptt_kernel_err_eval_MD_h = xdim2
!$OMP TARGET UPDATE TO(xdim2_adaptt_kernel_err_eval_MD)
            ydim2_adaptt_kernel_err_eval_MD = ydim2
            ydim2_adaptt_kernel_err_eval_MD_h = ydim2
!$OMP TARGET UPDATE TO(ydim2_adaptt_kernel_err_eval_MD)
    END IF
...
!   ==============================
    CALL adaptt_kernel_err_eval_MD_wrap( &
                        opsDat1Local, &
                        opsDat2Local, &
                        opsDat3Local, &
                        opsDat4Local(1), &
...
!   ========================
    ...
    CALL setKernelTime(316, kernelName//c_null_char, t3__-t2__, t2__-t1__, transfer_total, 0)
...
SUBROUTINE adaptt_kernel_err_eval_MD_host( userSubroutine, block, dim, range, &
    ...
    namelit = "adaptt_kernel_err_eval_MD"
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    5, 316, dim, 1, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(adaptt_kernel_err_eval_MD_host_execute))

END SUBROUTINE
#endif

END MODULE ADAPTT_KERNEL_ERR_EVAL_MD_MODULE
