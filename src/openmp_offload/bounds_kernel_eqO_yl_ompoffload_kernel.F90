! Auto-generated at 2026-04-28 18:44:31.699732 by ops-translator
...
!$OMP DECLARE TARGET(xdim1_bounds_kernel_eqO_yl)
    INTEGER(KIND=4) :: xdim1_bounds_kernel_eqO_yl
    INTEGER(KIND=4) :: xdim1_bounds_kernel_eqO_yl_h = -1
!$OMP DECLARE TARGET(ydim1_bounds_kernel_eqO_yl)
    INTEGER(KIND=4) :: ydim1_bounds_kernel_eqO_yl
    INTEGER(KIND=4) :: ydim1_bounds_kernel_eqO_yl_h = -1
#define OPS_ACC1(x,y,z) ((x) + (xdim1_bounds_kernel_eqO_yl*(y)) + (xdim1_bounds_kernel_eqO_yl*ydim1_bounds_kernel_eqO_yl*(z)) + 1)
...
SUBROUTINE bounds_kernel_eqO_yl(bclyyl)

    real(kind=8), dimension(1) ::  bclyyl

    bclyyl(OPS_ACC1(0,0,0)) = 0.0_8

END SUBROUTINE

#undef OPS_ACC1

SUBROUTINE bounds_kernel_eqO_yl_wrap( &
    opsDat1Local, &
    dat1_base, &
    dat1_dim, &
    start_indx, &
    ...
    DO n_z = 1, end_indx_3-start_indx_3+1
        DO n_y = 1, end_indx_2-start_indx_2+1
            DO n_x = 1, end_indx_1-start_indx_1+1
                ...
                CALL bounds_kernel_eqO_yl( &
                opsDat1Local(dat1_base + ((n_x-1)*1) + ((n_y-1)*xdim1_bounds_kernel_eqO_yl*0) + ((n_z-1)*ydim1_bounds_kernel_eqO_yl*xdim1_bounds_kernel_eqO_yl*1)) &
               )
...
#else
SUBROUTINE bounds_kernel_eqO_yl_host_execute( descPtr )
    ...
    kernelName = "bounds_kernel_eqO_yl"
...
!   ==========================
    ...
    CALL setKernelTime(418, kernelName//c_null_char, 0.0_8, 0.0_8, 0.0_4, 1)
    ...
    IF (&
         (xdim1 .NE. xdim1_bounds_kernel_eqO_yl_h) .OR. &
         (ydim1 .NE. ydim1_bounds_kernel_eqO_yl_h) &
       ) THEN
            xdim1_bounds_kernel_eqO_yl = xdim1
            xdim1_bounds_kernel_eqO_yl_h = xdim1
!$OMP TARGET UPDATE TO(xdim1_bounds_kernel_eqO_yl)
            ydim1_bounds_kernel_eqO_yl = ydim1
            ydim1_bounds_kernel_eqO_yl_h = ydim1
!$OMP TARGET UPDATE TO(ydim1_bounds_kernel_eqO_yl)
    END IF
...
!   ==============================
    CALL bounds_kernel_eqO_yl_wrap( &
                        opsDat1Local, &
                        dat1_base, &
                        opsDat1Cardinality, &
                        start_indx, &
...
!   ========================
    ...
    CALL setKernelTime(418, kernelName//c_null_char, t3__-t2__, t2__-t1__, transfer_total, 0)
...
SUBROUTINE bounds_kernel_eqO_yl_host( userSubroutine, block, dim, range, &
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    1, 418, dim, 1, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(bounds_kernel_eqO_yl_host_execute))
