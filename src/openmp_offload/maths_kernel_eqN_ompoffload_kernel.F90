! Auto-generated at 2026-04-28 18:44:30.521285 by ops-translator

MODULE MATHS_KERNEL_EQN_MODULE
...
!$OMP DECLARE TARGET(xdim1_maths_kernel_eqN)
    INTEGER(KIND=4) :: xdim1_maths_kernel_eqN
    INTEGER(KIND=4) :: xdim1_maths_kernel_eqN_h = -1
!$OMP DECLARE TARGET(ydim1_maths_kernel_eqN)
    INTEGER(KIND=4) :: ydim1_maths_kernel_eqN
    INTEGER(KIND=4) :: ydim1_maths_kernel_eqN_h = -1
#define OPS_ACC1(x,y,z) ((x) + (xdim1_maths_kernel_eqN*(y)) + (xdim1_maths_kernel_eqN*ydim1_maths_kernel_eqN*(z)) + 1)

!$OMP DECLARE TARGET(xdim2_maths_kernel_eqN)
    INTEGER(KIND=4) :: xdim2_maths_kernel_eqN
    INTEGER(KIND=4) :: xdim2_maths_kernel_eqN_h = -1
!$OMP DECLARE TARGET(ydim2_maths_kernel_eqN)
    INTEGER(KIND=4) :: ydim2_maths_kernel_eqN
    INTEGER(KIND=4) :: ydim2_maths_kernel_eqN_h = -1
#define OPS_ACC2(x,y,z) ((x) + (xdim2_maths_kernel_eqN*(y)) + (xdim2_maths_kernel_eqN*ydim2_maths_kernel_eqN*(z)) + 1)
...
SUBROUTINE maths_kernel_eqN_wrap( &
    opsDat1Local, &
    opsDat2Local, &
    opsGblDat3Device, &
    opsGblDat4Device, &
    ...
    DO n_z = 1, end_indx_3-start_indx_3+1
        DO n_y = 1, end_indx_2-start_indx_2+1
            DO n_x = 1, end_indx_1-start_indx_1+1
                ...
                CALL maths_kernel_eqN( &
                opsDat1Local(dat1_base + ((n_x-1)*1) + ((n_y-1)*xdim1_maths_kernel_eqN*1) + ((n_z-1)*ydim1_maths_kernel_eqN*xdim1_maths_kernel_eqN*1)), &
                opsDat2Local(dat2_base + ((n_x-1)*1) + ((n_y-1)*xdim2_maths_kernel_eqN*1) + ((n_z-1)*ydim2_maths_kernel_eqN*xdim2_maths_kernel_eqN*1)), &
                opsGblDat3Device, &
                opsGblDat4Device &
               )
...
!   ===============
#ifndef OPS_LAZY
SUBROUTINE maths_kernel_eqN_host( userSubroutine, block, dim, range, &
    opsArg1, &
    opsArg2, &
    opsArg3, &
    opsArg4 &
...
#else
SUBROUTINE maths_kernel_eqN_host_execute( descPtr )
...
!   ==========================
    ...
    CALL setKernelTime(296, kernelName//c_null_char, 0.0_8, 0.0_8, 0.0_4, 1)
    ...
    IF (&
         (xdim1 .NE. xdim1_maths_kernel_eqN_h) .OR. &
         (ydim1 .NE. ydim1_maths_kernel_eqN_h) .OR. &
         (xdim2 .NE. xdim2_maths_kernel_eqN_h) .OR. &
         (ydim2 .NE. ydim2_maths_kernel_eqN_h) &
       ) THEN
            xdim1_maths_kernel_eqN = xdim1
            xdim1_maths_kernel_eqN_h = xdim1
!$OMP TARGET UPDATE TO(xdim1_maths_kernel_eqN)
            ydim1_maths_kernel_eqN = ydim1
            ydim1_maths_kernel_eqN_h = ydim1
!$OMP TARGET UPDATE TO(ydim1_maths_kernel_eqN)
            xdim2_maths_kernel_eqN = xdim2
            xdim2_maths_kernel_eqN_h = xdim2
!$OMP TARGET UPDATE TO(xdim2_maths_kernel_eqN)
            ydim2_maths_kernel_eqN = ydim2
            ydim2_maths_kernel_eqN_h = ydim2
!$OMP TARGET UPDATE TO(ydim2_maths_kernel_eqN)
    END IF
...
!   ========================
    ...
    CALL setKernelTime(296, kernelName//c_null_char, t3__-t2__, t2__-t1__, transfer_total, 0)
...
SUBROUTINE maths_kernel_eqN_host( userSubroutine, block, dim, range, &
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    4, 296, dim, 1, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(maths_kernel_eqN_host_execute))

END SUBROUTINE
#endif

END MODULE MATHS_KERNEL_EQN_MODULE
