! Auto-generated at 2026-04-28 18:44:30.600369 by ops-translator
...
!$OMP DECLARE TARGET(xdim1_adaptt_kernel_err_eval)
    INTEGER(KIND=4) :: xdim1_adaptt_kernel_err_eval
    INTEGER(KIND=4) :: xdim1_adaptt_kernel_err_eval_h = -1
!$OMP DECLARE TARGET(ydim1_adaptt_kernel_err_eval)
    INTEGER(KIND=4) :: ydim1_adaptt_kernel_err_eval
...
!$OMP DECLARE TARGET(xdim2_adaptt_kernel_err_eval)
    INTEGER(KIND=4) :: xdim2_adaptt_kernel_err_eval
    INTEGER(KIND=4) :: xdim2_adaptt_kernel_err_eval_h = -1
!$OMP DECLARE TARGET(ydim2_adaptt_kernel_err_eval)
    INTEGER(KIND=4) :: ydim2_adaptt_kernel_err_eval
...
SUBROUTINE adaptt_kernel_err_eval_wrap( &
    opsDat1Local, &
    opsDat2Local, &
    opsGblDat3Device, &
    opsDat4Local, &
    ...
    REAL(KIND=8), DIMENSION(1), INTENT(INOUT) :: opsDat4Local
    REAL(KIND=8) :: ertot
    ...
    DO n_z = 1, end_indx_3-start_indx_3+1
        DO n_y = 1, end_indx_2-start_indx_2+1
            DO n_x = 1, end_indx_1-start_indx_1+1
                ...
                CALL adaptt_kernel_err_eval( &
                opsDat1Local(dat1_base + ((n_x-1)*1) + ((n_y-1)*xdim1_adaptt_kernel_err_eval*1) + ((n_z-1)*ydim1_adaptt_kernel_err_eval*xdim1_adaptt_kernel_err_eval*1)), &
                opsDat2Local(dat2_base + ((n_x-1)*1) + ((n_y-1)*xdim2_adaptt_kernel_err_eval*1) + ((n_z-1)*ydim2_adaptt_kernel_err_eval*xdim2_adaptt_kernel_err_eval*1)), &
                opsGblDat3Device, &
                ertot &
               )
...
!$OMP END TARGET TEAMS LOOP
    ...
    opsDat4Local(dat4_base) = MAX(opsDat4Local(dat4_base), ertot)
...
!   ===============
#ifndef OPS_LAZY
SUBROUTINE adaptt_kernel_err_eval_host( userSubroutine, block, dim, range, &
    opsArg1, &
    opsArg2, &
    opsArg3, &
    opsArg4 &
...
#else
SUBROUTINE adaptt_kernel_err_eval_host_execute( descPtr )
...
!   ==========================
    ...
    CALL c_f_pointer(getReductionPtrFromOpsArg(opsArg4,block), opsDat4Local, [opsDat4Cardinality])
    dat4_base = 1
...
!   ==============================
    CALL adaptt_kernel_err_eval_wrap( &
                        opsDat1Local, &
                        opsDat2Local, &
                        opsDat3Local(1), &
                        opsDat4Local, &
