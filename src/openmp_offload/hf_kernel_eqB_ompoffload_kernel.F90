! Auto-generated at 2026-04-28 18:44:30.132274 by ops-translator

MODULE HF_KERNEL_EQB_MODULE
...
!$OMP DECLARE TARGET(xdim1_hf_kernel_eqB)
    INTEGER(KIND=4) :: xdim1_hf_kernel_eqB
    INTEGER(KIND=4) :: xdim1_hf_kernel_eqB_h = -1
!$OMP DECLARE TARGET(ydim1_hf_kernel_eqB)
    INTEGER(KIND=4) :: ydim1_hf_kernel_eqB
    INTEGER(KIND=4) :: ydim1_hf_kernel_eqB_h = -1
#define OPS_ACC1(x,y,z) ((x) + (xdim1_hf_kernel_eqB*(y)) + (xdim1_hf_kernel_eqB*ydim1_hf_kernel_eqB*(z)) + 1)

!$OMP DECLARE TARGET(xdim2_hf_kernel_eqB)
    INTEGER(KIND=4) :: xdim2_hf_kernel_eqB
    INTEGER(KIND=4) :: xdim2_hf_kernel_eqB_h = -1
!$OMP DECLARE TARGET(ydim2_hf_kernel_eqB)
    INTEGER(KIND=4) :: ydim2_hf_kernel_eqB
    INTEGER(KIND=4) :: ydim2_hf_kernel_eqB_h = -1
#define OPS_ACC2(x,y,z) ((x) + (xdim2_hf_kernel_eqB*(y)) + (xdim2_hf_kernel_eqB*ydim2_hf_kernel_eqB*(z)) + 1)

!$OMP DECLARE TARGET(xdim3_hf_kernel_eqB)
    INTEGER(KIND=4) :: xdim3_hf_kernel_eqB
    INTEGER(KIND=4) :: xdim3_hf_kernel_eqB_h = -1
!$OMP DECLARE TARGET(ydim3_hf_kernel_eqB)
    INTEGER(KIND=4) :: ydim3_hf_kernel_eqB
    INTEGER(KIND=4) :: ydim3_hf_kernel_eqB_h = -1
#define OPS_ACC3(x,y,z) ((x) + (xdim3_hf_kernel_eqB*(y)) + (xdim3_hf_kernel_eqB*ydim3_hf_kernel_eqB*(z)) + 1)
    ...
    DO n_z = 1, end_indx_3-start_indx_3+1
        DO n_y = 1, end_indx_2-start_indx_2+1
            DO n_x = 1, end_indx_1-start_indx_1+1
                ...
                CALL hf_kernel_eqB( &
                opsDat1Local(dat1_base + ((n_x-1)*1) + ((n_y-1)*xdim1_hf_kernel_eqB*1) + ((n_z-1)*ydim1_hf_kernel_eqB*xdim1_hf_kernel_eqB*1)), &
                opsDat2Local(dat2_base + ((n_x-1)*1) + ((n_y-1)*xdim2_hf_kernel_eqB*1) + ((n_z-1)*ydim2_hf_kernel_eqB*xdim2_hf_kernel_eqB*1)), &
                opsDat3Local(dat3_base + ((n_x-1)*1) + ((n_y-1)*xdim3_hf_kernel_eqB*1) + ((n_z-1)*ydim3_hf_kernel_eqB*xdim3_hf_kernel_eqB*1)), &
                opsGblDat4Device &
               )
...
!   ===============
#ifndef OPS_LAZY
SUBROUTINE hf_kernel_eqB_host( userSubroutine, block, dim, range, &
    opsArg1, &
    opsArg2, &
    opsArg3, &
    opsArg4 &
...
#else
SUBROUTINE hf_kernel_eqB_host_execute( descPtr )
...
!   ==========================
    ...
    CALL setKernelTime(216, kernelName//c_null_char, 0.0_8, 0.0_8, 0.0_4, 1)
    ...
    IF (&
         (xdim1 .NE. xdim1_hf_kernel_eqB_h) .OR. &
         (ydim1 .NE. ydim1_hf_kernel_eqB_h) .OR. &
         (xdim2 .NE. xdim2_hf_kernel_eqB_h) .OR. &
         (ydim2 .NE. ydim2_hf_kernel_eqB_h) .OR. &
         (xdim3 .NE. xdim3_hf_kernel_eqB_h) .OR. &
         (ydim3 .NE. ydim3_hf_kernel_eqB_h) &
       ) THEN
            xdim1_hf_kernel_eqB = xdim1
            xdim1_hf_kernel_eqB_h = xdim1
!$OMP TARGET UPDATE TO(xdim1_hf_kernel_eqB)
            ydim1_hf_kernel_eqB = ydim1
            ydim1_hf_kernel_eqB_h = ydim1
!$OMP TARGET UPDATE TO(ydim1_hf_kernel_eqB)
            xdim2_hf_kernel_eqB = xdim2
            xdim2_hf_kernel_eqB_h = xdim2
!$OMP TARGET UPDATE TO(xdim2_hf_kernel_eqB)
            ydim2_hf_kernel_eqB = ydim2
            ydim2_hf_kernel_eqB_h = ydim2
!$OMP TARGET UPDATE TO(ydim2_hf_kernel_eqB)
            xdim3_hf_kernel_eqB = xdim3
            xdim3_hf_kernel_eqB_h = xdim3
!$OMP TARGET UPDATE TO(xdim3_hf_kernel_eqB)
            ydim3_hf_kernel_eqB = ydim3
            ydim3_hf_kernel_eqB_h = ydim3
!$OMP TARGET UPDATE TO(ydim3_hf_kernel_eqB)
    END IF
...
!   ========================
    ...
    CALL setKernelTime(216, kernelName//c_null_char, t3__-t2__, t2__-t1__, transfer_total, 0)
...
SUBROUTINE hf_kernel_eqB_host( userSubroutine, block, dim, range, &
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    4, 216, dim, 1, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(hf_kernel_eqB_host_execute))

END SUBROUTINE
#endif

END MODULE HF_KERNEL_EQB_MODULE
