! Auto-generated at 2026-04-28 18:44:28.516581 by ops-translator

MODULE SET_ZERO_KERNEL_ZDIR_MODULE
...
!$OMP DECLARE TARGET(xdim1_set_zero_kernel_zdir)
    INTEGER(KIND=4) :: xdim1_set_zero_kernel_zdir
    INTEGER(KIND=4) :: xdim1_set_zero_kernel_zdir_h = -1
!$OMP DECLARE TARGET(ydim1_set_zero_kernel_zdir)
    INTEGER(KIND=4) :: ydim1_set_zero_kernel_zdir
    INTEGER(KIND=4) :: ydim1_set_zero_kernel_zdir_h = -1
#define OPS_ACC1(x,y,z) ((x) + (xdim1_set_zero_kernel_zdir*(y)) + (xdim1_set_zero_kernel_zdir*ydim1_set_zero_kernel_zdir*(z)) + 1)
...
SUBROUTINE set_zero_kernel_zdir_wrap( &
    opsDat1Local, &
    dat1_base, &
    dat1_dim, &
    start_indx, &
    ...
    DO n_z = 1, end_indx_3-start_indx_3+1
        DO n_y = 1, end_indx_2-start_indx_2+1
            DO n_x = 1, end_indx_1-start_indx_1+1
                ...
                CALL set_zero_kernel_zdir( &
                opsDat1Local(dat1_base + ((n_x-1)*1) + ((n_y-1)*xdim1_set_zero_kernel_zdir*1) + ((n_z-1)*ydim1_set_zero_kernel_zdir*xdim1_set_zero_kernel_zdir*0)) &
               )
...
!   ===============
#ifndef OPS_LAZY
SUBROUTINE set_zero_kernel_zdir_host( userSubroutine, block, dim, range, &
    opsArg1 &
    )
...
#else
SUBROUTINE set_zero_kernel_zdir_host_execute( descPtr )
    ...
    kernelName = "set_zero_kernel_zdir"
...
!   ==========================
    ...
    IF (&
         (xdim1 .NE. xdim1_set_zero_kernel_zdir_h) .OR. &
         (ydim1 .NE. ydim1_set_zero_kernel_zdir_h) &
       ) THEN
            xdim1_set_zero_kernel_zdir = xdim1
            xdim1_set_zero_kernel_zdir_h = xdim1
!$OMP TARGET UPDATE TO(xdim1_set_zero_kernel_zdir)
            ydim1_set_zero_kernel_zdir = ydim1
            ydim1_set_zero_kernel_zdir_h = ydim1
!$OMP TARGET UPDATE TO(ydim1_set_zero_kernel_zdir)
    END IF
...
!   ==============================
    CALL set_zero_kernel_zdir_wrap( &
                        opsDat1Local, &
                        dat1_base, &
                        opsDat1Cardinality, &
                        start_indx, &
...
SUBROUTINE set_zero_kernel_zdir_host( userSubroutine, block, dim, range, &
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    1, 4, dim, 1, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(set_zero_kernel_zdir_host_execute))

END SUBROUTINE
#endif

END MODULE SET_ZERO_KERNEL_ZDIR_MODULE
