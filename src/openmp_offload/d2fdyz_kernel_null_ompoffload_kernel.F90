! Auto-generated at 2026-04-28 18:44:29.567666 by ops-translator

MODULE D2FDYZ_KERNEL_NULL_MODULE
...
!$OMP DECLARE TARGET(xdim1_d2fdyz_kernel_null)
    INTEGER(KIND=4) :: xdim1_d2fdyz_kernel_null
    INTEGER(KIND=4) :: xdim1_d2fdyz_kernel_null_h = -1
!$OMP DECLARE TARGET(ydim1_d2fdyz_kernel_null)
    INTEGER(KIND=4) :: ydim1_d2fdyz_kernel_null
    INTEGER(KIND=4) :: ydim1_d2fdyz_kernel_null_h = -1
#define OPS_ACC1(x,y,z) ((x) + (xdim1_d2fdyz_kernel_null*(y)) + (xdim1_d2fdyz_kernel_null*ydim1_d2fdyz_kernel_null*(z)) + 1)
...
SUBROUTINE d2fdyz_kernel_null_wrap( &
    opsDat1Local, &
    dat1_base, &
    dat1_dim, &
    start_indx, &
    ...
    DO n_z = 1, end_indx_3-start_indx_3+1
        DO n_y = 1, end_indx_2-start_indx_2+1
            DO n_x = 1, end_indx_1-start_indx_1+1
                ...
                CALL d2fdyz_kernel_null( &
                opsDat1Local(dat1_base + ((n_x-1)*1) + ((n_y-1)*xdim1_d2fdyz_kernel_null*1) + ((n_z-1)*ydim1_d2fdyz_kernel_null*xdim1_d2fdyz_kernel_null*1)) &
               )
...
!   ===============
#ifndef OPS_LAZY
SUBROUTINE d2fdyz_kernel_null_host( userSubroutine, block, dim, range, &
    opsArg1 &
    )
...
#else
SUBROUTINE d2fdyz_kernel_null_host_execute( descPtr )
...
!   ==========================
    ...
    CALL setKernelTime(134, kernelName//c_null_char, 0.0_8, 0.0_8, 0.0_4, 1)
    ...
    IF (&
         (xdim1 .NE. xdim1_d2fdyz_kernel_null_h) .OR. &
         (ydim1 .NE. ydim1_d2fdyz_kernel_null_h) &
       ) THEN
            xdim1_d2fdyz_kernel_null = xdim1
            xdim1_d2fdyz_kernel_null_h = xdim1
!$OMP TARGET UPDATE TO(xdim1_d2fdyz_kernel_null)
            ydim1_d2fdyz_kernel_null = ydim1
            ydim1_d2fdyz_kernel_null_h = ydim1
!$OMP TARGET UPDATE TO(ydim1_d2fdyz_kernel_null)
    END IF
...
!   ========================
    ...
    CALL setKernelTime(134, kernelName//c_null_char, t3__-t2__, t2__-t1__, transfer_total, 0)
...
SUBROUTINE d2fdyz_kernel_null_host( userSubroutine, block, dim, range, &
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    1, 134, dim, 1, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(d2fdyz_kernel_null_host_execute))

END SUBROUTINE
#endif

END MODULE D2FDYZ_KERNEL_NULL_MODULE
