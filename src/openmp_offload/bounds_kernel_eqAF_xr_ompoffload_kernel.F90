! Auto-generated at 2026-04-28 18:44:31.259797 by ops-translator

MODULE BOUNDS_KERNEL_EQAF_XR_MODULE
...
!$OMP DECLARE TARGET(xdim1_bounds_kernel_eqAF_xr)
    INTEGER(KIND=4) :: xdim1_bounds_kernel_eqAF_xr
    INTEGER(KIND=4) :: xdim1_bounds_kernel_eqAF_xr_h = -1
!$OMP DECLARE TARGET(ydim1_bounds_kernel_eqAF_xr)
    INTEGER(KIND=4) :: ydim1_bounds_kernel_eqAF_xr
    INTEGER(KIND=4) :: ydim1_bounds_kernel_eqAF_xr_h = -1
#define OPS_ACC1(x,y,z) ((x) + (xdim1_bounds_kernel_eqAF_xr*(y)) + (xdim1_bounds_kernel_eqAF_xr*ydim1_bounds_kernel_eqAF_xr*(z)) + 1)

!$OMP DECLARE TARGET(xdim2_bounds_kernel_eqAF_xr)
    INTEGER(KIND=4) :: xdim2_bounds_kernel_eqAF_xr
    INTEGER(KIND=4) :: xdim2_bounds_kernel_eqAF_xr_h = -1
!$OMP DECLARE TARGET(ydim2_bounds_kernel_eqAF_xr)
    INTEGER(KIND=4) :: ydim2_bounds_kernel_eqAF_xr
    INTEGER(KIND=4) :: ydim2_bounds_kernel_eqAF_xr_h = -1
#define OPS_ACC2(x,y,z) ((x) + (xdim2_bounds_kernel_eqAF_xr*(y)) + (xdim2_bounds_kernel_eqAF_xr*ydim2_bounds_kernel_eqAF_xr*(z)) + 1)
...
SUBROUTINE bounds_kernel_eqAF_xr(strtxr,trun)

    real(kind=8), dimension(1) :: strtxr, trun

    strtxr(OPS_ACC1(0,0,0)) = (48.0_8*trun(OPS_ACC2(-1,0,0)) - 36.0_8*trun(OPS_ACC2(-2,0,0))  &
                             + 16.0_8*trun(OPS_ACC2(-3,0,0)) - 3.0_8*trun(OPS_ACC2(-4,0,0)))/25.0_8

    trun(OPS_ACC2(0,0,0)) = strtxr(OPS_ACC1(0,0,0))
...
SUBROUTINE bounds_kernel_eqAF_xr_wrap( &
    opsDat1Local, &
    opsDat2Local, &
    dat1_base, &
    dat2_base, &
    ...
    DO n_z = 1, end_indx_3-start_indx_3+1
        DO n_y = 1, end_indx_2-start_indx_2+1
            DO n_x = 1, end_indx_1-start_indx_1+1
                ...
                CALL bounds_kernel_eqAF_xr( &
                opsDat1Local(dat1_base + ((n_x-1)*0) + ((n_y-1)*xdim1_bounds_kernel_eqAF_xr*1) + ((n_z-1)*ydim1_bounds_kernel_eqAF_xr*xdim1_bounds_kernel_eqAF_xr*1)), &
                opsDat2Local(dat2_base + ((n_x-1)*1) + ((n_y-1)*xdim2_bounds_kernel_eqAF_xr*1) + ((n_z-1)*ydim2_bounds_kernel_eqAF_xr*xdim2_bounds_kernel_eqAF_xr*1)) &
               )
...
!   ===============
#ifndef OPS_LAZY
SUBROUTINE bounds_kernel_eqAF_xr_host( userSubroutine, block, dim, range, &
    opsArg1, &
    opsArg2 &
    )
...
#else
SUBROUTINE bounds_kernel_eqAF_xr_host_execute( descPtr )
    ...
    kernelName = "bounds_kernel_eqAF_xr"
...
!   ==========================
    ...
    CALL setKernelTime(382, kernelName//c_null_char, 0.0_8, 0.0_8, 0.0_4, 1)
    ...
    IF (&
         (xdim1 .NE. xdim1_bounds_kernel_eqAF_xr_h) .OR. &
         (ydim1 .NE. ydim1_bounds_kernel_eqAF_xr_h) .OR. &
         (xdim2 .NE. xdim2_bounds_kernel_eqAF_xr_h) .OR. &
         (ydim2 .NE. ydim2_bounds_kernel_eqAF_xr_h) &
       ) THEN
            xdim1_bounds_kernel_eqAF_xr = xdim1
            xdim1_bounds_kernel_eqAF_xr_h = xdim1
!$OMP TARGET UPDATE TO(xdim1_bounds_kernel_eqAF_xr)
            ydim1_bounds_kernel_eqAF_xr = ydim1
            ydim1_bounds_kernel_eqAF_xr_h = ydim1
!$OMP TARGET UPDATE TO(ydim1_bounds_kernel_eqAF_xr)
            xdim2_bounds_kernel_eqAF_xr = xdim2
            xdim2_bounds_kernel_eqAF_xr_h = xdim2
!$OMP TARGET UPDATE TO(xdim2_bounds_kernel_eqAF_xr)
            ydim2_bounds_kernel_eqAF_xr = ydim2
            ydim2_bounds_kernel_eqAF_xr_h = ydim2
!$OMP TARGET UPDATE TO(ydim2_bounds_kernel_eqAF_xr)
    END IF
...
!   ==============================
    CALL bounds_kernel_eqAF_xr_wrap( &
                        opsDat1Local, &
                        opsDat2Local, &
                        dat1_base, &
                        dat2_base, &
...
!   ========================
    ...
    CALL setKernelTime(382, kernelName//c_null_char, t3__-t2__, t2__-t1__, transfer_total, 0)
...
SUBROUTINE bounds_kernel_eqAF_xr_host( userSubroutine, block, dim, range, &
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    2, 382, dim, 1, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(bounds_kernel_eqAF_xr_host_execute))

END SUBROUTINE
#endif

END MODULE BOUNDS_KERNEL_EQAF_XR_MODULE
