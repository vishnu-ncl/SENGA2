! Auto-generated at 2026-04-28 18:44:33.033501 by ops-translator

MODULE MATHS_KERNEL_EQE_MODULE
...
!$OMP DECLARE TARGET(xdim1_maths_kernel_eqE)
    INTEGER(KIND=4) :: xdim1_maths_kernel_eqE
    INTEGER(KIND=4) :: xdim1_maths_kernel_eqE_h = -1
!$OMP DECLARE TARGET(ydim1_maths_kernel_eqE)
    INTEGER(KIND=4) :: ydim1_maths_kernel_eqE
    INTEGER(KIND=4) :: ydim1_maths_kernel_eqE_h = -1
#define OPS_ACC1(x,y,z) ((x) + (xdim1_maths_kernel_eqE*(y)) + (xdim1_maths_kernel_eqE*ydim1_maths_kernel_eqE*(z)) + 1)
...
SUBROUTINE maths_kernel_eqE_wrap( &
    opsDat1Local, &
    opsGblDat2Device, &
    opsGblDat3Device, &
    dat1_base, &
    ...
    REAL(KIND=8), DIMENSION(dat2_dim), INTENT(IN) :: opsGblDat2Device
    ...
    DO n_z = 1, end_indx_3-start_indx_3+1
        DO n_y = 1, end_indx_2-start_indx_2+1
            DO n_x = 1, end_indx_1-start_indx_1+1
                ...
                CALL maths_kernel_eqE( &
                opsDat1Local(dat1_base + ((n_x-1)*1) + ((n_y-1)*xdim1_maths_kernel_eqE*1) + ((n_z-1)*ydim1_maths_kernel_eqE*xdim1_maths_kernel_eqE*1)), &
                opsGblDat2Device(1), &
                opsGblDat3Device &
               )
...
!   ===============
#ifndef OPS_LAZY
SUBROUTINE maths_kernel_eqE_host( userSubroutine, block, dim, range, &
    opsArg1, &
    opsArg2, &
    opsArg3 &
    )
...
#else
SUBROUTINE maths_kernel_eqE_host_execute( descPtr )
...
!   ==========================
    ...
    CALL setKernelTime(547, kernelName//c_null_char, 0.0_8, 0.0_8, 0.0_4, 1)
    ...
    CALL c_f_pointer(opsArgArray(2)%data_d, opsDat2Local, [opsDat2Cardinality])
    dat2_base = 1
    ...
    IF (&
         (xdim1 .NE. xdim1_maths_kernel_eqE_h) .OR. &
         (ydim1 .NE. ydim1_maths_kernel_eqE_h) &
       ) THEN
            xdim1_maths_kernel_eqE = xdim1
            xdim1_maths_kernel_eqE_h = xdim1
!$OMP TARGET UPDATE TO(xdim1_maths_kernel_eqE)
            ydim1_maths_kernel_eqE = ydim1
            ydim1_maths_kernel_eqE_h = ydim1
!$OMP TARGET UPDATE TO(ydim1_maths_kernel_eqE)
    END IF
...
!   ========================
    ...
    CALL setKernelTime(547, kernelName//c_null_char, t3__-t2__, t2__-t1__, transfer_total, 0)
...
SUBROUTINE maths_kernel_eqE_host( userSubroutine, block, dim, range, &
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    3, 547, dim, 1, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(maths_kernel_eqE_host_execute))

END SUBROUTINE
#endif

END MODULE MATHS_KERNEL_EQE_MODULE
