! Auto-generated at 2026-04-28 18:44:30.606089 by ops-translator

MODULE BCDT_KERNEL_XDIR_MODULE
...
!$OMP DECLARE TARGET(xdim1_bcdt_kernel_xdir)
    INTEGER(KIND=4) :: xdim1_bcdt_kernel_xdir
    INTEGER(KIND=4) :: xdim1_bcdt_kernel_xdir_h = -1
!$OMP DECLARE TARGET(ydim1_bcdt_kernel_xdir)
    INTEGER(KIND=4) :: ydim1_bcdt_kernel_xdir
    INTEGER(KIND=4) :: ydim1_bcdt_kernel_xdir_h = -1
#define OPS_ACC1(x,y,z) ((x) + (xdim1_bcdt_kernel_xdir*(y)) + (xdim1_bcdt_kernel_xdir*ydim1_bcdt_kernel_xdir*(z)) + 1)

!$OMP DECLARE TARGET(xdim2_bcdt_kernel_xdir)
    INTEGER(KIND=4) :: xdim2_bcdt_kernel_xdir
    INTEGER(KIND=4) :: xdim2_bcdt_kernel_xdir_h = -1
!$OMP DECLARE TARGET(ydim2_bcdt_kernel_xdir)
    INTEGER(KIND=4) :: ydim2_bcdt_kernel_xdir
    INTEGER(KIND=4) :: ydim2_bcdt_kernel_xdir_h = -1
#define OPS_ACC2(x,y,z) ((x) + (xdim2_bcdt_kernel_xdir*(y)) + (xdim2_bcdt_kernel_xdir*ydim2_bcdt_kernel_xdir*(z)) + 1)
...
SUBROUTINE bcdt_kernel_xdir_wrap( &
    opsDat1Local, &
    opsDat2Local, &
    opsGblDat3Device, &
    dat1_base, &
    ...
    DO n_z = 1, end_indx_3-start_indx_3+1
        DO n_y = 1, end_indx_2-start_indx_2+1
            DO n_x = 1, end_indx_1-start_indx_1+1
                ...
                CALL bcdt_kernel_xdir( &
                opsDat1Local(dat1_base + ((n_x-1)*0) + ((n_y-1)*xdim1_bcdt_kernel_xdir*1) + ((n_z-1)*ydim1_bcdt_kernel_xdir*xdim1_bcdt_kernel_xdir*1)), &
                opsDat2Local(dat2_base + ((n_x-1)*0) + ((n_y-1)*xdim2_bcdt_kernel_xdir*1) + ((n_z-1)*ydim2_bcdt_kernel_xdir*xdim2_bcdt_kernel_xdir*1)), &
                opsGblDat3Device &
               )
...
!   ===============
#ifndef OPS_LAZY
SUBROUTINE bcdt_kernel_xdir_host( userSubroutine, block, dim, range, &
    opsArg1, &
    opsArg2, &
    opsArg3 &
    )
...
#else
SUBROUTINE bcdt_kernel_xdir_host_execute( descPtr )
...
!   ==========================
    ...
    CALL setKernelTime(317, kernelName//c_null_char, 0.0_8, 0.0_8, 0.0_4, 1)
    ...
    IF (&
         (xdim1 .NE. xdim1_bcdt_kernel_xdir_h) .OR. &
         (ydim1 .NE. ydim1_bcdt_kernel_xdir_h) .OR. &
         (xdim2 .NE. xdim2_bcdt_kernel_xdir_h) .OR. &
         (ydim2 .NE. ydim2_bcdt_kernel_xdir_h) &
       ) THEN
            xdim1_bcdt_kernel_xdir = xdim1
            xdim1_bcdt_kernel_xdir_h = xdim1
!$OMP TARGET UPDATE TO(xdim1_bcdt_kernel_xdir)
            ydim1_bcdt_kernel_xdir = ydim1
            ydim1_bcdt_kernel_xdir_h = ydim1
!$OMP TARGET UPDATE TO(ydim1_bcdt_kernel_xdir)
            xdim2_bcdt_kernel_xdir = xdim2
            xdim2_bcdt_kernel_xdir_h = xdim2
!$OMP TARGET UPDATE TO(xdim2_bcdt_kernel_xdir)
            ydim2_bcdt_kernel_xdir = ydim2
            ydim2_bcdt_kernel_xdir_h = ydim2
!$OMP TARGET UPDATE TO(ydim2_bcdt_kernel_xdir)
    END IF
...
!   ========================
    ...
    CALL setKernelTime(317, kernelName//c_null_char, t3__-t2__, t2__-t1__, transfer_total, 0)
...
SUBROUTINE bcdt_kernel_xdir_host( userSubroutine, block, dim, range, &
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    3, 317, dim, 1, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(bcdt_kernel_xdir_host_execute))

END SUBROUTINE
#endif

END MODULE BCDT_KERNEL_XDIR_MODULE
