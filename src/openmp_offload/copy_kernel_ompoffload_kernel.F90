! Auto-generated at 2026-04-28 18:44:30.161415 by ops-translator
...
!$OMP DECLARE TARGET(xdim1_copy_kernel)
    INTEGER(KIND=4) :: xdim1_copy_kernel
    INTEGER(KIND=4) :: xdim1_copy_kernel_h = -1
!$OMP DECLARE TARGET(ydim1_copy_kernel)
    INTEGER(KIND=4) :: ydim1_copy_kernel
    INTEGER(KIND=4) :: ydim1_copy_kernel_h = -1
#define OPS_ACC1(x,y,z) ((x) + (xdim1_copy_kernel*(y)) + (xdim1_copy_kernel*ydim1_copy_kernel*(z)) + 1)

!$OMP DECLARE TARGET(xdim2_copy_kernel)
    INTEGER(KIND=4) :: xdim2_copy_kernel
    INTEGER(KIND=4) :: xdim2_copy_kernel_h = -1
!$OMP DECLARE TARGET(ydim2_copy_kernel)
    INTEGER(KIND=4) :: ydim2_copy_kernel
    INTEGER(KIND=4) :: ydim2_copy_kernel_h = -1
#define OPS_ACC2(x,y,z) ((x) + (xdim2_copy_kernel*(y)) + (xdim2_copy_kernel*ydim2_copy_kernel*(z)) + 1)
    ...
    DO n_z = 1, end_indx_3-start_indx_3+1
        DO n_y = 1, end_indx_2-start_indx_2+1
            DO n_x = 1, end_indx_1-start_indx_1+1
                ...
                CALL copy_kernel( &
                opsDat1Local(dat1_base + ((n_x-1)*1) + ((n_y-1)*xdim1_copy_kernel*1) + ((n_z-1)*ydim1_copy_kernel*xdim1_copy_kernel*1)), &
                opsDat2Local(dat2_base + ((n_x-1)*1) + ((n_y-1)*xdim2_copy_kernel*1) + ((n_z-1)*ydim2_copy_kernel*xdim2_copy_kernel*1)) &
               )
...
!   ==========================
    ...
    CALL setKernelTime(222, kernelName//c_null_char, 0.0_8, 0.0_8, 0.0_4, 1)
    ...
    IF (&
         (xdim1 .NE. xdim1_copy_kernel_h) .OR. &
         (ydim1 .NE. ydim1_copy_kernel_h) .OR. &
         (xdim2 .NE. xdim2_copy_kernel_h) .OR. &
         (ydim2 .NE. ydim2_copy_kernel_h) &
       ) THEN
            xdim1_copy_kernel = xdim1
            xdim1_copy_kernel_h = xdim1
!$OMP TARGET UPDATE TO(xdim1_copy_kernel)
            ydim1_copy_kernel = ydim1
            ydim1_copy_kernel_h = ydim1
!$OMP TARGET UPDATE TO(ydim1_copy_kernel)
            xdim2_copy_kernel = xdim2
            xdim2_copy_kernel_h = xdim2
!$OMP TARGET UPDATE TO(xdim2_copy_kernel)
            ydim2_copy_kernel = ydim2
            ydim2_copy_kernel_h = ydim2
!$OMP TARGET UPDATE TO(ydim2_copy_kernel)
    END IF
...
!   ========================
    ...
    CALL setKernelTime(222, kernelName//c_null_char, t3__-t2__, t2__-t1__, transfer_total, 0)
