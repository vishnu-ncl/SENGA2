! Auto-generated at 2026-04-28 18:44:32.901416 by ops-translator

MODULE BOUNDT_KERNEL_EQF_XDIR_MODULE
...
!$OMP DECLARE TARGET(xdim1_boundt_kernel_eqF_xdir)
    INTEGER(KIND=4) :: xdim1_boundt_kernel_eqF_xdir
    INTEGER(KIND=4) :: xdim1_boundt_kernel_eqF_xdir_h = -1
!$OMP DECLARE TARGET(ydim1_boundt_kernel_eqF_xdir)
    INTEGER(KIND=4) :: ydim1_boundt_kernel_eqF_xdir
    INTEGER(KIND=4) :: ydim1_boundt_kernel_eqF_xdir_h = -1
#define OPS_ACC1(x,y,z) ((x) + (xdim1_boundt_kernel_eqF_xdir*(y)) + (xdim1_boundt_kernel_eqF_xdir*ydim1_boundt_kernel_eqF_xdir*(z)) + 1)

!$OMP DECLARE TARGET(xdim2_boundt_kernel_eqF_xdir)
    INTEGER(KIND=4) :: xdim2_boundt_kernel_eqF_xdir
    INTEGER(KIND=4) :: xdim2_boundt_kernel_eqF_xdir_h = -1
!$OMP DECLARE TARGET(ydim2_boundt_kernel_eqF_xdir)
    INTEGER(KIND=4) :: ydim2_boundt_kernel_eqF_xdir
    INTEGER(KIND=4) :: ydim2_boundt_kernel_eqF_xdir_h = -1
#define OPS_ACC2(x,y,z) ((x) + (xdim2_boundt_kernel_eqF_xdir*(y)) + (xdim2_boundt_kernel_eqF_xdir*ydim2_boundt_kernel_eqF_xdir*(z)) + 1)

!$OMP DECLARE TARGET(xdim3_boundt_kernel_eqF_xdir)
    INTEGER(KIND=4) :: xdim3_boundt_kernel_eqF_xdir
    INTEGER(KIND=4) :: xdim3_boundt_kernel_eqF_xdir_h = -1
!$OMP DECLARE TARGET(ydim3_boundt_kernel_eqF_xdir)
    INTEGER(KIND=4) :: ydim3_boundt_kernel_eqF_xdir
    INTEGER(KIND=4) :: ydim3_boundt_kernel_eqF_xdir_h = -1
#define OPS_ACC3(x,y,z) ((x) + (xdim3_boundt_kernel_eqF_xdir*(y)) + (xdim3_boundt_kernel_eqF_xdir*ydim3_boundt_kernel_eqF_xdir*(z)) + 1)

!$OMP DECLARE TARGET(xdim4_boundt_kernel_eqF_xdir)
    INTEGER(KIND=4) :: xdim4_boundt_kernel_eqF_xdir
    INTEGER(KIND=4) :: xdim4_boundt_kernel_eqF_xdir_h = -1
!$OMP DECLARE TARGET(ydim4_boundt_kernel_eqF_xdir)
    INTEGER(KIND=4) :: ydim4_boundt_kernel_eqF_xdir
    INTEGER(KIND=4) :: ydim4_boundt_kernel_eqF_xdir_h = -1
#define OPS_ACC4(x,y,z) ((x) + (xdim4_boundt_kernel_eqF_xdir*(y)) + (xdim4_boundt_kernel_eqF_xdir*ydim4_boundt_kernel_eqF_xdir*(z)) + 1)

!$OMP DECLARE TARGET(xdim5_boundt_kernel_eqF_xdir)
    INTEGER(KIND=4) :: xdim5_boundt_kernel_eqF_xdir
    INTEGER(KIND=4) :: xdim5_boundt_kernel_eqF_xdir_h = -1
!$OMP DECLARE TARGET(ydim5_boundt_kernel_eqF_xdir)
    INTEGER(KIND=4) :: ydim5_boundt_kernel_eqF_xdir
    INTEGER(KIND=4) :: ydim5_boundt_kernel_eqF_xdir_h = -1
#define OPS_ACC5(x,y,z) ((x) + (xdim5_boundt_kernel_eqF_xdir*(y)) + (xdim5_boundt_kernel_eqF_xdir*ydim5_boundt_kernel_eqF_xdir*(z)) + 1)

!$OMP DECLARE TARGET(xdim6_boundt_kernel_eqF_xdir)
    INTEGER(KIND=4) :: xdim6_boundt_kernel_eqF_xdir
    INTEGER(KIND=4) :: xdim6_boundt_kernel_eqF_xdir_h = -1
!$OMP DECLARE TARGET(ydim6_boundt_kernel_eqF_xdir)
    INTEGER(KIND=4) :: ydim6_boundt_kernel_eqF_xdir
    INTEGER(KIND=4) :: ydim6_boundt_kernel_eqF_xdir_h = -1
#define OPS_ACC6(x,y,z) ((x) + (xdim6_boundt_kernel_eqF_xdir*(y)) + (xdim6_boundt_kernel_eqF_xdir*ydim6_boundt_kernel_eqF_xdir*(z)) + 1)
...
SUBROUTINE boundt_kernel_eqF_xdir_wrap( &
    opsDat1Local, &
    opsDat2Local, &
    opsDat3Local, &
    opsDat4Local, &
    ...
    DO n_z = 1, end_indx_3-start_indx_3+1
        DO n_y = 1, end_indx_2-start_indx_2+1
            DO n_x = 1, end_indx_1-start_indx_1+1
                ...
                CALL boundt_kernel_eqF_xdir( &
                opsDat1Local(dat1_base + ((n_x-1)*1) + ((n_y-1)*xdim1_boundt_kernel_eqF_xdir*1) + ((n_z-1)*ydim1_boundt_kernel_eqF_xdir*xdim1_boundt_kernel_eqF_xdir*1)), &
                opsDat2Local(dat2_base + ((n_x-1)*1) + ((n_y-1)*xdim2_boundt_kernel_eqF_xdir*1) + ((n_z-1)*ydim2_boundt_kernel_eqF_xdir*xdim2_boundt_kernel_eqF_xdir*1)), &
                opsDat3Local(dat3_base + ((n_x-1)*1) + ((n_y-1)*xdim3_boundt_kernel_eqF_xdir*1) + ((n_z-1)*ydim3_boundt_kernel_eqF_xdir*xdim3_boundt_kernel_eqF_xdir*1)), &
                opsDat4Local(dat4_base + ((n_x-1)*1) + ((n_y-1)*xdim4_boundt_kernel_eqF_xdir*1) + ((n_z-1)*ydim4_boundt_kernel_eqF_xdir*xdim4_boundt_kernel_eqF_xdir*1)), &
                opsDat5Local(dat5_base + ((n_x-1)*0) + ((n_y-1)*xdim5_boundt_kernel_eqF_xdir*1) + ((n_z-1)*ydim5_boundt_kernel_eqF_xdir*xdim5_boundt_kernel_eqF_xdir*1)), &
                opsDat6Local(dat6_base + ((n_x-1)*0) + ((n_y-1)*xdim6_boundt_kernel_eqF_xdir*1) + ((n_z-1)*ydim6_boundt_kernel_eqF_xdir*xdim6_boundt_kernel_eqF_xdir*1)), &
                opsGblDat7Device(1), &
                opsGblDat8Device(1), &
                opsGblDat9Device(1), &
                opsGblDat10Device(1), &
...
!   ===============
#ifndef OPS_LAZY
SUBROUTINE boundt_kernel_eqF_xdir_host( userSubroutine, block, dim, range, &
    opsArg1, &
    opsArg2, &
    opsArg3, &
    opsArg4, &
...
#else
SUBROUTINE boundt_kernel_eqF_xdir_host_execute( descPtr )
    ...
    kernelName = "boundt_kernel_eqF_xdir"
...
!   ==========================
    ...
    CALL setKernelTime(520, kernelName//c_null_char, 0.0_8, 0.0_8, 0.0_4, 1)
    ...
    IF (&
         (xdim1 .NE. xdim1_boundt_kernel_eqF_xdir_h) .OR. &
         (ydim1 .NE. ydim1_boundt_kernel_eqF_xdir_h) .OR. &
         (xdim2 .NE. xdim2_boundt_kernel_eqF_xdir_h) .OR. &
         (ydim2 .NE. ydim2_boundt_kernel_eqF_xdir_h) .OR. &
         (xdim3 .NE. xdim3_boundt_kernel_eqF_xdir_h) .OR. &
         (ydim3 .NE. ydim3_boundt_kernel_eqF_xdir_h) .OR. &
         (xdim4 .NE. xdim4_boundt_kernel_eqF_xdir_h) .OR. &
         (ydim4 .NE. ydim4_boundt_kernel_eqF_xdir_h) .OR. &
         (xdim5 .NE. xdim5_boundt_kernel_eqF_xdir_h) .OR. &
         (ydim5 .NE. ydim5_boundt_kernel_eqF_xdir_h) .OR. &
         (xdim6 .NE. xdim6_boundt_kernel_eqF_xdir_h) .OR. &
         (ydim6 .NE. ydim6_boundt_kernel_eqF_xdir_h) &
       ) THEN
            xdim1_boundt_kernel_eqF_xdir = xdim1
            xdim1_boundt_kernel_eqF_xdir_h = xdim1
!$OMP TARGET UPDATE TO(xdim1_boundt_kernel_eqF_xdir)
            ydim1_boundt_kernel_eqF_xdir = ydim1
            ydim1_boundt_kernel_eqF_xdir_h = ydim1
!$OMP TARGET UPDATE TO(ydim1_boundt_kernel_eqF_xdir)
            xdim2_boundt_kernel_eqF_xdir = xdim2
            xdim2_boundt_kernel_eqF_xdir_h = xdim2
!$OMP TARGET UPDATE TO(xdim2_boundt_kernel_eqF_xdir)
            ydim2_boundt_kernel_eqF_xdir = ydim2
            ydim2_boundt_kernel_eqF_xdir_h = ydim2
!$OMP TARGET UPDATE TO(ydim2_boundt_kernel_eqF_xdir)
            xdim3_boundt_kernel_eqF_xdir = xdim3
            xdim3_boundt_kernel_eqF_xdir_h = xdim3
!$OMP TARGET UPDATE TO(xdim3_boundt_kernel_eqF_xdir)
            ydim3_boundt_kernel_eqF_xdir = ydim3
            ydim3_boundt_kernel_eqF_xdir_h = ydim3
!$OMP TARGET UPDATE TO(ydim3_boundt_kernel_eqF_xdir)
            xdim4_boundt_kernel_eqF_xdir = xdim4
            xdim4_boundt_kernel_eqF_xdir_h = xdim4
!$OMP TARGET UPDATE TO(xdim4_boundt_kernel_eqF_xdir)
            ydim4_boundt_kernel_eqF_xdir = ydim4
            ydim4_boundt_kernel_eqF_xdir_h = ydim4
!$OMP TARGET UPDATE TO(ydim4_boundt_kernel_eqF_xdir)
            xdim5_boundt_kernel_eqF_xdir = xdim5
            xdim5_boundt_kernel_eqF_xdir_h = xdim5
!$OMP TARGET UPDATE TO(xdim5_boundt_kernel_eqF_xdir)
            ydim5_boundt_kernel_eqF_xdir = ydim5
            ydim5_boundt_kernel_eqF_xdir_h = ydim5
!$OMP TARGET UPDATE TO(ydim5_boundt_kernel_eqF_xdir)
            xdim6_boundt_kernel_eqF_xdir = xdim6
            xdim6_boundt_kernel_eqF_xdir_h = xdim6
!$OMP TARGET UPDATE TO(xdim6_boundt_kernel_eqF_xdir)
            ydim6_boundt_kernel_eqF_xdir = ydim6
            ydim6_boundt_kernel_eqF_xdir_h = ydim6
!$OMP TARGET UPDATE TO(ydim6_boundt_kernel_eqF_xdir)
    END IF
...
!   ==============================
    CALL boundt_kernel_eqF_xdir_wrap( &
                        opsDat1Local, &
                        opsDat2Local, &
                        opsDat3Local, &
                        opsDat4Local, &
...
!   ========================
    ...
    CALL setKernelTime(520, kernelName//c_null_char, t3__-t2__, t2__-t1__, transfer_total, 0)
...
SUBROUTINE boundt_kernel_eqF_xdir_host( userSubroutine, block, dim, range, &
    ...
    namelit = "boundt_kernel_eqF_xdir"
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    14, 520, dim, 1, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(boundt_kernel_eqF_xdir_host_execute))

END SUBROUTINE
#endif

END MODULE BOUNDT_KERNEL_EQF_XDIR_MODULE
