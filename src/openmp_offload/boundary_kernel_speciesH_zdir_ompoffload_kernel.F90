! Auto-generated at 2026-04-28 18:44:30.257693 by ops-translator

MODULE BOUNDARY_KERNEL_SPECIESH_ZDIR_MODULE
...
!$OMP DECLARE TARGET(xdim1_boundary_kernel_speciesH_zdir)
    INTEGER(KIND=4) :: xdim1_boundary_kernel_speciesH_zdir
    INTEGER(KIND=4) :: xdim1_boundary_kernel_speciesH_zdir_h = -1
!$OMP DECLARE TARGET(ydim1_boundary_kernel_speciesH_zdir)
    INTEGER(KIND=4) :: ydim1_boundary_kernel_speciesH_zdir
    INTEGER(KIND=4) :: ydim1_boundary_kernel_speciesH_zdir_h = -1
#define OPS_ACC1(x,y,z) ((x) + (xdim1_boundary_kernel_speciesH_zdir*(y)) + (xdim1_boundary_kernel_speciesH_zdir*ydim1_boundary_kernel_speciesH_zdir*(z)) + 1)

!$OMP DECLARE TARGET(xdim2_boundary_kernel_speciesH_zdir)
    INTEGER(KIND=4) :: xdim2_boundary_kernel_speciesH_zdir
    INTEGER(KIND=4) :: xdim2_boundary_kernel_speciesH_zdir_h = -1
!$OMP DECLARE TARGET(ydim2_boundary_kernel_speciesH_zdir)
    INTEGER(KIND=4) :: ydim2_boundary_kernel_speciesH_zdir
    INTEGER(KIND=4) :: ydim2_boundary_kernel_speciesH_zdir_h = -1
#define OPS_ACC2(x,y,z) ((x) + (xdim2_boundary_kernel_speciesH_zdir*(y)) + (xdim2_boundary_kernel_speciesH_zdir*ydim2_boundary_kernel_speciesH_zdir*(z)) + 1)
...
SUBROUTINE boundary_kernel_speciesH_zdir_wrap( &
    opsDat1Local, &
    opsDat2Local, &
    dat1_base, &
    dat2_base, &
    ...
    DO n_z = 1, end_indx_3-start_indx_3+1
        DO n_y = 1, end_indx_2-start_indx_2+1
            DO n_x = 1, end_indx_1-start_indx_1+1
                ...
                CALL boundary_kernel_speciesH_zdir( &
                opsDat1Local(dat1_base + ((n_x-1)*1) + ((n_y-1)*xdim1_boundary_kernel_speciesH_zdir*1) + ((n_z-1)*ydim1_boundary_kernel_speciesH_zdir*xdim1_boundary_kernel_speciesH_zdir*1)), &
                opsDat2Local(dat2_base + ((n_x-1)*1) + ((n_y-1)*xdim2_boundary_kernel_speciesH_zdir*1) + ((n_z-1)*ydim2_boundary_kernel_speciesH_zdir*xdim2_boundary_kernel_speciesH_zdir*0)) &
               )
...
!   ===============
#ifndef OPS_LAZY
SUBROUTINE boundary_kernel_speciesH_zdir_host( userSubroutine, block, dim, range, &
    opsArg1, &
    opsArg2 &
    )
...
#else
SUBROUTINE boundary_kernel_speciesH_zdir_host_execute( descPtr )
    ...
    kernelName = "boundary_kernel_speciesH_zdir"
...
!   ==========================
    ...
    CALL setKernelTime(242, kernelName//c_null_char, 0.0_8, 0.0_8, 0.0_4, 1)
    ...
    IF (&
         (xdim1 .NE. xdim1_boundary_kernel_speciesH_zdir_h) .OR. &
         (ydim1 .NE. ydim1_boundary_kernel_speciesH_zdir_h) .OR. &
         (xdim2 .NE. xdim2_boundary_kernel_speciesH_zdir_h) .OR. &
         (ydim2 .NE. ydim2_boundary_kernel_speciesH_zdir_h) &
       ) THEN
            xdim1_boundary_kernel_speciesH_zdir = xdim1
            xdim1_boundary_kernel_speciesH_zdir_h = xdim1
!$OMP TARGET UPDATE TO(xdim1_boundary_kernel_speciesH_zdir)
            ydim1_boundary_kernel_speciesH_zdir = ydim1
            ydim1_boundary_kernel_speciesH_zdir_h = ydim1
!$OMP TARGET UPDATE TO(ydim1_boundary_kernel_speciesH_zdir)
            xdim2_boundary_kernel_speciesH_zdir = xdim2
            xdim2_boundary_kernel_speciesH_zdir_h = xdim2
!$OMP TARGET UPDATE TO(xdim2_boundary_kernel_speciesH_zdir)
            ydim2_boundary_kernel_speciesH_zdir = ydim2
            ydim2_boundary_kernel_speciesH_zdir_h = ydim2
!$OMP TARGET UPDATE TO(ydim2_boundary_kernel_speciesH_zdir)
    END IF
...
!   ==============================
    CALL boundary_kernel_speciesH_zdir_wrap( &
                        opsDat1Local, &
                        opsDat2Local, &
                        dat1_base, &
                        dat2_base, &
...
!   ========================
    ...
    CALL setKernelTime(242, kernelName//c_null_char, t3__-t2__, t2__-t1__, transfer_total, 0)
...
SUBROUTINE boundary_kernel_speciesH_zdir_host( userSubroutine, block, dim, range, &
    ...
    namelit = "boundary_kernel_speciesH_zdir"
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    2, 242, dim, 1, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(boundary_kernel_speciesH_zdir_host_execute))

END SUBROUTINE
#endif

END MODULE BOUNDARY_KERNEL_SPECIESH_ZDIR_MODULE
