! Auto-generated at 2026-04-28 18:44:32.653047 by ops-translator

MODULE BOUNDS_KERNEL_EQAF_ZR_MODULE
...
!$OMP DECLARE TARGET(xdim1_bounds_kernel_eqAF_zr)
    INTEGER(KIND=4) :: xdim1_bounds_kernel_eqAF_zr
    INTEGER(KIND=4) :: xdim1_bounds_kernel_eqAF_zr_h = -1
!$OMP DECLARE TARGET(ydim1_bounds_kernel_eqAF_zr)
    INTEGER(KIND=4) :: ydim1_bounds_kernel_eqAF_zr
    INTEGER(KIND=4) :: ydim1_bounds_kernel_eqAF_zr_h = -1
#define OPS_ACC1(x,y,z) ((x) + (xdim1_bounds_kernel_eqAF_zr*(y)) + (xdim1_bounds_kernel_eqAF_zr*ydim1_bounds_kernel_eqAF_zr*(z)) + 1)

!$OMP DECLARE TARGET(xdim2_bounds_kernel_eqAF_zr)
    INTEGER(KIND=4) :: xdim2_bounds_kernel_eqAF_zr
    INTEGER(KIND=4) :: xdim2_bounds_kernel_eqAF_zr_h = -1
!$OMP DECLARE TARGET(ydim2_bounds_kernel_eqAF_zr)
    INTEGER(KIND=4) :: ydim2_bounds_kernel_eqAF_zr
    INTEGER(KIND=4) :: ydim2_bounds_kernel_eqAF_zr_h = -1
#define OPS_ACC2(x,y,z) ((x) + (xdim2_bounds_kernel_eqAF_zr*(y)) + (xdim2_bounds_kernel_eqAF_zr*ydim2_bounds_kernel_eqAF_zr*(z)) + 1)
...
SUBROUTINE bounds_kernel_eqAF_zr(strtzr,trun)

    real(kind=8), dimension(1) :: strtzr, trun

    strtzr(OPS_ACC1(0,0,0)) = (48.0_8*trun(OPS_ACC2(0,0,-1)) - 36.0_8*trun(OPS_ACC2(0,0,-2))  &
                             + 16.0_8*trun(OPS_ACC2(0,0,-3)) - 3.0_8*trun(OPS_ACC2(0,0,-4)))/25.0_8

    trun(OPS_ACC2(0,0,0)) = strtzr(OPS_ACC1(0,0,0))
...
SUBROUTINE bounds_kernel_eqAF_zr_wrap( &
    opsDat1Local, &
    opsDat2Local, &
    dat1_base, &
    dat2_base, &
    ...
    DO n_z = 1, end_indx_3-start_indx_3+1
        DO n_y = 1, end_indx_2-start_indx_2+1
            DO n_x = 1, end_indx_1-start_indx_1+1
                ...
                CALL bounds_kernel_eqAF_zr( &
                opsDat1Local(dat1_base + ((n_x-1)*1) + ((n_y-1)*xdim1_bounds_kernel_eqAF_zr*1) + ((n_z-1)*ydim1_bounds_kernel_eqAF_zr*xdim1_bounds_kernel_eqAF_zr*0)), &
                opsDat2Local(dat2_base + ((n_x-1)*1) + ((n_y-1)*xdim2_bounds_kernel_eqAF_zr*1) + ((n_z-1)*ydim2_bounds_kernel_eqAF_zr*xdim2_bounds_kernel_eqAF_zr*1)) &
               )
...
!   ===============
#ifndef OPS_LAZY
SUBROUTINE bounds_kernel_eqAF_zr_host( userSubroutine, block, dim, range, &
    opsArg1, &
    opsArg2 &
    )
...
#else
SUBROUTINE bounds_kernel_eqAF_zr_host_execute( descPtr )
    ...
    kernelName = "bounds_kernel_eqAF_zr"
...
!   ==========================
    ...
    CALL setKernelTime(484, kernelName//c_null_char, 0.0_8, 0.0_8, 0.0_4, 1)
    ...
    IF (&
         (xdim1 .NE. xdim1_bounds_kernel_eqAF_zr_h) .OR. &
         (ydim1 .NE. ydim1_bounds_kernel_eqAF_zr_h) .OR. &
         (xdim2 .NE. xdim2_bounds_kernel_eqAF_zr_h) .OR. &
         (ydim2 .NE. ydim2_bounds_kernel_eqAF_zr_h) &
       ) THEN
            xdim1_bounds_kernel_eqAF_zr = xdim1
            xdim1_bounds_kernel_eqAF_zr_h = xdim1
!$OMP TARGET UPDATE TO(xdim1_bounds_kernel_eqAF_zr)
            ydim1_bounds_kernel_eqAF_zr = ydim1
            ydim1_bounds_kernel_eqAF_zr_h = ydim1
!$OMP TARGET UPDATE TO(ydim1_bounds_kernel_eqAF_zr)
            xdim2_bounds_kernel_eqAF_zr = xdim2
            xdim2_bounds_kernel_eqAF_zr_h = xdim2
!$OMP TARGET UPDATE TO(xdim2_bounds_kernel_eqAF_zr)
            ydim2_bounds_kernel_eqAF_zr = ydim2
            ydim2_bounds_kernel_eqAF_zr_h = ydim2
!$OMP TARGET UPDATE TO(ydim2_bounds_kernel_eqAF_zr)
    END IF
...
!   ==============================
    CALL bounds_kernel_eqAF_zr_wrap( &
                        opsDat1Local, &
                        opsDat2Local, &
                        dat1_base, &
                        dat2_base, &
...
!   ========================
    ...
    CALL setKernelTime(484, kernelName//c_null_char, t3__-t2__, t2__-t1__, transfer_total, 0)
...
SUBROUTINE bounds_kernel_eqAF_zr_host( userSubroutine, block, dim, range, &
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    2, 484, dim, 1, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(bounds_kernel_eqAF_zr_host_execute))

END SUBROUTINE
#endif

END MODULE BOUNDS_KERNEL_EQAF_ZR_MODULE
