! Auto-generated at 2026-04-28 18:44:31.604121 by ops-translator

MODULE BOUNDS_KERNEL_EQAF_YL_MODULE
...
!$OMP DECLARE TARGET(xdim1_bounds_kernel_eqAF_yl)
    INTEGER(KIND=4) :: xdim1_bounds_kernel_eqAF_yl
    INTEGER(KIND=4) :: xdim1_bounds_kernel_eqAF_yl_h = -1
!$OMP DECLARE TARGET(ydim1_bounds_kernel_eqAF_yl)
    INTEGER(KIND=4) :: ydim1_bounds_kernel_eqAF_yl
    INTEGER(KIND=4) :: ydim1_bounds_kernel_eqAF_yl_h = -1
#define OPS_ACC1(x,y,z) ((x) + (xdim1_bounds_kernel_eqAF_yl*(y)) + (xdim1_bounds_kernel_eqAF_yl*ydim1_bounds_kernel_eqAF_yl*(z)) + 1)

!$OMP DECLARE TARGET(xdim2_bounds_kernel_eqAF_yl)
    INTEGER(KIND=4) :: xdim2_bounds_kernel_eqAF_yl
    INTEGER(KIND=4) :: xdim2_bounds_kernel_eqAF_yl_h = -1
!$OMP DECLARE TARGET(ydim2_bounds_kernel_eqAF_yl)
    INTEGER(KIND=4) :: ydim2_bounds_kernel_eqAF_yl
    INTEGER(KIND=4) :: ydim2_bounds_kernel_eqAF_yl_h = -1
#define OPS_ACC2(x,y,z) ((x) + (xdim2_bounds_kernel_eqAF_yl*(y)) + (xdim2_bounds_kernel_eqAF_yl*ydim2_bounds_kernel_eqAF_yl*(z)) + 1)
...
SUBROUTINE bounds_kernel_eqAF_yl(strtyl,trun)

    real(kind=8), dimension(1) :: strtyl, trun

    strtyl(OPS_ACC1(0,0,0)) = (48.0_8*trun(OPS_ACC2(0,1,0)) - 36.0_8*trun(OPS_ACC2(0,2,0))  &
                             + 16.0_8*trun(OPS_ACC2(0,3,0)) - 3.0_8*trun(OPS_ACC2(0,4,0)))/25.0_8

    trun(OPS_ACC2(0,0,0)) = strtyl(OPS_ACC1(0,0,0))
...
SUBROUTINE bounds_kernel_eqAF_yl_wrap( &
    opsDat1Local, &
    opsDat2Local, &
    dat1_base, &
    dat2_base, &
    ...
    DO n_z = 1, end_indx_3-start_indx_3+1
        DO n_y = 1, end_indx_2-start_indx_2+1
            DO n_x = 1, end_indx_1-start_indx_1+1
                ...
                CALL bounds_kernel_eqAF_yl( &
                opsDat1Local(dat1_base + ((n_x-1)*1) + ((n_y-1)*xdim1_bounds_kernel_eqAF_yl*0) + ((n_z-1)*ydim1_bounds_kernel_eqAF_yl*xdim1_bounds_kernel_eqAF_yl*1)), &
                opsDat2Local(dat2_base + ((n_x-1)*1) + ((n_y-1)*xdim2_bounds_kernel_eqAF_yl*1) + ((n_z-1)*ydim2_bounds_kernel_eqAF_yl*xdim2_bounds_kernel_eqAF_yl*1)) &
               )
...
!   ===============
#ifndef OPS_LAZY
SUBROUTINE bounds_kernel_eqAF_yl_host( userSubroutine, block, dim, range, &
    opsArg1, &
    opsArg2 &
    )
...
#else
SUBROUTINE bounds_kernel_eqAF_yl_host_execute( descPtr )
    ...
    kernelName = "bounds_kernel_eqAF_yl"
...
!   ==========================
    ...
    IF (&
         (xdim1 .NE. xdim1_bounds_kernel_eqAF_yl_h) .OR. &
         (ydim1 .NE. ydim1_bounds_kernel_eqAF_yl_h) .OR. &
         (xdim2 .NE. xdim2_bounds_kernel_eqAF_yl_h) .OR. &
         (ydim2 .NE. ydim2_bounds_kernel_eqAF_yl_h) &
       ) THEN
            xdim1_bounds_kernel_eqAF_yl = xdim1
            xdim1_bounds_kernel_eqAF_yl_h = xdim1
!$OMP TARGET UPDATE TO(xdim1_bounds_kernel_eqAF_yl)
            ydim1_bounds_kernel_eqAF_yl = ydim1
            ydim1_bounds_kernel_eqAF_yl_h = ydim1
!$OMP TARGET UPDATE TO(ydim1_bounds_kernel_eqAF_yl)
            xdim2_bounds_kernel_eqAF_yl = xdim2
            xdim2_bounds_kernel_eqAF_yl_h = xdim2
!$OMP TARGET UPDATE TO(xdim2_bounds_kernel_eqAF_yl)
            ydim2_bounds_kernel_eqAF_yl = ydim2
            ydim2_bounds_kernel_eqAF_yl_h = ydim2
!$OMP TARGET UPDATE TO(ydim2_bounds_kernel_eqAF_yl)
    END IF
...
!   ==============================
    CALL bounds_kernel_eqAF_yl_wrap( &
                        opsDat1Local, &
                        opsDat2Local, &
                        dat1_base, &
                        dat2_base, &
...
SUBROUTINE bounds_kernel_eqAF_yl_host( userSubroutine, block, dim, range, &
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    2, 411, dim, 1, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(bounds_kernel_eqAF_yl_host_execute))

END SUBROUTINE
#endif

END MODULE BOUNDS_KERNEL_EQAF_YL_MODULE
