! Auto-generated at 2026-04-28 18:44:30.171070 by ops-translator

MODULE COPY_KERNEL_YDIR_MODULE
...
!$OMP DECLARE TARGET(xdim1_copy_kernel_ydir)
    INTEGER(KIND=4) :: xdim1_copy_kernel_ydir
    INTEGER(KIND=4) :: xdim1_copy_kernel_ydir_h = -1
!$OMP DECLARE TARGET(ydim1_copy_kernel_ydir)
    INTEGER(KIND=4) :: ydim1_copy_kernel_ydir
    INTEGER(KIND=4) :: ydim1_copy_kernel_ydir_h = -1
#define OPS_ACC1(x,y,z) ((x) + (xdim1_copy_kernel_ydir*(y)) + (xdim1_copy_kernel_ydir*ydim1_copy_kernel_ydir*(z)) + 1)

!$OMP DECLARE TARGET(xdim2_copy_kernel_ydir)
    INTEGER(KIND=4) :: xdim2_copy_kernel_ydir
    INTEGER(KIND=4) :: xdim2_copy_kernel_ydir_h = -1
!$OMP DECLARE TARGET(ydim2_copy_kernel_ydir)
    INTEGER(KIND=4) :: ydim2_copy_kernel_ydir
    INTEGER(KIND=4) :: ydim2_copy_kernel_ydir_h = -1
#define OPS_ACC2(x,y,z) ((x) + (xdim2_copy_kernel_ydir*(y)) + (xdim2_copy_kernel_ydir*ydim2_copy_kernel_ydir*(z)) + 1)
...
SUBROUTINE copy_kernel_ydir_wrap( &
    opsDat1Local, &
    opsDat2Local, &
    dat1_base, &
    dat2_base, &
    ...
    DO n_z = 1, end_indx_3-start_indx_3+1
        DO n_y = 1, end_indx_2-start_indx_2+1
            DO n_x = 1, end_indx_1-start_indx_1+1
                ...
                CALL copy_kernel_ydir( &
                opsDat1Local(dat1_base + ((n_x-1)*1) + ((n_y-1)*xdim1_copy_kernel_ydir*0) + ((n_z-1)*ydim1_copy_kernel_ydir*xdim1_copy_kernel_ydir*1)), &
                opsDat2Local(dat2_base + ((n_x-1)*1) + ((n_y-1)*xdim2_copy_kernel_ydir*1) + ((n_z-1)*ydim2_copy_kernel_ydir*xdim2_copy_kernel_ydir*1)) &
               )
...
!   ===============
#ifndef OPS_LAZY
SUBROUTINE copy_kernel_ydir_host( userSubroutine, block, dim, range, &
    opsArg1, &
    opsArg2 &
    )
...
#else
SUBROUTINE copy_kernel_ydir_host_execute( descPtr )
...
!   ==========================
    ...
    CALL setKernelTime(224, kernelName//c_null_char, 0.0_8, 0.0_8, 0.0_4, 1)
    ...
    IF (&
         (xdim1 .NE. xdim1_copy_kernel_ydir_h) .OR. &
         (ydim1 .NE. ydim1_copy_kernel_ydir_h) .OR. &
         (xdim2 .NE. xdim2_copy_kernel_ydir_h) .OR. &
         (ydim2 .NE. ydim2_copy_kernel_ydir_h) &
       ) THEN
            xdim1_copy_kernel_ydir = xdim1
            xdim1_copy_kernel_ydir_h = xdim1
!$OMP TARGET UPDATE TO(xdim1_copy_kernel_ydir)
            ydim1_copy_kernel_ydir = ydim1
            ydim1_copy_kernel_ydir_h = ydim1
!$OMP TARGET UPDATE TO(ydim1_copy_kernel_ydir)
            xdim2_copy_kernel_ydir = xdim2
            xdim2_copy_kernel_ydir_h = xdim2
!$OMP TARGET UPDATE TO(xdim2_copy_kernel_ydir)
            ydim2_copy_kernel_ydir = ydim2
            ydim2_copy_kernel_ydir_h = ydim2
!$OMP TARGET UPDATE TO(ydim2_copy_kernel_ydir)
    END IF
...
!   ========================
    ...
    CALL setKernelTime(224, kernelName//c_null_char, t3__-t2__, t2__-t1__, transfer_total, 0)
...
SUBROUTINE copy_kernel_ydir_host( userSubroutine, block, dim, range, &
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    2, 224, dim, 1, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(copy_kernel_ydir_host_execute))

END SUBROUTINE
#endif

END MODULE COPY_KERNEL_YDIR_MODULE
