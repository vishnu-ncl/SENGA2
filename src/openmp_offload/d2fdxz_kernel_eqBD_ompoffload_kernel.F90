! Auto-generated at 2026-04-28 18:44:29.548920 by ops-translator

MODULE D2FDXZ_KERNEL_EQBD_MODULE
...
!$OMP DECLARE TARGET(xdim1_d2fdxz_kernel_eqBD)
    INTEGER(KIND=4) :: xdim1_d2fdxz_kernel_eqBD
    INTEGER(KIND=4) :: xdim1_d2fdxz_kernel_eqBD_h = -1
!$OMP DECLARE TARGET(ydim1_d2fdxz_kernel_eqBD)
    INTEGER(KIND=4) :: ydim1_d2fdxz_kernel_eqBD
    INTEGER(KIND=4) :: ydim1_d2fdxz_kernel_eqBD_h = -1
#define OPS_ACC1(x,y,z) ((x) + (xdim1_d2fdxz_kernel_eqBD*(y)) + (xdim1_d2fdxz_kernel_eqBD*ydim1_d2fdxz_kernel_eqBD*(z)) + 1)

!$OMP DECLARE TARGET(xdim2_d2fdxz_kernel_eqBD)
    INTEGER(KIND=4) :: xdim2_d2fdxz_kernel_eqBD
    INTEGER(KIND=4) :: xdim2_d2fdxz_kernel_eqBD_h = -1
!$OMP DECLARE TARGET(ydim2_d2fdxz_kernel_eqBD)
    INTEGER(KIND=4) :: ydim2_d2fdxz_kernel_eqBD
    INTEGER(KIND=4) :: ydim2_d2fdxz_kernel_eqBD_h = -1
#define OPS_ACC2(x,y,z) ((x) + (xdim2_d2fdxz_kernel_eqBD*(y)) + (xdim2_d2fdxz_kernel_eqBD*ydim2_d2fdxz_kernel_eqBD*(z)) + 1)
...
SUBROUTINE d2fdxz_kernel_eqBD_wrap( &
    opsDat1Local, &
    opsDat2Local, &
    opsGblDat3Device, &
    opsGblDat4Device, &
...
!$OMP TARGET TEAMS LOOP COLLAPSE(3) PRIVATE(n_x,n_y,n_z,idx_local)
    ...
    DO n_z = 1, end_indx_3-start_indx_3+1
        ...
        idx_local = [idx_1+n_x-1, idx_2+n_y-1, idx_3+n_z-1]
                ...
                CALL d2fdxz_kernel_eqBD( &
                opsDat1Local(dat1_base + ((n_x-1)*1) + ((n_y-1)*xdim1_d2fdxz_kernel_eqBD*1) + ((n_z-1)*ydim1_d2fdxz_kernel_eqBD*xdim1_d2fdxz_kernel_eqBD*1)), &
                opsDat2Local(dat2_base + ((n_x-1)*1) + ((n_y-1)*xdim2_d2fdxz_kernel_eqBD*1) + ((n_z-1)*ydim2_d2fdxz_kernel_eqBD*xdim2_d2fdxz_kernel_eqBD*1)), &
                opsGblDat3Device, &
                opsGblDat4Device, &
                idx_local &
               )
...
!   ===============
#ifndef OPS_LAZY
SUBROUTINE d2fdxz_kernel_eqBD_host( userSubroutine, block, dim, range, &
    opsArg1, &
    opsArg2, &
    opsArg3, &
    opsArg4, &
...
#else
SUBROUTINE d2fdxz_kernel_eqBD_host_execute( descPtr )
...
!   ==========================
    ...
    CALL setKernelTime(132, kernelName//c_null_char, 0.0_8, 0.0_8, 0.0_4, 1)
    ...
    IF (&
         (xdim1 .NE. xdim1_d2fdxz_kernel_eqBD_h) .OR. &
         (ydim1 .NE. ydim1_d2fdxz_kernel_eqBD_h) .OR. &
         (xdim2 .NE. xdim2_d2fdxz_kernel_eqBD_h) .OR. &
         (ydim2 .NE. ydim2_d2fdxz_kernel_eqBD_h) &
       ) THEN
            xdim1_d2fdxz_kernel_eqBD = xdim1
            xdim1_d2fdxz_kernel_eqBD_h = xdim1
!$OMP TARGET UPDATE TO(xdim1_d2fdxz_kernel_eqBD)
            ydim1_d2fdxz_kernel_eqBD = ydim1
            ydim1_d2fdxz_kernel_eqBD_h = ydim1
!$OMP TARGET UPDATE TO(ydim1_d2fdxz_kernel_eqBD)
            xdim2_d2fdxz_kernel_eqBD = xdim2
            xdim2_d2fdxz_kernel_eqBD_h = xdim2
!$OMP TARGET UPDATE TO(xdim2_d2fdxz_kernel_eqBD)
            ydim2_d2fdxz_kernel_eqBD = ydim2
            ydim2_d2fdxz_kernel_eqBD_h = ydim2
!$OMP TARGET UPDATE TO(ydim2_d2fdxz_kernel_eqBD)
    END IF
...
!   ========================
    ...
    CALL setKernelTime(132, kernelName//c_null_char, t3__-t2__, t2__-t1__, transfer_total, 0)
...
SUBROUTINE d2fdxz_kernel_eqBD_host( userSubroutine, block, dim, range, &
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    5, 132, dim, 1, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(d2fdxz_kernel_eqBD_host_execute))

END SUBROUTINE
#endif

END MODULE D2FDXZ_KERNEL_EQBD_MODULE
