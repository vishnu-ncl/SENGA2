! Auto-generated at 2026-04-28 18:44:29.710286 by ops-translator

MODULE D2FDYZ_KERNEL_EQY_MODULE
...
!$OMP DECLARE TARGET(xdim1_d2fdyz_kernel_eqY)
    INTEGER(KIND=4) :: xdim1_d2fdyz_kernel_eqY
    INTEGER(KIND=4) :: xdim1_d2fdyz_kernel_eqY_h = -1
!$OMP DECLARE TARGET(ydim1_d2fdyz_kernel_eqY)
    INTEGER(KIND=4) :: ydim1_d2fdyz_kernel_eqY
    INTEGER(KIND=4) :: ydim1_d2fdyz_kernel_eqY_h = -1
#define OPS_ACC1(x,y,z) ((x) + (xdim1_d2fdyz_kernel_eqY*(y)) + (xdim1_d2fdyz_kernel_eqY*ydim1_d2fdyz_kernel_eqY*(z)) + 1)

!$OMP DECLARE TARGET(xdim2_d2fdyz_kernel_eqY)
    INTEGER(KIND=4) :: xdim2_d2fdyz_kernel_eqY
    INTEGER(KIND=4) :: xdim2_d2fdyz_kernel_eqY_h = -1
!$OMP DECLARE TARGET(ydim2_d2fdyz_kernel_eqY)
    INTEGER(KIND=4) :: ydim2_d2fdyz_kernel_eqY
    INTEGER(KIND=4) :: ydim2_d2fdyz_kernel_eqY_h = -1
#define OPS_ACC2(x,y,z) ((x) + (xdim2_d2fdyz_kernel_eqY*(y)) + (xdim2_d2fdyz_kernel_eqY*ydim2_d2fdyz_kernel_eqY*(z)) + 1)
...
SUBROUTINE d2fdyz_kernel_eqY_wrap( &
    opsDat1Local, &
    opsDat2Local, &
    dat1_base, &
    dat2_base, &
    ...
    DO n_z = 1, end_indx_3-start_indx_3+1
        DO n_y = 1, end_indx_2-start_indx_2+1
            DO n_x = 1, end_indx_1-start_indx_1+1
                ...
                CALL d2fdyz_kernel_eqY( &
                opsDat1Local(dat1_base + ((n_x-1)*1) + ((n_y-1)*xdim1_d2fdyz_kernel_eqY*1) + ((n_z-1)*ydim1_d2fdyz_kernel_eqY*xdim1_d2fdyz_kernel_eqY*1)), &
                opsDat2Local(dat2_base + ((n_x-1)*1) + ((n_y-1)*xdim2_d2fdyz_kernel_eqY*1) + ((n_z-1)*ydim2_d2fdyz_kernel_eqY*xdim2_d2fdyz_kernel_eqY*1)) &
               )
...
!   ===============
#ifndef OPS_LAZY
SUBROUTINE d2fdyz_kernel_eqY_host( userSubroutine, block, dim, range, &
    opsArg1, &
    opsArg2 &
    )
...
#else
SUBROUTINE d2fdyz_kernel_eqY_host_execute( descPtr )
...
!   ==========================
    ...
    CALL setKernelTime(160, kernelName//c_null_char, 0.0_8, 0.0_8, 0.0_4, 1)
    ...
    IF (&
         (xdim1 .NE. xdim1_d2fdyz_kernel_eqY_h) .OR. &
         (ydim1 .NE. ydim1_d2fdyz_kernel_eqY_h) .OR. &
         (xdim2 .NE. xdim2_d2fdyz_kernel_eqY_h) .OR. &
         (ydim2 .NE. ydim2_d2fdyz_kernel_eqY_h) &
       ) THEN
            xdim1_d2fdyz_kernel_eqY = xdim1
            xdim1_d2fdyz_kernel_eqY_h = xdim1
!$OMP TARGET UPDATE TO(xdim1_d2fdyz_kernel_eqY)
            ydim1_d2fdyz_kernel_eqY = ydim1
            ydim1_d2fdyz_kernel_eqY_h = ydim1
!$OMP TARGET UPDATE TO(ydim1_d2fdyz_kernel_eqY)
            xdim2_d2fdyz_kernel_eqY = xdim2
            xdim2_d2fdyz_kernel_eqY_h = xdim2
!$OMP TARGET UPDATE TO(xdim2_d2fdyz_kernel_eqY)
            ydim2_d2fdyz_kernel_eqY = ydim2
            ydim2_d2fdyz_kernel_eqY_h = ydim2
!$OMP TARGET UPDATE TO(ydim2_d2fdyz_kernel_eqY)
    END IF
...
!   ========================
    ...
    CALL setKernelTime(160, kernelName//c_null_char, t3__-t2__, t2__-t1__, transfer_total, 0)
...
SUBROUTINE d2fdyz_kernel_eqY_host( userSubroutine, block, dim, range, &
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    2, 160, dim, 1, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(d2fdyz_kernel_eqY_host_execute))

END SUBROUTINE
#endif

END MODULE D2FDYZ_KERNEL_EQY_MODULE
