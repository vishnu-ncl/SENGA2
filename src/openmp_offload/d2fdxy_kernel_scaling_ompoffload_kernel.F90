! Auto-generated at 2026-04-28 18:44:29.034378 by ops-translator

MODULE D2FDXY_KERNEL_SCALING_MODULE
...
!$OMP DECLARE TARGET(xdim1_d2fdxy_kernel_scaling)
    INTEGER(KIND=4) :: xdim1_d2fdxy_kernel_scaling
    INTEGER(KIND=4) :: xdim1_d2fdxy_kernel_scaling_h = -1
!$OMP DECLARE TARGET(ydim1_d2fdxy_kernel_scaling)
    INTEGER(KIND=4) :: ydim1_d2fdxy_kernel_scaling
    INTEGER(KIND=4) :: ydim1_d2fdxy_kernel_scaling_h = -1
#define OPS_ACC1(x,y,z) ((x) + (xdim1_d2fdxy_kernel_scaling*(y)) + (xdim1_d2fdxy_kernel_scaling*ydim1_d2fdxy_kernel_scaling*(z)) + 1)
...
SUBROUTINE d2fdxy_kernel_scaling_wrap( &
    opsDat1Local, &
    dat1_base, &
    dat1_dim, &
    start_indx, &
    ...
    DO n_z = 1, end_indx_3-start_indx_3+1
        DO n_y = 1, end_indx_2-start_indx_2+1
            DO n_x = 1, end_indx_1-start_indx_1+1
                ...
                CALL d2fdxy_kernel_scaling( &
                opsDat1Local(dat1_base + ((n_x-1)*1) + ((n_y-1)*xdim1_d2fdxy_kernel_scaling*1) + ((n_z-1)*ydim1_d2fdxy_kernel_scaling*xdim1_d2fdxy_kernel_scaling*1)) &
               )
...
!   ===============
#ifndef OPS_LAZY
SUBROUTINE d2fdxy_kernel_scaling_host( userSubroutine, block, dim, range, &
    opsArg1 &
    )
...
#else
SUBROUTINE d2fdxy_kernel_scaling_host_execute( descPtr )
    ...
    kernelName = "d2fdxy_kernel_scaling"
...
!   ==========================
    ...
    CALL setKernelTime(74, kernelName//c_null_char, 0.0_8, 0.0_8, 0.0_4, 1)
    ...
    IF (&
         (xdim1 .NE. xdim1_d2fdxy_kernel_scaling_h) .OR. &
         (ydim1 .NE. ydim1_d2fdxy_kernel_scaling_h) &
       ) THEN
            xdim1_d2fdxy_kernel_scaling = xdim1
            xdim1_d2fdxy_kernel_scaling_h = xdim1
!$OMP TARGET UPDATE TO(xdim1_d2fdxy_kernel_scaling)
            ydim1_d2fdxy_kernel_scaling = ydim1
            ydim1_d2fdxy_kernel_scaling_h = ydim1
!$OMP TARGET UPDATE TO(ydim1_d2fdxy_kernel_scaling)
    END IF
...
!   ==============================
    CALL d2fdxy_kernel_scaling_wrap( &
                        opsDat1Local, &
                        dat1_base, &
                        opsDat1Cardinality, &
                        start_indx, &
...
!   ========================
    ...
    CALL setKernelTime(74, kernelName//c_null_char, t3__-t2__, t2__-t1__, transfer_total, 0)
...
SUBROUTINE d2fdxy_kernel_scaling_host( userSubroutine, block, dim, range, &
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    1, 74, dim, 1, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(d2fdxy_kernel_scaling_host_execute))

END SUBROUTINE
#endif

END MODULE D2FDXY_KERNEL_SCALING_MODULE
