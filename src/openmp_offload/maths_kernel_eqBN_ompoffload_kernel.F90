! Auto-generated at 2026-04-28 18:44:33.022393 by ops-translator
...
!$OMP DECLARE TARGET(xdim1_maths_kernel_eqBN)
    INTEGER(KIND=4) :: xdim1_maths_kernel_eqBN
    INTEGER(KIND=4) :: xdim1_maths_kernel_eqBN_h = -1
!$OMP DECLARE TARGET(ydim1_maths_kernel_eqBN)
    INTEGER(KIND=4) :: ydim1_maths_kernel_eqBN
    INTEGER(KIND=4) :: ydim1_maths_kernel_eqBN_h = -1
#define OPS_ACC1(x,y,z) ((x) + (xdim1_maths_kernel_eqBN*(y)) + (xdim1_maths_kernel_eqBN*ydim1_maths_kernel_eqBN*(z)) + 1)

!$OMP DECLARE TARGET(xdim2_maths_kernel_eqBN)
    INTEGER(KIND=4) :: xdim2_maths_kernel_eqBN
    INTEGER(KIND=4) :: xdim2_maths_kernel_eqBN_h = -1
!$OMP DECLARE TARGET(ydim2_maths_kernel_eqBN)
    INTEGER(KIND=4) :: ydim2_maths_kernel_eqBN
    INTEGER(KIND=4) :: ydim2_maths_kernel_eqBN_h = -1
#define OPS_ACC2(x,y,z) ((x) + (xdim2_maths_kernel_eqBN*(y)) + (xdim2_maths_kernel_eqBN*ydim2_maths_kernel_eqBN*(z)) + 1)

!$OMP DECLARE TARGET(xdim3_maths_kernel_eqBN)
    INTEGER(KIND=4) :: xdim3_maths_kernel_eqBN
    INTEGER(KIND=4) :: xdim3_maths_kernel_eqBN_h = -1
!$OMP DECLARE TARGET(ydim3_maths_kernel_eqBN)
    INTEGER(KIND=4) :: ydim3_maths_kernel_eqBN
    INTEGER(KIND=4) :: ydim3_maths_kernel_eqBN_h = -1
...
SUBROUTINE maths_kernel_eqBN_wrap( &
    opsDat1Local, &
    opsDat2Local, &
    opsDat3Local, &
    opsGblDat4Device, &
    ...
    DO n_z = 1, end_indx_3-start_indx_3+1
        DO n_y = 1, end_indx_2-start_indx_2+1
            DO n_x = 1, end_indx_1-start_indx_1+1
                ...
                CALL maths_kernel_eqBN( &
                opsDat1Local(dat1_base + ((n_x-1)*1) + ((n_y-1)*xdim1_maths_kernel_eqBN*1) + ((n_z-1)*ydim1_maths_kernel_eqBN*xdim1_maths_kernel_eqBN*1)), &
                opsDat2Local(dat2_base + ((n_x-1)*1) + ((n_y-1)*xdim2_maths_kernel_eqBN*1) + ((n_z-1)*ydim2_maths_kernel_eqBN*xdim2_maths_kernel_eqBN*1)), &
                opsDat3Local(dat3_base + ((n_x-1)*1) + ((n_y-1)*xdim3_maths_kernel_eqBN*1) + ((n_z-1)*ydim3_maths_kernel_eqBN*xdim3_maths_kernel_eqBN*1)), &
                opsGblDat4Device(1), &
                opsGblDat5Device(1), &
                opsGblDat6Device(1), &
                opsGblDat7Device(1), &
...
!   ===============
#ifndef OPS_LAZY
SUBROUTINE maths_kernel_eqBN_host( userSubroutine, block, dim, range, &
    opsArg1, &
    opsArg2, &
    opsArg3, &
    opsArg4, &
...
#else
SUBROUTINE maths_kernel_eqBN_host_execute( descPtr )
...
!   ==========================
    ...
    CALL setKernelTime(544, kernelName//c_null_char, 0.0_8, 0.0_8, 0.0_4, 1)
    ...
    IF (&
         (xdim1 .NE. xdim1_maths_kernel_eqBN_h) .OR. &
...
!   ========================
    ...
    CALL setKernelTime(544, kernelName//c_null_char, t3__-t2__, t2__-t1__, transfer_total, 0)
...
SUBROUTINE maths_kernel_eqBN_host( userSubroutine, block, dim, range, &
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    15, 544, dim, 1, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(maths_kernel_eqBN_host_execute))

END SUBROUTINE
#endif

END MODULE MATHS_KERNEL_EQBN_MODULE
