! Auto-generated at 2026-04-28 18:44:33.084008 by ops-translator

MODULE SET_ZERO_KERNEL_MD5_MODULE
...
!$OMP DECLARE TARGET(xdim1_set_zero_kernel_MD5)
    INTEGER(KIND=4) :: xdim1_set_zero_kernel_MD5
    INTEGER(KIND=4) :: xdim1_set_zero_kernel_MD5_h = -1
!$OMP DECLARE TARGET(ydim1_set_zero_kernel_MD5)
    INTEGER(KIND=4) :: ydim1_set_zero_kernel_MD5
    INTEGER(KIND=4) :: ydim1_set_zero_kernel_MD5_h = -1
!$OMP DECLARE TARGET(zdim1_set_zero_kernel_MD5)
    INTEGER(KIND=4) :: zdim1_set_zero_kernel_MD5
    INTEGER(KIND=4) :: zdim1_set_zero_kernel_MD5_h = -1
#define OPS_ACC_MD1(d,x,y,z) ((x) + (xdim1_set_zero_kernel_MD5*(y)) + (xdim1_set_zero_kernel_MD5*ydim1_set_zero_kernel_MD5*(z)) + (xdim1_set_zero_kernel_MD5*ydim1_set_zero_kernel_MD5*zdim1_set_zero_kernel_MD5*(d-1)) + 1)
...
SUBROUTINE set_zero_kernel_MD5_wrap( &
    opsDat1Local, &
    dat1_base, &
    dat1_dim, &
    start_indx, &
    ...
    DO n_z = 1, end_indx_3-start_indx_3+1
        DO n_y = 1, end_indx_2-start_indx_2+1
            DO n_x = 1, end_indx_1-start_indx_1+1
                ...
                CALL set_zero_kernel_MD5( &
                opsDat1Local(dat1_base + ((n_x-1)*1) + ((n_y-1)*xdim1_set_zero_kernel_MD5*1) + ((n_z-1)*ydim1_set_zero_kernel_MD5*xdim1_set_zero_kernel_MD5*1)) &
               )
...
!   ===============
#ifndef OPS_LAZY
SUBROUTINE set_zero_kernel_MD5_host( userSubroutine, block, dim, range, &
    opsArg1 &
    )
...
#else
SUBROUTINE set_zero_kernel_MD5_host_execute( descPtr )
    ...
    kernelName = "set_zero_kernel_MD5"
...
!   ==========================
    ...
    CALL setKernelTime(559, kernelName//c_null_char, 0.0_8, 0.0_8, 0.0_4, 1)
...
!$OMP TARGET UPDATE TO(multi_d1)
    ...
    IF (&
         (xdim1 .NE. xdim1_set_zero_kernel_MD5_h) .OR. &
         (ydim1 .NE. ydim1_set_zero_kernel_MD5_h) .OR. &
         (zdim1 .NE. zdim1_set_zero_kernel_MD5_h) &
       ) THEN
            xdim1_set_zero_kernel_MD5 = xdim1
            xdim1_set_zero_kernel_MD5_h = xdim1
!$OMP TARGET UPDATE TO(xdim1_set_zero_kernel_MD5)
            ydim1_set_zero_kernel_MD5 = ydim1
            ydim1_set_zero_kernel_MD5_h = ydim1
            zdim1_set_zero_kernel_MD5 = zdim1
            zdim1_set_zero_kernel_MD5_h = zdim1
!$OMP TARGET UPDATE TO(ydim1_set_zero_kernel_MD5,zdim1_set_zero_kernel_MD5)
    END IF
...
!   ========================
    ...
    CALL setKernelTime(559, kernelName//c_null_char, t3__-t2__, t2__-t1__, transfer_total, 0)
...
SUBROUTINE set_zero_kernel_MD5_host( userSubroutine, block, dim, range, &
    ...
    CALL create_kerneldesc_and_enque(namelit//c_null_char, c_loc(opsArgArray), &
                                    1, 559, dim, 1, c_loc(range_tmp), &
                                    block%blockCptr, c_funloc(set_zero_kernel_MD5_host_execute))

END SUBROUTINE
#endif

END MODULE SET_ZERO_KERNEL_MD5_MODULE
