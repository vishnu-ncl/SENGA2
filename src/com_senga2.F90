...
!   GLOBAL GRID SIZE
    integer(4), parameter :: nxglbl=16, nyglbl=16, nzglbl=16
!   SET NGZMAX=MAX(NXGLBL,NYGLBL,NZGLBL)
    integer(4), parameter :: ngzmax=nxglbl

!   NUMBER OF PROCESSORS
    integer(4), parameter :: nxproc=1, nyproc=1, nzproc=1
!   SET NPRMAX=MAX(NXPROC,NYPROC,NZPROC)
    integer(4), parameter :: nprmax=nxproc

!   LOCAL GRID SIZE
    integer(4), parameter :: nxsize=16, nysize=16, nzsize=16
!   SET NSZMAX=MAX(NXSIZE,NYSIZE,NZSIZE)
    integer(4), parameter :: nszmax=nxsize

!   SIZE OF HALO
    integer(4), parameter :: nhalox=5,nhaloy=5,nhaloz=5
...
!   AND ALSO LARGE ENOUGH FOR PARALLEL TRANSFER OF INITIAL TURBULENCE DATA
    integer(4), parameter :: nparay=8000000

!   LOCAL BIG ARRAY SIZE
    integer(4), parameter :: nxbigl=1-nhalox, nxbigr=nxsize+nhalox,  &
...
!   NUMBER OF PENCILS
    integer(4), parameter :: npenmx=1

    real(8) :: fftrow(2*ngzmax,3,npenmx), ftpart(2*nszmax,3,npenmx),  &
               fftinx(2*ngzmax)
...
!   MAX NO OF THIRD BODIES
    integer(4), parameter :: nbdymx=10
!   MAX NO OF LINDEMANN STEPS
    integer(4), parameter :: nllmax=10

!   UNIVERSAL GAS CONSTANT
    real(8), parameter :: rguniv=8314.20_8

!   CHEMICAL DATA
!   =============
    real(8) :: amolcp(ncofmx,ntinmx,nspcmx)
    real(8) :: amolgb(ncofmx,ntinmx,nspcmx)
    real(8) :: amascp(ncofmx,ntinmx,nspcmx)
    real(8) :: amasct(ncofmx,ntinmx,nspcmx)
    real(8) :: amasch(ncofmx,ntinmx,nspcmx)
    real(8) :: amascs(ncofmx,ntinmx,nspcmx)
    real(8) :: tintlo(ntinmx,nspcmx),tinthi(ntinmx,nspcmx)
    real(8) :: diffmu(nssmax,nstpmx),diffmw(nssmax,nstpmx)
    real(8) :: crspec(nssmax,nstpmx),cpspec(nssmax,nstpmx)
    real(8) :: effy3b(nspcmx,nbdymx)
    real(8) :: rparam(3,nstpmx)
    real(8) :: rclind(4,nllmax)
    real(8) :: rctroe(12,nllmax)
    real(8) :: rcsrif(8,nllmax)
    real(8) :: wmolar(nspcmx),ovwmol(nspcmx),rgspec(nspcmx)
    real(8) :: clewis(nspcmx),olewis(nspcmx)
    real(8) :: prefgb

    integer(4) :: nsspec(nssmax,nstpmx)
    integer(4) :: nrspec(nssmax,nstpmx),npspec(nssmax,nstpmx)
    integer(4) :: nrcpec(nssmax,nstpmx),npcpec(nssmax,nstpmx)
    integer(4) :: ncofcp(ntinmx,nspcmx)
    integer(4) :: ncpoly(ntinmx,nspcmx),ncpom1(ntinmx,nspcmx)
    integer(4) :: ncenth(ntinmx,nspcmx),ncenpy(ntinmx,nspcmx)
    integer(4) :: nsslen(nstpmx)
    integer(4) :: nrslen(nstpmx),npslen(nstpmx)
    integer(4) :: nrclen(nstpmx),npclen(nstpmx)
    integer(4) :: mblist(nstpmx)
    integer(4) :: mglist(nstpmx)
    integer(4) :: mllist(nstpmx)
    integer(4) :: mtlist(nstpmx)
    integer(4) :: mslist(nstpmx)
    integer(4) :: ntint(nspcmx)
    integer(4) :: nspec,nspm1,nstep,nbody,ngibb,nlind,ntroe,nsrif

!   MAX LENGTH OF SPECIES SYMBOL STRINGS
    integer(4), parameter :: nspstr=10
...
!   MASS FRACTION TOLERANCE
    real(8), parameter :: dfctol = 0.0000000000010_8

!   MOLECULAR TRANSPORT DATA
    real(8) :: diffco(ndcfmx,nspcmx,nspcmx)
    real(8) :: tdrcco(ndcfmx,nspcmx,nspcmx)
    real(8) :: wilko1(nspcmx,nspcmx),wilko2(nspcmx,nspcmx)
    real(8) :: viscco(nvcfmx,nspcmx)
    real(8) :: condco(nccfmx,nspcmx)
    real(8) :: pdifgb,tdifgb
    real(8) :: wmltdr

    integer(4) :: ncodif,ncotdr,ncovis,ncocon
    integer(4) :: ncodm1,ncotm1,ncovm1,ncocm1
    integer(4) :: nfmavt,nfmixw,nfmixp,nfmsor,nfmduf
...
!   RADIATION TREATMENT
    ...
    real(8),parameter :: stefbo = 0.00000005670373210_8

    real(8) :: akprad(ncfrmx,nspcmx)
    real(8) :: trefrn

    integer(4) :: nsprid(nspcmx),nkprad(nspcmx),nkprm1(nspcmx)
    integer(4) :: nsprad,nfradn
...
!   MASS FRACTION TOLERANCE
    real(8), parameter :: ytoler = 1.0E-10

!   NUMBERS
    real(8), parameter :: zero=0.0_8, one=1.0_8, two=2.0_8, three=3.0_8,  &
...
!   PHYSICAL AND NUMERICAL DATA
    real(8) :: yrin(nspcmx)
    real(8) :: prin,trin,drin,urin,vrin,wrin,erin
    real(8) :: xgdlen,ygdlen,zgdlen
    real(8) :: etime,tstep,deltax,deltay,deltaz,pi,clnten
    integer(4) :: itime,ntime,ntime1,ntime2,nstpsw,nsaved, inturb,inflam,inseed,  &
...
!   DERIVATIVE BC STATUS
    integer(4), parameter :: nobc=0,nbound=1,nperi=2

!   COEFFICIENTS OF SPATIAL DIFFERENCING SCHEMES
    real(8) :: acoeff,bcoeff,ccoeff,dcoeff,ecoeff,  &
...
!   SPATIAL DERIVATIVE END CONDITIONS
    integer(4) :: nendxl,nendxr,nendyl,nendyr,nendzl,nendzr

!   DFDIFF-------------------------------------------------------------------
!   EMSTRT-------------------------------------------------------------------

!   DATA FOR MESH STRETCHING
    real(8) :: gcmreg(nszmax),gcmstr(nszmax),  &
...
!   RUNGE-KUTTA PARAMETERS
    real(8) :: rklhs(nrkmax),rkrhs(nrkmax)
    real(8) :: rkerr(nrkmax),rktim(nrkmax)
    real(8) :: ctmult,ctalph,ctbeta,ctgama
    real(8) :: errtol,errlow,trmin,trmax,tsmin,tsmax
    real(8) :: errold,errldr,btime
    real(8) :: erdnrm,erunrm,ervnrm,erwnrm,erenrm
    real(8) :: erynrm(nspcmx)
    integer(4) :: irkstp,nrkstp,nrksm1
    integer(4) :: inderr
    logical :: fupelc
...
!   RUNGE-KUTTA SUBSTEP ERROR NORMS
    real(8) :: erdrhs(nrkmax),erurhs(nrkmax),ervrhs(nrkmax),  &
               erwrhs(nrkmax),ererhs(nrkmax)
    real(8) :: eryrhs(nspcmx,nrkmax)
...
!   WORKSPACE (PARALLEL TRANSFER ARRAY)
    real(8) :: parray(nparay)

!   WORKSP-------------------------------------------------------------------
!   NSBCCL-------------------------------------------------------------------

!   BC TYPE PARAMETERS
    integer(4), parameter :: nsnull=0, nsperi=1, nsbci1=11,nsbci2=12,nsbci3=13,nsbci4=14,  &
                             nsbco1=21,nsbco2=22,nsbco3=23,nsbco4=24,  &
                             nsbcw1=31,nsbcw2=32,nsbcw3=33,nsbcw4=34

!   GLOBAL BC TYPE VARIABLES
    integer(4) :: ngbcxl,ngbcxr,ngbcyl,ngbcyr,ngbczl,ngbczr

!   LOCAL BC TYPE VARIABLES
    integer(4) :: nsbcxl,nsbcxr,nsbcyl,nsbcyr,nsbczl,nsbczr
...
!   BC DATA
    real(8) :: rxlprm(nbcprr),rxrprm(nbcprr),  &
               rylprm(nbcprr),ryrprm(nbcprr), rzlprm(nbcprr),rzrprm(nbcprr)
    real(8) :: cobcxl,cobcxr,cobcyl,cobcyr,cobczl,cobczr,  &
               pinfxl,pinfxr,pinfyl,pinfyr,pinfzl,pinfzr
    real(8) :: slocxl,slocxr,slocyl,slocyr,sloczl,sloczr,  &
    ...
    integer(4) :: nxlprm(nbcpri),nxrprm(nbcpri), nylprm(nbcpri),nyrprm(nbcpri),  &
                  nzlprm(nbcpri),nzrprm(nbcpri)
    integer(4) :: istaxl,istoxl,istayl,istoyl,istazl,istozl,  &
...
!   WALL BC DIFFERENCING DATA
    real(8) :: acbcxl(ncbcsz),acbcxr(ncbcsz),  &
...
!   PARALLEL DOMAIN DECOMPOSITION DATA
    integer(4) :: npmapx(0:nxproc),npmapy(0:nyproc),npmapz(0:nzproc),  &
...
!   DATA FOR ON-LINE STATISTICS
    ...
    integer(4), parameter :: nstore=32

    real(8) :: umax(nstore),umin(nstore),vmax(nstore),vmin(nstore),  &
