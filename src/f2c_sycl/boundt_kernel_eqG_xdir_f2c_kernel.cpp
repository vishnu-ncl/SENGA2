// Auto-generated at 2026-04-28 18:44:57.464596 by ops-translator
...
#ifdef OPS_DEBUG
    ops_register_args(block->instance, args, "boundt_kernel_eqG_xdir");
#endif
...
//  ======================================================
    int xdim0_boundt_kernel_eqG_xdir = args[0].dat->size[0];
    int ydim0_boundt_kernel_eqG_xdir = args[0].dat->size[1];
    int xdim1_boundt_kernel_eqG_xdir = args[1].dat->size[0];
    int ydim1_boundt_kernel_eqG_xdir = args[1].dat->size[1];
    int xdim2_boundt_kernel_eqG_xdir = args[2].dat->size[0];
    int ydim2_boundt_kernel_eqG_xdir = args[2].dat->size[1];
    int xdim3_boundt_kernel_eqG_xdir = args[3].dat->size[0];
    int ydim3_boundt_kernel_eqG_xdir = args[3].dat->size[1];
...
//  =============
    ...
    if ((end_0-start_0)>0 && (end_1-start_1)>0 && (end_2-start_2)>0) {
        block->instance->sycl_instance->queue->submit([&](cl::sycl::handler &cgh) {
            ...
            cgh.parallel_for<class boundt_kernel_eqG_xdir_sycl>(
                cl::sycl::nd_range<3>(
                    cl::sycl::range<3>(
            ...
            , [=](cl::sycl::nd_item<3> item
                ...
                int n_x = item.get_global_id()[2];
                 ...
                 ACC<double> erhs(xdim0_boundt_kernel_eqG_xdir, ydim0_boundt_kernel_eqG_xdir, erhs_p + (n_x * 1) + (n_y * xdim0_boundt_kernel_eqG_xdir * 1) + (n_z * xdim0_boundt_kernel_eqG_xdir * ydim0_boundt_kernel_eqG_xdir * 1));
                const  ACC<double> yrhs(xdim1_boundt_kernel_eqG_xdir, ydim1_boundt_kernel_eqG_xdir, yrhs_p + (n_x * 1) + (n_y * xdim1_boundt_kernel_eqG_xdir * 1) + (n_z * xdim1_boundt_kernel_eqG_xdir * ydim1_boundt_kernel_eqG_xdir * 1));
                const  ACC<int> itndex(xdim2_boundt_kernel_eqG_xdir, ydim2_boundt_kernel_eqG_xdir, itndex_p + (n_x * 1) + (n_y * xdim2_boundt_kernel_eqG_xdir * 1) + (n_z * xdim2_boundt_kernel_eqG_xdir * ydim2_boundt_kernel_eqG_xdir * 1));
                const  ACC<double> strtx(xdim3_boundt_kernel_eqG_xdir, ydim3_boundt_kernel_eqG_xdir, strtx_p + (n_x * 0) + (n_y * xdim3_boundt_kernel_eqG_xdir * 1) + (n_z * xdim3_boundt_kernel_eqG_xdir * ydim3_boundt_kernel_eqG_xdir * 1));
