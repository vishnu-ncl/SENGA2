// Auto-generated at 2026-04-28 18:44:49.018109 by ops-translator
...
//  Subtracting 1 here as start_indx and end_indx is in Fortran style - converting it to c-style range
    ...
    double *arg4_data_d = (double*)(block->instance->OPS_reduct_d + reduct_bytes);
...
//  =============
    ...
    if ((end_0-start_0)>0 && (end_1-start_1)>0 && (end_2-start_2)>0) {
        block->instance->sycl_instance->queue->submit([&](cl::sycl::handler &cgh) {
            ...
            cgh.parallel_for<class adaptt_kernel_err_eval_MD_sycl>(
                cl::sycl::nd_range<3>(
                    cl::sycl::range<3>(
            ...
            , [=](cl::sycl::nd_item<3> item
                ...
                const double *ernrm = arg2_data_d;
...
// =========
    ...
    *ertot = f2c::max(*ertot, fornow);
                ...
                for (int d = 0; d < 1; d++) {
                    ops_reduction_sycl<OPS_MAX>(arg4_data_d + d + item.get_group_linear_id()*1, ertot[d], (double*)&local_mem[0], item, group_size);
                }
            });
        });
    }
...
//  ==============================
    ...
    for (int b = 0; b < maxblocks; b++)
        for (int d = 0; d < 1; d++)
            p_a4[d] = MAX(p_a4[d], ((double *)arg4.data)[d+b*1]);
