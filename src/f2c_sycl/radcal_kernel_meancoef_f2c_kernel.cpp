// Auto-generated at 2026-04-28 18:44:58.564591 by ops-translator
...
//  =============
    ...
    if ((end_0-start_0)>0 && (end_1-start_1)>0 && (end_2-start_2)>0) {
        block->instance->sycl_instance->queue->submit([&](cl::sycl::handler &cgh) {
            ...
            auto ncfrmx_sycl = (*ncfrmx_p).template get_access<cl::sycl::access::mode::read>(cgh);

            cgh.parallel_for<class radcal_kernel_meancoef_sycl>(
                cl::sycl::nd_range<3>(
                    cl::sycl::range<3>(
            ...
            , [=](cl::sycl::nd_item<3> item
                ...
                const double *akprad = arg2_data_d;
                const int *nkprad = arg3_data_d;
                const int *nkprm1 = arg4_data_d;
...
// =========
    ...
    fornow = trun(0, 0, 0);
    plspec = akprad[(nkprad[jspec[0]-1]-1)+(jspec[0]-1)*ncfrmx_sycl[0]];
    for (icp = nkprm1[jspec[0]-1]; icp >= 1; icp -= 1) {
        plspec = plspec * fornow + akprad[(icp-1)+(jspec[0]-1)*ncfrmx_sycl[0]];
    }
    store2(0, 0, 0) = plspec;
