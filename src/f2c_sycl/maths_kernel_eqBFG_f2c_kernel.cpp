// Auto-generated at 2026-04-28 18:44:46.451020 by ops-translator
...
//  =======================================================
    ...
    double pdifgb_val = *(double *)args[9].data;

    double dfctol_val = *(double *)args[10].data;

    int ncodif_val = *(int *)args[11].data;

    int ncodm1_val = *(int *)args[12].data;
...
//  =============
    ...
    if ((end_0-start_0)>0 && (end_1-start_1)>0 && (end_2-start_2)>0) {
        block->instance->sycl_instance->queue->submit([&](cl::sycl::handler &cgh) {
            ...
            cgh.parallel_for<class maths_kernel_eqBFG_sycl>(
                cl::sycl::nd_range<3>(
                    cl::sycl::range<3>(
            ...
            , [=](cl::sycl::nd_item<3> item
                ...
                const double *diffco = arg7_data_d;
                const double *ovwmol = arg8_data_d;
                const double *pdifgb = &pdifgb_val;
                const double *dfctol = &dfctol_val;
                const int *ncodif = &ncodif_val;
                const int *ncodm1 = &ncodm1_val;
...
// =========
    ...
    for (jspec = 1; jspec <= nspcmx_sycl[0]; ++jspec) {
        fornow = diffco[(ncodif[0]-1)+(jspec-1)*ndcfmx_sycl[0]+(ispec[0]-1)*ndcfmx_sycl[0]*nspcmx_sycl[0]];
        for (icp = ncodm1[0]; icp >= 1; icp -= 1) {
            fornow = fornow * transp(0, 0, 0) + diffco[(icp-1)+(jspec-1)*ndcfmx_sycl[0]+(ispec[0]-1)*ndcfmx_sycl[0]*nspcmx_sycl[0]];
        }
        ctrans[jspec-1] = cl::sycl::exp(fornow) * pdifgb[0] / prun(0, 0, 0);
    }
    combo1 = 0.0;
    combo2 = 0.0;
    for (jspec = 1; jspec <= nspcmx_sycl[0]; ++jspec) {
