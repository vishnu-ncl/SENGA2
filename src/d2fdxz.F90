...
!           EXPLICIT 4TH,4TH,4TH,6TH,8TH COMPATIBLE ORDER BOUNDARY TREATMENT
            rangexyz = (/1,1,1,nyglbl,6,nzglbl-5/)
            ...
            rangexyz = (/2,2,1,nyglbl,6,nzglbl-5/)
            ...
            rangexyz = (/3,3,1,nyglbl,6,nzglbl-5/)
            ...
            rangexyz = (/4,4,1,nyglbl,6,nzglbl-5/)
            ...
            rangexyz = (/5,5,1,nyglbl,6,nzglbl-5/)
...
!           EXPLICIT 4TH,4TH,4TH,6TH,8TH COMPATIBLE ORDER BOUNDARY TREATMENT
            ...
            rangexyz = (/nxglbl,nxglbl,1,nyglbl,6,nzglbl-5/)
...
!           EXPLICIT 4TH,4TH,4TH,6TH,8TH COMPATIBLE ORDER BOUNDARY TREATMENT
            rangexyz = (/6,nxglbl-5,1,nyglbl,1,1/)
            ...
            rangexyz = (/6,nxglbl-5,1,nyglbl,2,2/)
            ...
            call ops_par_loop(d2fdxz_kernel_lh_zdir_corner_eqI, "d2fdxz_lh_zdir_corner_eqI", senga_grid, 3, rangexyz, &
                            ops_arg_dat(functn, 1, s3d_p202_m202_mixed_xz, "real(8)", OPS_READ), &
                            ops_arg_dat(fderiv, 1, s3d_000, "real(8)", OPS_WRITE), &
                            ops_arg_dat(d_fstoreaxz, 9, s3d_000_strid3d_y, "real(8)", OPS_WRITE), &
                            ops_arg_dat(d_fstorebxz, 9, s3d_000_strid3d_y, "real(8)", OPS_WRITE), &
                            ops_arg_idx())

            rangexyz = (/4,5,1,nyglbl,4,5/)
            call ops_par_loop(d2fdxz_kernel_lh_zdir_corner_eqJ, "d2fdxz_lh_zdir_corner_eqJ", senga_grid, 3, rangexzz, &
                            ops_arg_dat(functn, 1, s3d_p303_m303_mixed_small_xz, "real(8)", OPS_READ), &
                            ops_arg_dat(fderiv, 1, s3d_000, "real(8)", OPS_WRITE), &
                            ops_arg_dat(d_fstoreaxz, 9, s3d_000_strid3d_y, "real(8)", OPS_READ), &
                            ops_arg_dat(d_fstorebxz, 9, s3d_000_strid3d_y, "real(8)", OPS_READ), &
                            ops_arg_dat(d_fstorecxz, 4, s3d_000_strid3d_y, "real(8)", OPS_WRITE), &
                            ops_arg_idx())

            rangexyz = (/5,5,1,nyglbl,5,5/)
            call ops_par_loop(d2fdxz_kernel_lh_zdir_corner_eqK, "d2fdxz_lh_zdir_corner_eqK", senga_grid, 3, rangexzz, &
                            ops_arg_dat(functn, 1, s3d_p404_m404_mixed_small_xz, "real(8)", OPS_READ), &
                            ops_arg_dat(fderiv, 1, s3d_000, "real(8)", OPS_WRITE), &
                            ops_arg_dat(d_fstoreaxz, 9, s3d_000_strid3d_y, "real(8)", OPS_READ), &
                            ops_arg_dat(d_fstorebxz, 9, s3d_000_strid3d_y, "real(8)", OPS_READ), &
                            ops_arg_dat(d_fstorecxz, 4, s3d_000_strid3d_y, "real(8)", OPS_READ), &
                            ops_arg_idx())
...
!           ======================
            ...
            call ops_par_loop(d2fdxz_kernel_lh_zdir_corner_eqT, "d2fdxz_lh_zdir_corner_eqT", senga_grid, 3, rangexzz, &
                            ops_arg_dat(functn, 1, s3d_p202_m202_mixed_xz, "real(8)", OPS_READ), &
                            ops_arg_dat(fderiv, 1, s3d_000, "real(8)", OPS_WRITE), &
                            ops_arg_dat(d_fstoreaxz, 9, s3d_000_strid3d_y, "real(8)", OPS_WRITE), &
                            ops_arg_dat(d_fstorebxz, 9, s3d_000_strid3d_y, "real(8)", OPS_WRITE), &
            ...
            rangexyz = (/nxglbl-4,nxglbl-3,1,nyglbl,4,5/)
            call ops_par_loop(d2fdxz_kernel_lh_zdir_corner_eqU, "d2fdxz_lh_zdir_corner_eqU", senga_grid, 3, rangexzz, &
                            ops_arg_dat(functn, 1, s3d_p303_m303_mixed_small_xz, "real(8)", OPS_READ), &
                            ops_arg_dat(fderiv, 1, s3d_000, "real(8)", OPS_WRITE), &
                            ops_arg_dat(d_fstoreaxz, 9, s3d_000_strid3d_y, "real(8)", OPS_READ), &
                            ops_arg_dat(d_fstorebxz, 9, s3d_000_strid3d_y, "real(8)", OPS_READ), &
            ...
            rangexyz = (/nxglbl-4,nxglbl-4,1,nyglbl,5,5/)
            call ops_par_loop(d2fdxz_kernel_lh_zdir_corner_eqV, "d2fdxz_lh_zdir_corner_eqV", senga_grid, 3, rangexzz, &
                            ops_arg_dat(functn, 1, s3d_p404_m404_mixed_small_xz, "real(8)", OPS_READ), &
                            ops_arg_dat(fderiv, 1, s3d_000, "real(8)", OPS_WRITE), &
                            ops_arg_dat(d_fstoreaxz, 9, s3d_000_strid3d_y, "real(8)", OPS_READ), &
                            ops_arg_dat(d_fstorebxz, 9, s3d_000_strid3d_y, "real(8)", OPS_READ), &
...
!           EXPLICIT 2ND,2ND,4TH,6TH,8TH COMPATIBLE ORDER BOUNDARY TREATMENT
            ...
            rangexyz = (/6,nxglbl-5,1,nyglbl,nzglbl,nzglbl/)
...
!           ======================
            ...
            call ops_par_loop(d2fdxz_kernel_rh_zdir_corner_eqI, "d2fdxz_rh_zdir_corner_eqI", senga_grid, 3, rangexzz, &
                            ops_arg_dat(functn, 1, s3d_p202_m202_mixed_xz, "real(8)", OPS_READ), &
                            ops_arg_dat(fderiv, 1, s3d_000, "real(8)", OPS_WRITE), &
                            ops_arg_dat(d_fstoreaxz, 9, s3d_000_strid3d_y, "real(8)", OPS_WRITE), &
                            ops_arg_dat(d_fstorebxz, 9, s3d_000_strid3d_y, "real(8)", OPS_WRITE), &
            ...
            rangexyz = (/4,5,1,nyglbl,nzglbl-4,nzglbl-3/)
            call ops_par_loop(d2fdxz_kernel_rh_zdir_corner_eqJ, "d2fdxz_rh_zdir_corner_eqJ", senga_grid, 3, rangexzz, &
                            ops_arg_dat(functn, 1, s3d_p303_m303_mixed_small_xz, "real(8)", OPS_READ), &
                            ops_arg_dat(fderiv, 1, s3d_000, "real(8)", OPS_WRITE), &
                            ops_arg_dat(d_fstoreaxz, 9, s3d_000_strid3d_y, "real(8)", OPS_READ), &
                            ops_arg_dat(d_fstorebxz, 9, s3d_000_strid3d_y, "real(8)", OPS_READ), &
            ...
            rangexyz = (/5,5,1,nyglbl,nzglbl-4,nzglbl-4/)
            call ops_par_loop(d2fdxz_kernel_rh_zdir_corner_eqK, "d2fdxz_rh_zdir_corner_eqK", senga_grid, 3, rangexzz, &
                            ops_arg_dat(functn, 1, s3d_p404_m404_mixed_small_xz, "real(8)", OPS_READ), &
                            ops_arg_dat(fderiv, 1, s3d_000, "real(8)", OPS_WRITE), &
                            ops_arg_dat(d_fstoreaxz, 9, s3d_000_strid3d_y, "real(8)", OPS_READ), &
                            ops_arg_dat(d_fstorebxz, 9, s3d_000_strid3d_y, "real(8)", OPS_READ), &
...
!           ======================
            ...
            call ops_par_loop(d2fdxz_kernel_rh_zdir_corner_eqT, "d2fdxz_rh_zdir_corner_eqT", senga_grid, 3, rangexzz, &
                            ops_arg_dat(functn, 1, s3d_p202_m202_mixed_xz, "real(8)", OPS_READ), &
                            ops_arg_dat(fderiv, 1, s3d_000, "real(8)", OPS_WRITE), &
                            ops_arg_dat(d_fstoreaxz, 9, s3d_000_strid3d_y, "real(8)", OPS_WRITE), &
                            ops_arg_dat(d_fstorebxz, 9, s3d_000_strid3d_y, "real(8)", OPS_WRITE), &
            ...
            rangexyz = (/nxglbl-4,nxglbl-3,1,nyglbl,nzglbl-4,nzglbl-3/)
            call ops_par_loop(d2fdxz_kernel_rh_zdir_corner_eqU, "d2fdxz_rh_zdir_corner_eqU", senga_grid, 3, rangexzz, &
                            ops_arg_dat(functn, 1, s3d_p303_m303_mixed_small_xz, "real(8)", OPS_READ), &
                            ops_arg_dat(fderiv, 1, s3d_000, "real(8)", OPS_WRITE), &
                            ops_arg_dat(d_fstoreaxz, 9, s3d_000_strid3d_y, "real(8)", OPS_READ), &
                            ops_arg_dat(d_fstorebxz, 9, s3d_000_strid3d_y, "real(8)", OPS_READ), &
            ...
            rangexyz = (/nxglbl-4,nxglbl-4,1,nyglbl,nzglbl-4,nzglbl-4/)
            call ops_par_loop(d2fdxz_kernel_rh_zdir_corner_eqV, "d2fdxz_rh_zdir_corner_eqV", senga_grid, 3, rangexzz, &
                            ops_arg_dat(functn, 1, s3d_p303_m303_mixed_small_xz, "real(8)", OPS_READ), &
                            ops_arg_dat(fderiv, 1, s3d_000, "real(8)", OPS_WRITE), &
                            ops_arg_dat(d_fstoreaxz, 9, s3d_000_strid3d_y, "real(8)", OPS_READ), &
                            ops_arg_dat(d_fstorebxz, 9, s3d_000_strid3d_y, "real(8)", OPS_READ), &
