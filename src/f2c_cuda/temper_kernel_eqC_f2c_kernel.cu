// Auto-generated at 2026-04-28 18:44:05.249792 by ops-translator

__constant__ int dims_temper_kernel_eqC[4][3];
static int dims_temper_kernel_eqC_h[4][3] = {{0}};

//  =============
//  User function
//  =============
__device__ void temper_kernel_eqC_gpu(ACC<double> &store7, const ACC<double> &yrhs, const double *rgspec, const int *ispec) {
...
//  ============================
__global__ void ops_temper_kernel_eqC(double* __restrict arg0, int xstride_0, int ystride_0, int zstride_0, 
...
int size0, int size1, int size2) {
    ...
    arg0 += idx_x * xstride_0 + idx_y * ystride_0 * dims_temper_kernel_eqC[0][0] + idx_z * zstride_0 * dims_temper_kernel_eqC[0][0] * dims_temper_kernel_eqC[0][1];
    arg1 += idx_x * xstride_1 + idx_y * ystride_1 * dims_temper_kernel_eqC[1][0] + idx_z * zstride_1 * dims_temper_kernel_eqC[1][0] * dims_temper_kernel_eqC[1][1];

    if(idx_x < size0 && idx_y < size1 && idx_z < size2) {

        ACC<double> argp0(dims_temper_kernel_eqC[0][0], dims_temper_kernel_eqC[0][1], arg0);
        const ACC<double> argp1(dims_temper_kernel_eqC[1][0], dims_temper_kernel_eqC[1][1], arg1);

        temper_kernel_eqC_gpu(argp0, argp1, arg2, &arg3);
...
//  ======================================================
    ...
    if (xdim0 != dims_temper_kernel_eqC_h[0][0] || ydim0 != dims_temper_kernel_eqC_h[0][1] || zdim0 != dims_temper_kernel_eqC_h[0][2] || xdim1 != dims_temper_kernel_eqC_h[1][0] || ydim1 != dims_temper_kernel_eqC_h[1][1] || zdim1 != dims_temper_kernel_eqC_h[1][2]) {
        dims_temper_kernel_eqC_h[0][0] = xdim0;
        dims_temper_kernel_eqC_h[0][1] = ydim0;
        dims_temper_kernel_eqC_h[0][2] = zdim0;
        dims_temper_kernel_eqC_h[1][0] = xdim1;
        dims_temper_kernel_eqC_h[1][1] = ydim1;
        dims_temper_kernel_eqC_h[1][2] = zdim1;

        cutilSafeCall(block->instance->ostream(), cudaMemcpyToSymbol( dims_temper_kernel_eqC, dims_temper_kernel_eqC_h, sizeof(dims_temper_kernel_eqC)));
    }
...
//  call kernel wrapper function, passing in pointers to data
    if (x_size > 0 && y_size > 0 && z_size > 0) {
        ...
        ops_temper_kernel_eqC<<<grid, tblock >>> (
