// Auto-generated at 2026-04-28 18:43:46.932598 by ops-translator

__constant__ int dims_d2fdxy_kernel_eqAV[2][3];
static int dims_d2fdxy_kernel_eqAV_h[2][3] = {{0}};

//  =============
//  User function
//  =============
__device__ void d2fdxy_kernel_eqAV_gpu(const ACC<double> &functn, ACC<double> &fderiv) {
...
//  ============================
__global__ void ops_d2fdxy_kernel_eqAV(double* __restrict arg0, int xstride_0, int ystride_0, int zstride_0, 
...
int size0, int size1, int size2) {
    ...
    arg0 += idx_x * xstride_0 + idx_y * ystride_0 * dims_d2fdxy_kernel_eqAV[0][0] + idx_z * zstride_0 * dims_d2fdxy_kernel_eqAV[0][0] * dims_d2fdxy_kernel_eqAV[0][1];
    arg1 += idx_x * xstride_1 + idx_y * ystride_1 * dims_d2fdxy_kernel_eqAV[1][0] + idx_z * zstride_1 * dims_d2fdxy_kernel_eqAV[1][0] * dims_d2fdxy_kernel_eqAV[1][1];

    if(idx_x < size0 && idx_y < size1 && idx_z < size2) {

        const ACC<double> argp0(dims_d2fdxy_kernel_eqAV[0][0], dims_d2fdxy_kernel_eqAV[0][1], arg0);
        ACC<double> argp1(dims_d2fdxy_kernel_eqAV[1][0], dims_d2fdxy_kernel_eqAV[1][1], arg1);

        d2fdxy_kernel_eqAV_gpu(argp0, argp1);
...
//  ======================================================
    ...
    if (xdim0 != dims_d2fdxy_kernel_eqAV_h[0][0] || ydim0 != dims_d2fdxy_kernel_eqAV_h[0][1] || zdim0 != dims_d2fdxy_kernel_eqAV_h[0][2] || xdim1 != dims_d2fdxy_kernel_eqAV_h[1][0] || ydim1 != dims_d2fdxy_kernel_eqAV_h[1][1] || zdim1 != dims_d2fdxy_kernel_eqAV_h[1][2]) {
        dims_d2fdxy_kernel_eqAV_h[0][0] = xdim0;
        dims_d2fdxy_kernel_eqAV_h[0][1] = ydim0;
        dims_d2fdxy_kernel_eqAV_h[0][2] = zdim0;
        dims_d2fdxy_kernel_eqAV_h[1][0] = xdim1;
        dims_d2fdxy_kernel_eqAV_h[1][1] = ydim1;
        dims_d2fdxy_kernel_eqAV_h[1][2] = zdim1;

        cutilSafeCall(block->instance->ostream(), cudaMemcpyToSymbol( dims_d2fdxy_kernel_eqAV, dims_d2fdxy_kernel_eqAV_h, sizeof(dims_d2fdxy_kernel_eqAV)));
    }
...
//  call kernel wrapper function, passing in pointers to data
    if (x_size > 0 && y_size > 0 && z_size > 0) {
        ...
        ops_d2fdxy_kernel_eqAV<<<grid, tblock >>> (
    ...
    )
{
    ...
    create_kerneldesc_and_enque("d2fdxy_kernel_eqAV", args, 2, 65, dim, 1, range, block, d2fdxy_kernel_eqAV_execute);
}
#endif
