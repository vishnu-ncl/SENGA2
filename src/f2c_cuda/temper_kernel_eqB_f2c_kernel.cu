// Auto-generated at 2026-04-28 18:44:05.231664 by ops-translator

__constant__ int dims_temper_kernel_eqB[12][3];
static int dims_temper_kernel_eqB_h[12][3] = {{0}};

//  =============
//  User function
//  =============
__device__ void temper_kernel_eqB_gpu(ACC<double> &tcoeff, ACC<double> &tderiv, const ACC<int> &itndex, const ACC<double> &yrhs, const double *amascp, const double *amasct, const int *ncpoly, const int *ncenth, const int *ipower, const int *icoef1, const int *icoef2, const int *ispec) {

    int itint;
    int icp;

    itint = 1 + f2c::mod(itndex(0, 0, 0), icoef1[0]) / icoef2[0];
    tcoeff(1-1, 0, 0, 0) = tcoeff(1-1, 0, 0, 0) + yrhs(0, 0, 0) * amascp[(ncenth[(itint-1)+(ispec[0]-1)*ntinmx]-1)+(itint-1)*ncofmx+(ispec[0]-1)*ncofmx*ntinmx];
    tcoeff(2-1, 0, 0, 0) = tcoeff(2-1, 0, 0, 0) + yrhs(0, 0, 0) * amasct[(1-1)+(itint-1)*ncofmx+(ispec[0]-1)*ncofmx*ntinmx];
    tderiv(1-1, 0, 0, 0) = tcoeff(2-1, 0, 0, 0);
    for (icp = 2; icp <= ncpoly[(itint-1)+(ispec[0]-1)*ntinmx]; ++icp) {
        tcoeff(icp + 1-1, 0, 0, 0) = tcoeff(icp + 1-1, 0, 0, 0) + yrhs(0, 0, 0) * amasct[(icp-1)+(itint-1)*ncofmx+(ispec[0]-1)*ncofmx*ntinmx];
        tderiv(icp-1, 0, 0, 0) = tderiv(icp-1, 0, 0, 0) + yrhs(0, 0, 0) * amascp[(icp-1)+(itint-1)*ncofmx+(ispec[0]-1)*ncofmx*ntinmx];
    }
...
//  ============================
__global__ void ops_temper_kernel_eqB(double* __restrict arg0, int xstride_0, int ystride_0, int zstride_0, 
...
int size0, int size1, int size2) {
    ...
    arg0 += idx_x * xstride_0 + idx_y * ystride_0 * dims_temper_kernel_eqB[0][0] + idx_z * zstride_0 * dims_temper_kernel_eqB[0][0] * dims_temper_kernel_eqB[0][1];
    arg1 += idx_x * xstride_1 + idx_y * ystride_1 * dims_temper_kernel_eqB[1][0] + idx_z * zstride_1 * dims_temper_kernel_eqB[1][0] * dims_temper_kernel_eqB[1][1];
    arg2 += idx_x * xstride_2 + idx_y * ystride_2 * dims_temper_kernel_eqB[2][0] + idx_z * zstride_2 * dims_temper_kernel_eqB[2][0] * dims_temper_kernel_eqB[2][1];
    arg3 += idx_x * xstride_3 + idx_y * ystride_3 * dims_temper_kernel_eqB[3][0] + idx_z * zstride_3 * dims_temper_kernel_eqB[3][0] * dims_temper_kernel_eqB[3][1];

    if(idx_x < size0 && idx_y < size1 && idx_z < size2) {

        ACC<double> argp0(6, dims_temper_kernel_eqB[0][0], dims_temper_kernel_eqB[0][1], dims_temper_kernel_eqB[0][2], arg0);
        ACC<double> argp1(5, dims_temper_kernel_eqB[1][0], dims_temper_kernel_eqB[1][1], dims_temper_kernel_eqB[1][2], arg1);
        const ACC<int> argp2(dims_temper_kernel_eqB[2][0], dims_temper_kernel_eqB[2][1], arg2);
        const ACC<double> argp3(dims_temper_kernel_eqB[3][0], dims_temper_kernel_eqB[3][1], arg3);

        temper_kernel_eqB_gpu(argp0, argp1, argp2, argp3, arg4, arg5, arg6, arg7, &arg8, &arg9, &arg10, &arg11);
...
//  ======================================================
    ...
    if (xdim0 != dims_temper_kernel_eqB_h[0][0] || ydim0 != dims_temper_kernel_eqB_h[0][1] || zdim0 != dims_temper_kernel_eqB_h[0][2] || xdim1 != dims_temper_kernel_eqB_h[1][0] || ydim1 != dims_temper_kernel_eqB_h[1][1] || zdim1 != dims_temper_kernel_eqB_h[1][2] || xdim2 != dims_temper_kernel_eqB_h[2][0] || ydim2 != dims_temper_kernel_eqB_h[2][1] || zdim2 != dims_temper_kernel_eqB_h[2][2] || xdim3 != dims_temper_kernel_eqB_h[3][0] || ydim3 != dims_temper_kernel_eqB_h[3][1] || zdim3 != dims_temper_kernel_eqB_h[3][2]) {
        dims_temper_kernel_eqB_h[0][0] = xdim0;
        dims_temper_kernel_eqB_h[0][1] = ydim0;
        dims_temper_kernel_eqB_h[0][2] = zdim0;
        dims_temper_kernel_eqB_h[1][0] = xdim1;
        dims_temper_kernel_eqB_h[1][1] = ydim1;
        dims_temper_kernel_eqB_h[1][2] = zdim1;
        dims_temper_kernel_eqB_h[2][0] = xdim2;
        dims_temper_kernel_eqB_h[2][1] = ydim2;
        dims_temper_kernel_eqB_h[2][2] = zdim2;
        dims_temper_kernel_eqB_h[3][0] = xdim3;
        dims_temper_kernel_eqB_h[3][1] = ydim3;
        dims_temper_kernel_eqB_h[3][2] = zdim3;

        cutilSafeCall(block->instance->ostream(), cudaMemcpyToSymbol( dims_temper_kernel_eqB, dims_temper_kernel_eqB_h, sizeof(dims_temper_kernel_eqB)));
    }
...
//  call kernel wrapper function, passing in pointers to data
    if (x_size > 0 && y_size > 0 && z_size > 0) {
        ...
        ops_temper_kernel_eqB<<<grid, tblock >>> (
