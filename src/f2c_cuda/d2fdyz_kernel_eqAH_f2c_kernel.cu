// Auto-generated at 2026-04-28 18:43:51.928488 by ops-translator

__constant__ int dims_d2fdyz_kernel_eqAH[2][3];
static int dims_d2fdyz_kernel_eqAH_h[2][3] = {{0}};

//  =============
//  User function
//  =============
__device__ void d2fdyz_kernel_eqAH_gpu(const ACC<double> &functn, ACC<double> &fderiv) {
...
//  ============================
__global__ void ops_d2fdyz_kernel_eqAH(double* __restrict arg0, int xstride_0, int ystride_0, int zstride_0, 
...
int size0, int size1, int size2) {
    ...
    arg0 += idx_x * xstride_0 + idx_y * ystride_0 * dims_d2fdyz_kernel_eqAH[0][0] + idx_z * zstride_0 * dims_d2fdyz_kernel_eqAH[0][0] * dims_d2fdyz_kernel_eqAH[0][1];
    arg1 += idx_x * xstride_1 + idx_y * ystride_1 * dims_d2fdyz_kernel_eqAH[1][0] + idx_z * zstride_1 * dims_d2fdyz_kernel_eqAH[1][0] * dims_d2fdyz_kernel_eqAH[1][1];

    if(idx_x < size0 && idx_y < size1 && idx_z < size2) {

        const ACC<double> argp0(dims_d2fdyz_kernel_eqAH[0][0], dims_d2fdyz_kernel_eqAH[0][1], arg0);
        ACC<double> argp1(dims_d2fdyz_kernel_eqAH[1][0], dims_d2fdyz_kernel_eqAH[1][1], arg1);

        d2fdyz_kernel_eqAH_gpu(argp0, argp1);
...
//  ======================================================
    ...
    if (xdim0 != dims_d2fdyz_kernel_eqAH_h[0][0] || ydim0 != dims_d2fdyz_kernel_eqAH_h[0][1] || zdim0 != dims_d2fdyz_kernel_eqAH_h[0][2] || xdim1 != dims_d2fdyz_kernel_eqAH_h[1][0] || ydim1 != dims_d2fdyz_kernel_eqAH_h[1][1] || zdim1 != dims_d2fdyz_kernel_eqAH_h[1][2]) {
        dims_d2fdyz_kernel_eqAH_h[0][0] = xdim0;
        dims_d2fdyz_kernel_eqAH_h[0][1] = ydim0;
        dims_d2fdyz_kernel_eqAH_h[0][2] = zdim0;
        dims_d2fdyz_kernel_eqAH_h[1][0] = xdim1;
        dims_d2fdyz_kernel_eqAH_h[1][1] = ydim1;
        dims_d2fdyz_kernel_eqAH_h[1][2] = zdim1;

        cutilSafeCall(block->instance->ostream(), cudaMemcpyToSymbol( dims_d2fdyz_kernel_eqAH, dims_d2fdyz_kernel_eqAH_h, sizeof(dims_d2fdyz_kernel_eqAH)));
    }
...
//  call kernel wrapper function, passing in pointers to data
    if (x_size > 0 && y_size > 0 && z_size > 0) {
        ...
        ops_d2fdyz_kernel_eqAH<<<grid, tblock >>> (
    ...
    )
{
    ...
    create_kerneldesc_and_enque("d2fdyz_kernel_eqAH", args, 2, 169, dim, 1, range, block, d2fdyz_kernel_eqAH_execute);
}
#endif
