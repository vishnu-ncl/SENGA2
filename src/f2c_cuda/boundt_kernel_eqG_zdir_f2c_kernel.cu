// Auto-generated at 2026-04-28 18:44:04.158920 by ops-translator

__constant__ int dims_boundt_kernel_eqG_zdir[12][3];
static int dims_boundt_kernel_eqG_zdir_h[12][3] = {{0}};

//  =============
//  User function
//  =============
__device__ void boundt_kernel_eqG_zdir_gpu(ACC<double> &erhs, const ACC<double> &yrhs, const ACC<int> &itndex, const ACC<double> &strtz, const double *amasch, const double *rgspec, const int *ncpoly, const int *ncpom1, const int *ncenth, const int *ispec, const int *icoef1, const int *icoef2) {
...
//  ============================
__global__ void ops_boundt_kernel_eqG_zdir(double* __restrict arg0, int xstride_0, int ystride_0, int zstride_0, 
...
int size0, int size1, int size2) {
    ...
    arg0 += idx_x * xstride_0 + idx_y * ystride_0 * dims_boundt_kernel_eqG_zdir[0][0] + idx_z * zstride_0 * dims_boundt_kernel_eqG_zdir[0][0] * dims_boundt_kernel_eqG_zdir[0][1];
    arg1 += idx_x * xstride_1 + idx_y * ystride_1 * dims_boundt_kernel_eqG_zdir[1][0] + idx_z * zstride_1 * dims_boundt_kernel_eqG_zdir[1][0] * dims_boundt_kernel_eqG_zdir[1][1];
    arg2 += idx_x * xstride_2 + idx_y * ystride_2 * dims_boundt_kernel_eqG_zdir[2][0] + idx_z * zstride_2 * dims_boundt_kernel_eqG_zdir[2][0] * dims_boundt_kernel_eqG_zdir[2][1];
    arg3 += idx_x * xstride_3 + idx_y * ystride_3 * dims_boundt_kernel_eqG_zdir[3][0] + idx_z * zstride_3 * dims_boundt_kernel_eqG_zdir[3][0] * dims_boundt_kernel_eqG_zdir[3][1];

    if(idx_x < size0 && idx_y < size1 && idx_z < size2) {

        ACC<double> argp0(dims_boundt_kernel_eqG_zdir[0][0], dims_boundt_kernel_eqG_zdir[0][1], arg0);
        const ACC<double> argp1(dims_boundt_kernel_eqG_zdir[1][0], dims_boundt_kernel_eqG_zdir[1][1], arg1);
        const ACC<int> argp2(dims_boundt_kernel_eqG_zdir[2][0], dims_boundt_kernel_eqG_zdir[2][1], arg2);
        const ACC<double> argp3(dims_boundt_kernel_eqG_zdir[3][0], dims_boundt_kernel_eqG_zdir[3][1], arg3);

        boundt_kernel_eqG_zdir_gpu(argp0, argp1, argp2, argp3, arg4, arg5, arg6, arg7, arg8, &arg9, &arg10, &arg11);
...
//  ======================================================
    ...
    if (xdim0 != dims_boundt_kernel_eqG_zdir_h[0][0] || ydim0 != dims_boundt_kernel_eqG_zdir_h[0][1] || zdim0 != dims_boundt_kernel_eqG_zdir_h[0][2] || xdim1 != dims_boundt_kernel_eqG_zdir_h[1][0] || ydim1 != dims_boundt_kernel_eqG_zdir_h[1][1] || zdim1 != dims_boundt_kernel_eqG_zdir_h[1][2] || xdim2 != dims_boundt_kernel_eqG_zdir_h[2][0] || ydim2 != dims_boundt_kernel_eqG_zdir_h[2][1] || zdim2 != dims_boundt_kernel_eqG_zdir_h[2][2] || xdim3 != dims_boundt_kernel_eqG_zdir_h[3][0] || ydim3 != dims_boundt_kernel_eqG_zdir_h[3][1] || zdim3 != dims_boundt_kernel_eqG_zdir_h[3][2]) {
        dims_boundt_kernel_eqG_zdir_h[0][0] = xdim0;
        dims_boundt_kernel_eqG_zdir_h[0][1] = ydim0;
        dims_boundt_kernel_eqG_zdir_h[0][2] = zdim0;
        dims_boundt_kernel_eqG_zdir_h[1][0] = xdim1;
        dims_boundt_kernel_eqG_zdir_h[1][1] = ydim1;
        dims_boundt_kernel_eqG_zdir_h[1][2] = zdim1;
        dims_boundt_kernel_eqG_zdir_h[2][0] = xdim2;
        dims_boundt_kernel_eqG_zdir_h[2][1] = ydim2;
        dims_boundt_kernel_eqG_zdir_h[2][2] = zdim2;
        dims_boundt_kernel_eqG_zdir_h[3][0] = xdim3;
        dims_boundt_kernel_eqG_zdir_h[3][1] = ydim3;
        dims_boundt_kernel_eqG_zdir_h[3][2] = zdim3;

        cutilSafeCall(block->instance->ostream(), cudaMemcpyToSymbol( dims_boundt_kernel_eqG_zdir, dims_boundt_kernel_eqG_zdir_h, sizeof(dims_boundt_kernel_eqG_zdir)));
    }
...
//  =======================
    ...
    double * __restrict__ strtz_p = (double *)(args[3].data_d) + base3 - 1; // Subtracting 1 to convert to C-style
    p_a[3] = (char *)strtz_p;
...
//  call kernel wrapper function, passing in pointers to data
    if (x_size > 0 && y_size > 0 && z_size > 0) {
        ...
        ops_boundt_kernel_eqG_zdir<<<grid, tblock >>> (
    ...
    )
{
    ...
    create_kerneldesc_and_enque("boundt_kernel_eqG_zdir", args, 12, 516, dim, 1, range, block, boundt_kernel_eqG_zdir_execute);
}
#endif
