...
!   PARAMETER I1=1, R1=W-VELOCITY
    IF(nzlprm(1) == 1) THEN
        ...
        call ops_par_loop(bcut_kernel_zdir, "bcut_kernel_zdir", senga_grid, 3, rangexyz,  &
                        ...
                        ops_arg_dat(d_dudtzl, 1, s3d_000_strid3d_xy, "real(dp)", OPS_WRITE), &
                        ops_arg_dat(d_dvdtzl, 1, s3d_000_strid3d_xy, "real(dp)", OPS_WRITE),  &
                        ops_arg_dat(d_dwdtzl, 1, s3d_000_strid3d_xy, "real(dp)", OPS_WRITE), &
                        ops_arg_gbl(rzlprm(1), 1, "real(dp)", OPS_READ))
