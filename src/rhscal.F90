...
!       CONDUCTIVITY FOR EACH SPECIES
        DO ispec = 1, nspec
            call ops_par_loop(maths_kernel_eqBD, "THERMAL CONDUCTIVITY - part 2", senga_grid, 3, rangexyz, &
                            ...
                            ops_arg_gbl(ncocon, 1, "integer(4)", OPS_READ), &
                            ops_arg_gbl(ncocm1, 1, "integer(4)", OPS_READ), &
...
!               COMBINATION RULE FOR MASS DIFFUSIVITY
                call ops_par_loop(maths_kernel_eqBF, "MASS DIFFUSIVITY FOR EACH SPECIES - part 1", senga_grid, 3, rangexyz, &
                                ...
                                ops_arg_dat(d_transp, 1, s3d_000, "real(8)", OPS_READ), &
                                ops_arg_dat(d_prun, 1, s3d_000, "real(8)", OPS_READ), &
                                ops_arg_dat(d_yrhs(jspec), 1, s3d_000, "real(8)", OPS_READ), &
                                ...
                                ops_arg_gbl(ncodif, 1, "integer(4)", OPS_READ), &
                                ops_arg_gbl(ncodm1, 1, "integer(4)", OPS_READ), &
                                ops_arg_gbl(jspec, 1, "integer(4)", OPS_READ), &
                                ops_arg_gbl(ispec, 1, "integer(4)", OPS_READ))
            END DO
            ...
            DO jspec = 1, nspec
                ...
                IF(flmtds)THEN
                    ...
                    call ops_par_loop(maths_kernel_eqBH, "THERMAL DIFFUSION RATIO FOR EACH SPECIES", senga_grid, 3, rangexyz, &
                                    ...
                                    ops_arg_gbl(ovwmol, nspcmx, "real(8)", OPS_READ), &
                                    ops_arg_gbl(tdifgb, 1, "real(8)", OPS_READ), &
                                    ops_arg_gbl(ncotdr, 1, "integer(4)", OPS_READ), &
                                    ops_arg_gbl(jspec, 1, "integer(4)", OPS_READ), &
                                    ops_arg_gbl(ispec, 1, "integer(4)", OPS_READ), &
                                    ops_arg_gbl(ncotm1, 1, "integer(4)", OPS_READ))
    ...
    IF(flmavt)THEN
        ...
        DO ispec = 1, nspec
            call ops_par_loop(maths_kernel_eqBI, "STORE VISCOSITY IN DIFMIX - part 1", senga_grid, 3, rangexyz, &
                            ...
                            ops_arg_gbl(ncovis, 1, "integer(4)", OPS_READ), &
                            ops_arg_gbl(ncovm1, 1, "integer(4)", OPS_READ), &
                            ops_arg_gbl(ispec, 1, "integer(4)", OPS_READ))
        END DO
