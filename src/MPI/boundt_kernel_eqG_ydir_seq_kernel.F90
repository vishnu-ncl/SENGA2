...
SUBROUTINE boundt_kernel_eqG_ydir(erhs,yrhs,itndex,strty,amasch,rgspec,ncpoly,ncpom1,ncenth,ispec,iindex,icoef1,icoef2)
    use data_types
    implicit none
    ...
    erhs(OPS_ACC1(0,0,0)) = erhs(OPS_ACC1(0,0,0))  &
                      + (fornow-rgspec(ispec)*strty(OPS_ACC4(0,0,0)))*yrhs(OPS_ACC_MD2(ispec,0,0,0))
...
subroutine boundt_kernel_eqG_ydir_host_execute( descPtr )
  ...
  call setKernelTime(485,userSubroutine,0.0_8,0.0_8,0.0_4,1)
  ...
  transfer_total = transfer_total + transfer
  call setKernelTime(485,userSubroutine,t3-t2,t2-t1,transfer_total,0)
end subroutine
...
& opsArg13)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 13, 485, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(boundt_kernel_eqG_ydir_host_execute))
end subroutine
#endif
