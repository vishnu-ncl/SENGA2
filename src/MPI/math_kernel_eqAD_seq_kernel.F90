...
!
MODULE MATH_KERNEL_EQAD_MODULE
...
!user function
!DEC$ ATTRIBUTES FORCEINLINE :: math_kernel_eqAD
...
subroutine math_kernel_eqAD_wrap( &
& opsDat1Local, &
& opsDat2Local, &
& opsDat3Local, &
& opsDat4Local, &
...
!host subroutine
#ifndef OPS_LAZY
subroutine math_kernel_eqAD_host( userSubroutine, block, dim, range, &
& opsArg1, &
& opsArg2, &
& opsArg3, &
& opsArg4, &
...
#else
subroutine math_kernel_eqAD_host_execute( descPtr )
  use iso_c_binding
  IMPLICIT NONE
  ...
  call setKernelTime(206,userSubroutine,0.0_8,0.0_8,0.0_4,1)
  ...
  transfer_total = transfer_total + transfer
  call setKernelTime(206,userSubroutine,t3-t2,t2-t1,transfer_total,0)
end subroutine
...
& opsArg8)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 8, 206, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(math_kernel_eqAD_host_execute))
end subroutine
#endif
