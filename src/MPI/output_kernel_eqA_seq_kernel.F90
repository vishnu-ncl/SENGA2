...
!user function
!DEC$ ATTRIBUTES FORCEINLINE :: output_kernel_eqA
SUBROUTINE output_kernel_eqA(drun,utgv,vtgv,wtgv,tkes)
  use data_types
  implicit none
  ...
  tkes=tkes+(0.5d0*drun(OPS_ACC1(0,0,0))*(utgv(OPS_ACC1(0,0,0))*utgv(OPS_ACC1(0,0,0))+ &
	  ...
	  *(1.0d0/ovdelx)*(1.0d0/ovdelx)*(1.0d0/ovdelx))
...
subroutine output_kernel_eqA_wrap( &
& opsDat1Local, &
& opsDat2Local, &
& opsDat3Local, &
& opsDat4Local, &
...
!host subroutine
#ifndef OPS_LAZY
subroutine output_kernel_eqA_host( userSubroutine, block, dim, range, &
& opsArg1, &
& opsArg2, &
& opsArg3, &
& opsArg4, &
...
#else
subroutine output_kernel_eqA_host_execute( descPtr )
...
!  Set from kernel descriptor
  ...
  call setKernelTime(535,userSubroutine,0.0_8,0.0_8,0.0_4,1)
...
& opsArg5)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 5, 535, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(output_kernel_eqA_host_execute))
