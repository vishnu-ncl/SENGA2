...
!user function
!DEC$ ATTRIBUTES FORCEINLINE :: math_kernel_eqD


SUBROUTINE math_kernel_eqD(out_arr, in_arr1, var, ispec)
    use data_types
    implicit none
...
subroutine math_kernel_eqD_wrap( &
& opsDat1Local, &
& opsDat2Local, &
& opsDat3Local, &
& opsDat4Local, &
...
!host subroutine
#ifndef OPS_LAZY
subroutine math_kernel_eqD_host( userSubroutine, block, dim, range, &
& opsArg1, &
& opsArg2, &
& opsArg3, &
& opsArg4)
...
#else
subroutine math_kernel_eqD_host_execute( descPtr )
  use iso_c_binding
  IMPLICIT NONE
...
& opsArg4)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 4, 234, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(math_kernel_eqD_host_execute))
end subroutine
#endif
