...
SUBROUTINE bounds_kernel_inflowBC1_LYX_xl(bclyxl,ratexl,struxl,strdxl)
    use data_types
    implicit none
...
!  Set from kernel descriptor
  ...
  call setKernelTime(337,userSubroutine,0.0_8,0.0_8,0.0_4,1)
  ...
  transfer_total = transfer_total + transfer
  call setKernelTime(337,userSubroutine,t3-t2,t2-t1,transfer_total,0)
...
& opsArg4)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 4, 337, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(bounds_kernel_inflowBC1_LYX_xl_host_execute))
