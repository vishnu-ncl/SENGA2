...
SUBROUTINE boundt_kernel_eqE_xdir(strtx,itndex,tinthi,ntint)
    use data_types
    implicit none
  ...
  DO n_z = 1, end(3)-start(3)+1
    DO n_y = 1, end(2)-start(2)+1
      ...
      DO n_x = 1, end(1)-start(1)+1
        ...
        & opsDat2Local(dat2_base+(n_x-1)*2 + (n_y-1)*xdim2*2 + (n_z-1)*ydim2*xdim2*2), &
...
subroutine boundt_kernel_eqE_xdir_host_execute( descPtr )
  ...
  integer, POINTER, DIMENSION(:) :: opsDat2Local
  ...
  call setKernelTime(471,userSubroutine,0.0_8,0.0_8,0.0_4,1)
  ...
  transfer_total = transfer_total + transfer
  call setKernelTime(471,userSubroutine,t3-t2,t2-t1,transfer_total,0)
end subroutine
...
& opsArg4)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 4, 471, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(boundt_kernel_eqE_xdir_host_execute))
end subroutine
#endif
