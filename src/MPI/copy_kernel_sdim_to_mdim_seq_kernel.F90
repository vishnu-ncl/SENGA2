  ...
  DO n_z = 1, end_indx(3)-start_indx(3)+1
    DO n_y = 1, end_indx(2)-start_indx(2)+1
      ...
      DO n_x = 1, end_indx(1)-start_indx(1)+1
        ...
        & opsDat1Local(dat1_base+(n_x-1)*2 + (n_y-1)*xdim1*2 + (n_z-1)*ydim1*xdim1*2), &
...
!  Set from kernel descriptor
  ...
  call setKernelTime(532,userSubroutine,0.0_8,0.0_8,0.0_4,1)
  ...
  transfer_total = transfer_total + transfer
  call setKernelTime(532,userSubroutine,t3-t2,t2-t1,transfer_total,0)
...
& opsArg3)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 3, 532, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(copy_kernel_sdim_to_mdim_host_execute))
