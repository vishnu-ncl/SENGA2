...
& end_indx )
  ...
  integer(4) :: dat2_base
  integer(kind=4), dimension(*), intent(inout) :: opsDat3Local
  integer(4) :: dat3_base
  ...
  DO n_z = 1, end_indx(3)-start_indx(3)+1
    DO n_y = 1, end_indx(2)-start_indx(2)+1
      ...
      DO n_x = 1, end_indx(1)-start_indx(1)+1
        ...
        & opsDat1Local(dat1_base+(n_x-1)*6 + (n_y-1)*xdim1*6 + (n_z-1)*ydim1*xdim1*6), &
        & opsDat2Local(dat2_base+(n_x-1)*5 + (n_y-1)*xdim2*5 + (n_z-1)*ydim2*xdim2*5), &
...
!  Set from kernel descriptor
  ...
  call setKernelTime(527,userSubroutine,0.0_8,0.0_8,0.0_4,1)
...
& opsArg12)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 12, 527, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(temper_kernel_eqB_host_execute))
