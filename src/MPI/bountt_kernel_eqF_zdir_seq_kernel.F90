...
& end )
  ...
  real(8) opsDat3Local(*)
  real(8) opsDat4Local(*)
  integer, INTENT(IN) :: opsDat5Local(*)
  ...
  DO n_z = 1, end(3)-start(3)+1
    DO n_y = 1, end(2)-start(2)+1
      ...
      DO n_x = 1, end(1)-start(1)+1
        ...
        & opsDat8Local(dat8_base+(n_x-1)*9 + (n_y-1)*xdim8*9 + (n_z-1)*ydim8*xdim8*9), &
...
subroutine bountt_kernel_eqF_zdir_host_execute( descPtr )
  ...
  integer, POINTER, DIMENSION(:) :: opsDat17Local
  ...
  call setKernelTime(489,userSubroutine,0.0_8,0.0_8,0.0_4,1)
  ...
  multi_d8 = getDatDimFromOpsArg(opsArg8) ! dimension of the dat
  dat8_base = getDatBaseFromOpsArg3D(opsArg8,start,multi_d8)
  ...
  transfer_total = transfer_total + transfer
  call setKernelTime(489,userSubroutine,t3-t2,t2-t1,transfer_total,0)
end subroutine
...
& opsArg17)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 17, 489, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(bountt_kernel_eqF_zdir_host_execute))
end subroutine
#endif
