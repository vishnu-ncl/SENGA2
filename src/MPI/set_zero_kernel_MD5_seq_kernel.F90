...
SUBROUTINE set_zero_kernel_MD5(farray, ispec)
    use data_types
    implicit none
  ...
  DO n_z = 1, end_indx(3)-start_indx(3)+1
    DO n_y = 1, end_indx(2)-start_indx(2)+1
      ...
      DO n_x = 1, end_indx(1)-start_indx(1)+1
        ...
        & opsDat1Local(dat1_base+(n_x-1)*5 + (n_y-1)*xdim1*5 + (n_z-1)*ydim1*xdim1*5), &
...
& opsArg2)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 2, 526, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(set_zero_kernel_MD5_host_execute))
