  ...
  DO n_z = 1, end_indx(3)-start_indx(3)+1
    ...
    DO n_y = 1, end_indx(2)-start_indx(2)+1
      ...
      DO n_x = 1, end_indx(1)-start_indx(1)+1
        ...
        & opsDat7Local(dat7_base+(n_x-1)*2 + (n_y-1)*xdim7*2 + (n_z-1)*ydim7*xdim7*2), &
...
!  Set from kernel descriptor
  ...
  call setKernelTime(533,userSubroutine,0.0_8,0.0_8,0.0_4,1)
...
& opsArg17)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 17, 533, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(tempin_kernel_main_host_execute))
