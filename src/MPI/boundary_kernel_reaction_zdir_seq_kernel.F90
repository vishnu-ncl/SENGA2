...
!
MODULE BOUNDARY_KERNEL_REACTION_ZDIR_MODULE
...
!user function
!DEC$ ATTRIBUTES FORCEINLINE :: boundary_kernel_reaction_zdir
...
subroutine boundary_kernel_reaction_zdir_wrap( &
& opsDat1Local, &
& opsDat2Local, &
& opsDat3Local, &
& dat1_base, &
  ...
  DO n_z = 1, end(3)-start(3)+1
    DO n_y = 1, end(2)-start(2)+1
      ...
      DO n_x = 1, end(1)-start(1)+1
        call boundary_kernel_reaction_zdir( &
...
!host subroutine
#ifndef OPS_LAZY
subroutine boundary_kernel_reaction_zdir_host( userSubroutine, block, dim, range, &
& opsArg1, &
& opsArg2, &
& opsArg3)
  IMPLICIT NONE
...
#else
subroutine boundary_kernel_reaction_zdir_host_execute( descPtr )
  use iso_c_binding
  IMPLICIT NONE
  ...
  call setKernelTime(223,userSubroutine,0.0_8,0.0_8,0.0_4,1)
  ...
  call boundary_kernel_reaction_zdir_wrap( &
  & opsDat1Local, &
  & opsDat2Local, &
  & opsDat3Local, &
  & dat1_base, &
  ...
  transfer_total = transfer_total + transfer
  call setKernelTime(223,userSubroutine,t3-t2,t2-t1,transfer_total,0)
end subroutine
...
& opsArg3)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 3, 223, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(boundary_kernel_reaction_zdir_host_execute))
end subroutine
#endif
