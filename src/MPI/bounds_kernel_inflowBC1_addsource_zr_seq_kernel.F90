...
!
MODULE BOUNDS_KERNEL_INFLOWBC1_ADDSOURCE_ZR_MODULE
...
!user function
!DEC$ ATTRIBUTES FORCEINLINE :: bounds_kernel_inflowBC1_addsource_zr
...
subroutine bounds_kernel_inflowBC1_addsource_zr_wrap( &
& opsDat1Local, &
& opsDat2Local, &
& opsDat3Local, &
& opsDat4Local, &
...
& end )
  ...
  real(8), INTENT(IN) :: opsDat12Local(*)
  real(8), INTENT(IN) :: opsDat13Local(*)
  real(8), INTENT(IN) :: opsDat14Local(*)
  real(8), INTENT(IN) :: opsDat15Local(*)
  real(8), INTENT(IN) :: opsDat16Local(*)
  real(8), INTENT(IN) :: opsDat17Local(*)
  integer dat1_base
  integer dat2_base
  integer dat3_base
  integer dat4_base
  ...
  DO n_z = 1, end(3)-start(3)+1
    DO n_y = 1, end(2)-start(2)+1
      ...
      DO n_x = 1, end(1)-start(1)+1
        call bounds_kernel_inflowBC1_addsource_zr( &
        ...
        & opsDat16Local(dat16_base+(n_x-1)*1 + (n_y-1)*xdim16*1 + (n_z-1)*ydim16*xdim16*1), &
        & opsDat17Local(dat17_base+(n_x-1)*1 + (n_y-1)*xdim17*1 + (n_z-1)*ydim17*xdim17*1) )
      END DO
    END DO
  END DO
end subroutine
...
!host subroutine
#ifndef OPS_LAZY
subroutine bounds_kernel_inflowBC1_addsource_zr_host( userSubroutine, block, dim, range, &
& opsArg1, &
& opsArg2, &
& opsArg3, &
& opsArg4, &
...
#else
subroutine bounds_kernel_inflowBC1_addsource_zr_host_execute( descPtr )
  use iso_c_binding
  IMPLICIT NONE
  ...
  call setKernelTime(457,userSubroutine,0.0_8,0.0_8,0.0_4,1)
  ...
  dat11_base = getDatBaseFromOpsArg3D(opsArg11,start,1)
  ...
  dat12_base = getDatBaseFromOpsArg3D(opsArg12,start,1)
  ...
  dat13_base = getDatBaseFromOpsArg3D(opsArg13,start,1)
  ...
  dat14_base = getDatBaseFromOpsArg3D(opsArg14,start,1)
  ...
  dat15_base = getDatBaseFromOpsArg3D(opsArg15,start,1)
  ...
  dat16_base = getDatBaseFromOpsArg3D(opsArg16,start,1)
  ...
  dat17_base = getDatBaseFromOpsArg3D(opsArg17,start,1)
  ...
  call bounds_kernel_inflowBC1_addsource_zr_wrap( &
  & opsDat1Local, &
  & opsDat2Local, &
  & opsDat3Local, &
  & opsDat4Local, &
  ...
  transfer_total = transfer_total + transfer
  call ops_compute_transfer(3, start, end, opsArg12,transfer)
  transfer_total = transfer_total + transfer
  call ops_compute_transfer(3, start, end, opsArg13,transfer)
  transfer_total = transfer_total + transfer
  call ops_compute_transfer(3, start, end, opsArg14,transfer)
  transfer_total = transfer_total + transfer
  call ops_compute_transfer(3, start, end, opsArg15,transfer)
  transfer_total = transfer_total + transfer
  call ops_compute_transfer(3, start, end, opsArg16,transfer)
  transfer_total = transfer_total + transfer
  call ops_compute_transfer(3, start, end, opsArg17,transfer)
  transfer_total = transfer_total + transfer
  call setKernelTime(457,userSubroutine,t3-t2,t2-t1,transfer_total,0)
end subroutine
...
& opsArg17)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 17, 457, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(bounds_kernel_inflowBC1_addsource_zr_host_execute))
end subroutine
#endif
