...
MODULE BOUNDS_KERNEL_REDUCED_ENERGY_ZDIR_MODULE
...
!user function
!DEC$ ATTRIBUTES FORCEINLINE :: bounds_kernel_reduced_energy_zdir
...
subroutine bounds_kernel_reduced_energy_zdir_wrap( &
& opsDat1Local, &
& opsDat2Local, &
& opsDat3Local, &
& opsDat4Local, &
  ...
  DO n_z = 1, end_indx(3)-start_indx(3)+1
    DO n_y = 1, end_indx(2)-start_indx(2)+1
      ...
      DO n_x = 1, end_indx(1)-start_indx(1)+1
        ...
        call bounds_kernel_reduced_energy_zdir( &
...
!host subroutine
#ifndef OPS_LAZY
subroutine bounds_kernel_reduced_energy_zdir_host( userSubroutine, block, dim, range, &
& opsArg1, &
& opsArg2, &
& opsArg3, &
& opsArg4, &
...
#else
subroutine bounds_kernel_reduced_energy_zdir_host_execute( descPtr )
...
!  Set from kernel descriptor
  ...
  call bounds_kernel_reduced_energy_zdir_wrap( &
  & opsDat1Local, &
  & opsDat2Local, &
  & opsDat3Local, &
  & opsDat4Local, &
...
& opsArg6)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 6, 419, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(bounds_kernel_reduced_energy_zdir_host_execute))
