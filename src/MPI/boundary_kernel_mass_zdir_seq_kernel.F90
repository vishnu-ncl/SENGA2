...
SUBROUTINE boundary_kernel_mass_zdir(yrhs, store3, stryz, bclyz)
    use data_types
    implicit none
    ...
    stryz(OPS_ACC3(0,0,0)) = yrhs(OPS_ACC1(0,0,0))
    bclyz(OPS_ACC4(0,0,0)) = store3(OPS_ACC2(0,0,0))
...
!  Set from kernel descriptor
  ...
  call setKernelTime(227,userSubroutine,0.0_8,0.0_8,0.0_4,1)
  ...
  transfer_total = transfer_total + transfer
  call setKernelTime(227,userSubroutine,t3-t2,t2-t1,transfer_total,0)
...
& opsArg4)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 4, 227, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(boundary_kernel_mass_zdir_host_execute))
