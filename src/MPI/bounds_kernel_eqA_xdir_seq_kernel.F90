...
SUBROUTINE bounds_kernel_eqA_xdir(sorpx,strhx,ratex)
    use data_types
    implicit none
...
!  Set from kernel descriptor
  ...
  call setKernelTime(331,userSubroutine,0.0_8,0.0_8,0.0_4,1)
  ...
  transfer_total = transfer_total + transfer
  call setKernelTime(331,userSubroutine,t3-t2,t2-t1,transfer_total,0)
...
& opsArg3)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 3, 331, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(bounds_kernel_eqA_xdir_host_execute))
