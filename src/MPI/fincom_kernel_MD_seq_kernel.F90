...
!
MODULE FINCOM_KERNEL_MD_MODULE
...
!user function
!DEC$ ATTRIBUTES FORCEINLINE :: fincom_kernel_MD


SUBROUTINE fincom_kernel_MD(err_arr,run_arr,rhs_arr,rkerr,rklhs,nrkstp,ispec)
    use data_types
    implicit none

    real(kind=8), dimension(1) :: err_arr, run_arr, rhs_arr
    integer, intent(in) :: ispec,nrkstp
...
subroutine fincom_kernel_MD_wrap( &
& opsDat1Local, &
& opsDat2Local, &
& opsDat3Local, &
& opsDat4Local, &
...
!host subroutine
#ifndef OPS_LAZY
subroutine fincom_kernel_MD_host( userSubroutine, block, dim, range, &
& opsArg1, &
& opsArg2, &
& opsArg3, &
& opsArg4, &
...
#else
subroutine fincom_kernel_MD_host_execute( descPtr )
  use iso_c_binding
  IMPLICIT NONE
  ...
  call setKernelTime(315,userSubroutine,0.0_8,0.0_8,0.0_4,1)
  ...
  transfer_total = transfer_total + transfer
  call setKernelTime(315,userSubroutine,t3-t2,t2-t1,transfer_total,0)
end subroutine
...
& opsArg7)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 7, 315, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(fincom_kernel_MD_host_execute))
end subroutine
#endif
