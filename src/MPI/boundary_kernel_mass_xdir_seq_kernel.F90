...
SUBROUTINE boundary_kernel_mass_xdir(yrhs, store1, stryx, bclyx)
    use data_types
    implicit none
    ...
    stryx(OPS_ACC3(0,0,0)) = yrhs(OPS_ACC1(0,0,0))
    bclyx(OPS_ACC4(0,0,0)) = store1(OPS_ACC2(0,0,0))
...
& opsArg4)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 4, 225, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(boundary_kernel_mass_xdir_host_execute))
