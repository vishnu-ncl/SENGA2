...
!
MODULE LINCOM_KERNEL_MD_MODULE
...
!user function
!DEC$ ATTRIBUTES FORCEINLINE :: lincom_kernel_MD


SUBROUTINE lincom_kernel_MD(err_arr,run_arr,rhs_arr,rkerr,rklhs,rkrhs,irkstp,ispec)
    use data_types
    implicit none

    real(kind=8), dimension(1) :: err_arr, run_arr, rhs_arr
    integer, intent(in) :: ispec,irkstp
    real(kind=8), dimension(nrkmax), intent(in) :: rkerr, rklhs, rkrhs
    real(kind=8) :: fornow

    err_arr(OPS_ACC_MD1(ispec,0,0,0)) = err_arr(OPS_ACC_MD1(ispec,0,0,0)) + rkerr(irkstp) * rhs_arr(OPS_ACC_MD3(ispec,0,0,0))

    fornow = run_arr(OPS_ACC_MD2(ispec,0,0,0))
    run_arr(OPS_ACC_MD2(ispec,0,0,0)) = fornow + rklhs(irkstp) * rhs_arr(OPS_ACC_MD3(ispec,0,0,0))
    rhs_arr(OPS_ACC_MD3(ispec,0,0,0)) = fornow + rkrhs(irkstp) * rhs_arr(OPS_ACC_MD3(ispec,0,0,0))
...
subroutine lincom_kernel_MD_wrap( &
& opsDat1Local, &
& opsDat2Local, &
& opsDat3Local, &
& opsDat4Local, &
...
!host subroutine
#ifndef OPS_LAZY
subroutine lincom_kernel_MD_host( userSubroutine, block, dim, range, &
& opsArg1, &
& opsArg2, &
& opsArg3, &
& opsArg4, &
...
#else
subroutine lincom_kernel_MD_host_execute( descPtr )
  use iso_c_binding
  IMPLICIT NONE
  ...
  call setKernelTime(313,userSubroutine,0.0_8,0.0_8,0.0_4,1)
  ...
  transfer_total = transfer_total + transfer
  call setKernelTime(313,userSubroutine,t3-t2,t2-t1,transfer_total,0)
end subroutine
...
& opsArg8)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 8, 313, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(lincom_kernel_MD_host_execute))
end subroutine
#endif
