...
MODULE BOUNDS_KERNEL_OUTFLOWBC1_COMPUTEL_XL_MODULE
...
!user function
!DEC$ ATTRIBUTES FORCEINLINE :: bounds_kernel_outflowBC1_computeL_xl
...
SUBROUTINE bounds_kernel_outflowBC1_computeL_xl(bcl5xl,struxl,acouxl,strdxl,bcl1xl,sorpxl,strpxl,cobcxl,pinfxl)
    ...
    real(kind=8), dimension(1) :: bcl5xl
    real(kind=8), dimension(1), intent(in) :: struxl,acouxl,strdxl,bcl1xl,sorpxl,strpxl
    real(kind=8), intent(in) :: cobcxl,pinfxl

    bcl5xl(OPS_ACC1(0,0,0)) = 0.5_8*(struxl(OPS_ACC2(0,0,0))+acouxl(OPS_ACC3(0,0,0)))  &
            *(bcl5xl(OPS_ACC1(0,0,0))+strdxl(OPS_ACC4(0,0,0))*acouxl(OPS_ACC3(0,0,0))*bcl1xl(OPS_ACC5(0,0,0)))

    bcl5xl(OPS_ACC1(0,0,0))= 0.5_8*sorpxl(OPS_ACC6(0,0,0))  &
...
subroutine bounds_kernel_outflowBC1_computeL_xl_wrap( &
& opsDat1Local, &
& opsDat2Local, &
& opsDat3Local, &
& opsDat4Local, &
  ...
  DO n_z = 1, end_indx(3)-start_indx(3)+1
    DO n_y = 1, end_indx(2)-start_indx(2)+1
      ...
      DO n_x = 1, end_indx(1)-start_indx(1)+1
        ...
        call bounds_kernel_outflowBC1_computeL_xl( &
...
!host subroutine
#ifndef OPS_LAZY
subroutine bounds_kernel_outflowBC1_computeL_xl_host( userSubroutine, block, dim, range, &
& opsArg1, &
& opsArg2, &
& opsArg3, &
& opsArg4, &
...
#else
subroutine bounds_kernel_outflowBC1_computeL_xl_host_execute( descPtr )
...
!  Set from kernel descriptor
  ...
  call setKernelTime(333,userSubroutine,0.0_8,0.0_8,0.0_4,1)
  ...
  call bounds_kernel_outflowBC1_computeL_xl_wrap( &
  & opsDat1Local, &
  & opsDat2Local, &
  & opsDat3Local, &
  & opsDat4Local, &
  ...
  transfer_total = transfer_total + transfer
  call setKernelTime(333,userSubroutine,t3-t2,t2-t1,transfer_total,0)
...
& opsArg9)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 9, 333, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(bounds_kernel_outflowBC1_computeL_xl_host_execute))
