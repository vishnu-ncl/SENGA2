...
!
MODULE TEMPER_KERNEL_MAIN_MODULE
...
!user function
!DEC$ ATTRIBUTES FORCEINLINE :: temper_kernel_main
SUBROUTINE temper_kernel_main(store7,trun,transp,prun,itndex,urhs,vrhs,wrhs,drhs,erhs,yrhs,amascp,amasct,ncenth,ncpom1,ncpoly,tinthi,rgspec,ntint,nspec,idx)
    use data_types
    implicit none
...
subroutine temper_kernel_main_wrap( &
& opsDat1Local, &
& opsDat2Local, &
& opsDat3Local, &
& opsDat4Local, &
...
!host subroutine
#ifndef OPS_LAZY
subroutine temper_kernel_main_host( userSubroutine, block, dim, range, &
& opsArg1, &
& opsArg2, &
& opsArg3, &
& opsArg4, &
...
& opsArg21)
  ...
  type ( ops_arg ) , DIMENSION(21) :: opsArgArray
#else
subroutine temper_kernel_main_host_execute( descPtr )
  use iso_c_binding
  IMPLICIT NONE
  ...
  integer, POINTER, DIMENSION(:) :: opsDat20Local
  ...
  call setKernelTime(540,userSubroutine,0.0_8,0.0_8,0.0_4,1)
...
#ifndef OPS_LAZY
  call ops_H_D_exchanges_host(opsArgArray,21)
  call ops_halo_exchanges(opsArgArray,21,range)
  call ops_H_D_exchanges_host(opsArgArray,21)
#endif

  call ops_timers_core(t2)

  call temper_kernel_main_wrap( &
  & opsDat1Local, &
  & opsDat2Local, &
  & opsDat3Local, &
  & opsDat4Local, &
...
#ifndef OPS_LAZY
  call ops_set_dirtybit_host(opsArgArray, 21)
  ...
  transfer_total = transfer_total + transfer
  call setKernelTime(540,userSubroutine,t3-t2,t2-t1,transfer_total,0)
end subroutine
...
& opsArg21)
  ...
  type ( ops_arg ), INTENT(IN) :: opsArg20
  type ( ops_arg ), INTENT(IN) :: opsArg21
  type ( ops_arg ), DIMENSION(21), TARGET :: opsArgArray
  integer(kind=4) :: n
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 21, 540, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(temper_kernel_main_host_execute))
end subroutine
#endif
