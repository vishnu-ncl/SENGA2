...
MODULE HEAT_FLUX_KERNEL_SORET_FYLCNW_MODULE
...
!user function
!DEC$ ATTRIBUTES FORCEINLINE :: heat_flux_kernel_soret_fylcnw


SUBROUTINE heat_flux_kernel_soret_fylcnw(erhs,trun,tdrmix,store7,td1y,acbcyl,rgspec,ispec)
    use data_types
    implicit none
...
subroutine heat_flux_kernel_soret_fylcnw_wrap( &
& opsDat1Local, &
& opsDat2Local, &
& opsDat3Local, &
& opsDat4Local, &
  ...
  DO n_z = 1, end_indx(3)-start_indx(3)+1
    DO n_y = 1, end_indx(2)-start_indx(2)+1
      ...
      DO n_x = 1, end_indx(1)-start_indx(1)+1
        ...
        call heat_flux_kernel_soret_fylcnw( &
...
!host subroutine
#ifndef OPS_LAZY
subroutine heat_flux_kernel_soret_fylcnw_host( userSubroutine, block, dim, range, &
& opsArg1, &
& opsArg2, &
& opsArg3, &
& opsArg4, &
...
#else
subroutine heat_flux_kernel_soret_fylcnw_host_execute( descPtr )
...
!  Set from kernel descriptor
  ...
  call setKernelTime(260,userSubroutine,0.0_8,0.0_8,0.0_4,1)
  ...
  call heat_flux_kernel_soret_fylcnw_wrap( &
  & opsDat1Local, &
  & opsDat2Local, &
  & opsDat3Local, &
  & opsDat4Local, &
  ...
  transfer_total = transfer_total + transfer
  call setKernelTime(260,userSubroutine,t3-t2,t2-t1,transfer_total,0)
...
& opsArg8)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 8, 260, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(heat_flux_kernel_soret_fylcnw_host_execute))
