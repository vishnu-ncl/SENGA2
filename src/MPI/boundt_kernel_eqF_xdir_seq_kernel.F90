...
SUBROUTINE boundt_kernel_eqF_xdir(erhs,yrhs,itndex,drhs,strtx,stryx,amasch,rgspec,ncpoly,ncpom1,ncenth,ispec,iindex,icoef1,icoef2)
    use data_types
    implicit none
    ...
    erhs(OPS_ACC1(0,0,0)) = erhs(OPS_ACC1(0,0,0))  &
                      + (fornow-rgspec(ispec)*strtx(OPS_ACC5(0,0,0)))*yrhs(OPS_ACC_MD2(ispec,0,0,0))
  ...
  DO n_z = 1, end(3)-start(3)+1
    DO n_y = 1, end(2)-start(2)+1
      ...
      DO n_x = 1, end(1)-start(1)+1
        ...
        & opsDat6Local(dat6_base+(n_x-1)*9 + (n_y-1)*xdim6*9 + (n_z-1)*ydim6*xdim6*9), &
...
subroutine boundt_kernel_eqF_xdir_host_execute( descPtr )
  ...
  dat6_base = getDatBaseFromOpsArg3D(opsArg6,start,multi_d6)
...
& opsArg15)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 15, 495, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(boundt_kernel_eqF_xdir_host_execute))
end subroutine
#endif
