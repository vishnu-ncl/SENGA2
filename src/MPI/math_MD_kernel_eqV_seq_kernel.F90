...
!user function
!DEC$ ATTRIBUTES FORCEINLINE :: math_MD_kernel_eqV


SUBROUTINE math_MD_kernel_eqV(transp,store7,wmomix,trun,yrhs,drhs,condco,ovwmol,tdifgb,ncocon,ncocm1)
    use data_types
    implicit none
...
subroutine math_MD_kernel_eqV_wrap( &
& opsDat1Local, &
& opsDat2Local, &
& opsDat3Local, &
& opsDat4Local, &
...
!host subroutine
#ifndef OPS_LAZY
subroutine math_MD_kernel_eqV_host( userSubroutine, block, dim, range, &
& opsArg1, &
& opsArg2, &
& opsArg3, &
& opsArg4, &
...
#else
subroutine math_MD_kernel_eqV_host_execute( descPtr )
  use iso_c_binding
  IMPLICIT NONE
  ...
  call setKernelTime(211,userSubroutine,0.0_8,0.0_8,0.0_4,1)
...
& opsArg11)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 11, 211, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(math_MD_kernel_eqV_host_execute))
end subroutine
#endif
