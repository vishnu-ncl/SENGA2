...
MODULE BOUNDS_KERNEL_OUTFLOWBC1_COMPUTEL_YR_MODULE
...
!user function
!DEC$ ATTRIBUTES FORCEINLINE :: bounds_kernel_outflowBC1_computeL_yr
...
SUBROUTINE bounds_kernel_outflowBC1_computeL_yr(bcl1yr,strvyr,acouyr,bcl5yr,strdyr,sorpyr,strpyr,cobcyr,pinfyr)
    ...
    real(kind=8), dimension(1) :: bcl1yr
    real(kind=8), dimension(1), intent(in) :: strvyr,acouyr,bcl5yr,strdyr,sorpyr,strpyr
    real(kind=8), intent(in) :: cobcyr,pinfyr

    bcl1yr(OPS_ACC1(0,0,0)) = 0.5_8*(strvyr(OPS_ACC2(0,0,0))-acouyr(OPS_ACC3(0,0,0)))  &
            *(bcl5yr(OPS_ACC4(0,0,0))-strdyr(OPS_ACC5(0,0,0))*acouyr(OPS_ACC3(0,0,0))*bcl1yr(OPS_ACC1(0,0,0)))

    bcl1yr(OPS_ACC1(0,0,0))= 0.5_8*sorpyr(OPS_ACC6(0,0,0))  &
...
subroutine bounds_kernel_outflowBC1_computeL_yr_wrap( &
& opsDat1Local, &
& opsDat2Local, &
& opsDat3Local, &
& opsDat4Local, &
  ...
  DO n_z = 1, end_indx(3)-start_indx(3)+1
    DO n_y = 1, end_indx(2)-start_indx(2)+1
      ...
      DO n_x = 1, end_indx(1)-start_indx(1)+1
        ...
        call bounds_kernel_outflowBC1_computeL_yr( &
...
!host subroutine
#ifndef OPS_LAZY
subroutine bounds_kernel_outflowBC1_computeL_yr_host( userSubroutine, block, dim, range, &
& opsArg1, &
& opsArg2, &
& opsArg3, &
& opsArg4, &
...
#else
subroutine bounds_kernel_outflowBC1_computeL_yr_host_execute( descPtr )
...
!  Set from kernel descriptor
  ...
  call bounds_kernel_outflowBC1_computeL_yr_wrap( &
  & opsDat1Local, &
  & opsDat2Local, &
  & opsDat3Local, &
  & opsDat4Local, &
...
& opsArg9)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 9, 399, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(bounds_kernel_outflowBC1_computeL_yr_host_execute))
