...
!user function
!DEC$ ATTRIBUTES FORCEINLINE :: math_kernel_eqF


SUBROUTINE math_kernel_eqF(out_arr, in_arr1, in_arr2, rgspec, ispec)
    use data_types
    implicit none
...
subroutine math_kernel_eqF_wrap( &
& opsDat1Local, &
& opsDat2Local, &
& opsDat3Local, &
& opsDat4Local, &
...
!host subroutine
#ifndef OPS_LAZY
subroutine math_kernel_eqF_host( userSubroutine, block, dim, range, &
& opsArg1, &
& opsArg2, &
& opsArg3, &
& opsArg4, &
...
#else
subroutine math_kernel_eqF_host_execute( descPtr )
  use iso_c_binding
  IMPLICIT NONE
...
& opsArg5)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 5, 242, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(math_kernel_eqF_host_execute))
end subroutine
#endif
