...
!
MODULE BOUNDS_KERNEL_INFLOWBC1_LYZ_ZR_MODULE
...
!user function
!DEC$ ATTRIBUTES FORCEINLINE :: bounds_kernel_inflowBC1_LYZ_zr
...
subroutine bounds_kernel_inflowBC1_LYZ_zr_wrap( &
& opsDat1Local, &
& opsDat2Local, &
& opsDat3Local, &
& opsDat4Local, &
  ...
  DO n_z = 1, end(3)-start(3)+1
    DO n_y = 1, end(2)-start(2)+1
      ...
      DO n_x = 1, end(1)-start(1)+1
        call bounds_kernel_inflowBC1_LYZ_zr( &
...
!host subroutine
#ifndef OPS_LAZY
subroutine bounds_kernel_inflowBC1_LYZ_zr_host( userSubroutine, block, dim, range, &
& opsArg1, &
& opsArg2, &
& opsArg3, &
& opsArg4, &
...
#else
subroutine bounds_kernel_inflowBC1_LYZ_zr_host_execute( descPtr )
  use iso_c_binding
  IMPLICIT NONE
  ...
  call bounds_kernel_inflowBC1_LYZ_zr_wrap( &
  & opsDat1Local, &
  & opsDat2Local, &
  & opsDat3Local, &
  & opsDat4Local, &
...
& opsArg5)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 5, 456, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(bounds_kernel_inflowBC1_LYZ_zr_host_execute))
end subroutine
#endif
