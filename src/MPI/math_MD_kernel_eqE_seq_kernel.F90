...
!
MODULE MATH_MD_KERNEL_EQE_MODULE
...
!user function
!DEC$ ATTRIBUTES FORCEINLINE :: math_MD_kernel_eqE
...
subroutine math_MD_kernel_eqE_wrap( &
& opsDat1Local, &
& opsDat2Local, &
& opsDat3Local, &
& dat1_base, &
...
!host subroutine
#ifndef OPS_LAZY
subroutine math_MD_kernel_eqE_host( userSubroutine, block, dim, range, &
& opsArg1, &
& opsArg2, &
& opsArg3)
  IMPLICIT NONE
...
#else
subroutine math_MD_kernel_eqE_host_execute( descPtr )
  use iso_c_binding
  IMPLICIT NONE
  ...
  call setKernelTime(225,userSubroutine,0.0_8,0.0_8,0.0_4,1)
  ...
  transfer_total = transfer_total + transfer
  call setKernelTime(225,userSubroutine,t3-t2,t2-t1,transfer_total,0)
end subroutine
...
& opsArg3)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 3, 225, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(math_MD_kernel_eqE_host_execute))
end subroutine
#endif
