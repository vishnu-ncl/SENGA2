...
SUBROUTINE adaptt_kernel_err_eval_MD(err_arr, run_arr, ernrm, ispec, ertot)
    ...
    fornow = abs(err_arr(OPS_ACC_MD1(ispec,0,0,0)))/(abs(run_arr(OPS_ACC_MD2(ispec,0,0,0)))+ernrm(ispec))
...
& opsArg5)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 5, 317, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(adaptt_kernel_err_eval_MD_host_execute))
end subroutine
#endif
