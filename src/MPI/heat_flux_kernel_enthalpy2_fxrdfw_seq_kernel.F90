...
SUBROUTINE heat_flux_kernel_enthalpy2_fxrdfw(rate,erhs,vtmp,utmp,store7,arr1,acbcxr)
    use data_types
    implicit none
...
!  Set from kernel descriptor
  ...
  call setKernelTime(242,userSubroutine,0.0_8,0.0_8,0.0_4,1)
  ...
  transfer_total = transfer_total + transfer
  call setKernelTime(242,userSubroutine,t3-t2,t2-t1,transfer_total,0)
...
& opsArg7)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 7, 242, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(heat_flux_kernel_enthalpy2_fxrdfw_host_execute))
