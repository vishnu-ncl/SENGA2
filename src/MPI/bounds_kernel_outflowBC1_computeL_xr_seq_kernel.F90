...
MODULE BOUNDS_KERNEL_OUTFLOWBC1_COMPUTEL_XR_MODULE
...
!user function
!DEC$ ATTRIBUTES FORCEINLINE :: bounds_kernel_outflowBC1_computeL_xr
...
SUBROUTINE bounds_kernel_outflowBC1_computeL_xr(bcl1xr,struxr,acouxr,bcl5xr,strdxr,sorpxr,strpxr,cobcxr,pinfxr)
    ...
    real(kind=8), dimension(1) :: bcl1xr
    real(kind=8), dimension(1), intent(in) :: struxr,acouxr,bcl5xr,strdxr,sorpxr,strpxr
    real(kind=8), intent(in) :: cobcxr,pinfxr

    bcl1xr(OPS_ACC1(0,0,0)) = 0.5_8*(struxr(OPS_ACC2(0,0,0))-acouxr(OPS_ACC3(0,0,0)))  &
            *(bcl5xr(OPS_ACC4(0,0,0))-strdxr(OPS_ACC5(0,0,0))*acouxr(OPS_ACC3(0,0,0))*bcl1xr(OPS_ACC1(0,0,0)))

    bcl1xr(OPS_ACC1(0,0,0))= 0.5_8*sorpxr(OPS_ACC6(0,0,0))  &
...
subroutine bounds_kernel_outflowBC1_computeL_xr_wrap( &
& opsDat1Local, &
& opsDat2Local, &
& opsDat3Local, &
& opsDat4Local, &
  ...
  DO n_z = 1, end_indx(3)-start_indx(3)+1
    DO n_y = 1, end_indx(2)-start_indx(2)+1
      ...
      DO n_x = 1, end_indx(1)-start_indx(1)+1
        ...
        call bounds_kernel_outflowBC1_computeL_xr( &
...
!host subroutine
#ifndef OPS_LAZY
subroutine bounds_kernel_outflowBC1_computeL_xr_host( userSubroutine, block, dim, range, &
& opsArg1, &
& opsArg2, &
& opsArg3, &
& opsArg4, &
...
#else
subroutine bounds_kernel_outflowBC1_computeL_xr_host_execute( descPtr )
...
!  Set from kernel descriptor
  ...
  call bounds_kernel_outflowBC1_computeL_xr_wrap( &
  & opsDat1Local, &
  & opsDat2Local, &
  & opsDat3Local, &
  & opsDat4Local, &
...
& opsArg9)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 9, 354, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(bounds_kernel_outflowBC1_computeL_xr_host_execute))
