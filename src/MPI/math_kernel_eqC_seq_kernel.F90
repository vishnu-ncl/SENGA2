...
!user function
!DEC$ ATTRIBUTES FORCEINLINE :: math_kernel_eqC
...
subroutine math_kernel_eqC_wrap( &
& opsDat1Local, &
& opsDat2Local, &
& dat1_base, &
& dat2_base, &
...
!host subroutine
#ifndef OPS_LAZY
subroutine math_kernel_eqC_host( userSubroutine, block, dim, range, &
& opsArg1, &
& opsArg2)
  IMPLICIT NONE
...
#else
subroutine math_kernel_eqC_host_execute( descPtr )
  use iso_c_binding
  IMPLICIT NONE
...
& opsArg2)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 2, 300, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(math_kernel_eqC_host_execute))
end subroutine
#endif
