...
MODULE D2FDXY_KERNEL_RH_YDIR_CORNER_EQO_MODULE
...
!user function
!DEC$ ATTRIBUTES FORCEINLINE :: d2fdxy_kernel_rh_ydir_corner_eqO
...
subroutine d2fdxy_kernel_rh_ydir_corner_eqO_wrap( &
& opsDat1Local, &
& opsDat2Local, &
& dat1_base, &
& dat2_base, &
  ...
  DO n_z = 1, end_indx(3)-start_indx(3)+1
    DO n_y = 1, end_indx(2)-start_indx(2)+1
      ...
      DO n_x = 1, end_indx(1)-start_indx(1)+1
        ...
        call d2fdxy_kernel_rh_ydir_corner_eqO( &
...
!host subroutine
#ifndef OPS_LAZY
subroutine d2fdxy_kernel_rh_ydir_corner_eqO_host( userSubroutine, block, dim, range, &
& opsArg1, &
& opsArg2)
...
#else
subroutine d2fdxy_kernel_rh_ydir_corner_eqO_host_execute( descPtr )
...
!  Set from kernel descriptor
  ...
  call d2fdxy_kernel_rh_ydir_corner_eqO_wrap( &
  & opsDat1Local, &
  & opsDat2Local, &
  & dat1_base, &
  & dat2_base, &
...
& opsArg2)
  ...
  call create_kerneldesc_and_enque(userSubroutine//c_null_char, c_loc(opsArgArray), 2, 67, dim, 0, c_loc(range_tmp), block%blockCptr, c_funloc(d2fdxy_kernel_rh_ydir_corner_eqO_host_execute))
