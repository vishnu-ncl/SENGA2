...
C     DESCRIPTION
C     -----------
C     PREPROCESSES THERMOCHEMICAL DATA FOR PPDIFF
C
C     *************************************************************************
...
C          WRITE(NCREPT,'(A)')ALINE
          ...
          IF(ISTOTL.EQ.0)THEN
            ERRSTR = 'blank line in thermochemical data'
...
1200      CONTINUE
            IF(SPCFLG(ISPEC))THEN
              ...
              ENDIF
            ELSE
              ERRSTR ='end of thermochemical data file: data incomplete'
...
C             PROCEED TO READ THE SPECIES DATA
              ERRSTR = 'invalid thermochemical data format'
...
C              WRITE(NCREPT,'(A)')ALINE
              ...
              VALIDS = CHKSTR(PARSTR(1),LENSTR(1),ICHEND)
              IF(.NOT.VALIDS)THEN
                ERRSTR = 'thermochemical data file: missing END'
                CALL ERHAND(ERRSTR,IEFATL)
              ENDIF
...
C     CLOSE THE THERMOCHEMICAL DATA FILE
C     ----------------------------------
      CLOSE(NCTHMO)
...
C     FILE ERROR HANDLER
      GOTO 1030
1010  ERRSTR = 'Error opening thermochemical data file:'
      CALL ERHAND(ERRSTR,IEFATL)
1020  ERRSTR = 'unexpected end of thermochemical data file'
      CALL ERHAND(ERRSTR,IEFATL)
1030  CONTINUE
